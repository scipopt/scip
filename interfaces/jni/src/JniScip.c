   ...
   )
{
   ...
   assert(SCIP_OBJSENSE_MAXIMIZE == JNIPACKAGENAME(JniScipObjSense_SCIP_MAXIMIZE));
   assert(SCIP_OBJSENSE_MINIMIZE == JNIPACKAGENAME(JniScipObjSense_SCIP_MINIMIZE));

   /* check if the VARTYPE enums of JNI SCIP are mapped correctly to the ones of SCIP */
   assert(SCIP_VARTYPE_BINARY == JNIPACKAGENAME(JniScipVarType_SCIP_BINARY));
   assert(SCIP_VARTYPE_INTEGER == JNIPACKAGENAME(JniScipVarType_SCIP_INTEGER));
   assert(SCIP_VARTYPE_IMPLINT == JNIPACKAGENAME(JniScipVarType_SCIP_IMPLINT));
   assert(SCIP_VARTYPE_CONTINUOUS == JNIPACKAGENAME(JniScipVarType_SCIP_CONTINUOUS));

   /* check if the VARSTATUS enums of JNI SCIP are mapped correctly to the ones of SCIP */
   assert(SCIP_VARSTATUS_ORIGINAL == JNIPACKAGENAME(JniScipVarStatus_SCIP_ORIGINAL));
   assert(SCIP_VARSTATUS_LOOSE == JNIPACKAGENAME(JniScipVarStatus_SCIP_LOOSE));
   assert(SCIP_VARSTATUS_COLUMN == JNIPACKAGENAME(JniScipVarStatus_SCIP_COLUMN));
   assert(SCIP_VARSTATUS_FIXED == JNIPACKAGENAME(JniScipVarStatus_SCIP_FIXED));
   assert(SCIP_VARSTATUS_AGGREGATED == JNIPACKAGENAME(JniScipVarStatus_SCIP_AGGREGATED));
   assert(SCIP_VARSTATUS_MULTAGGR == JNIPACKAGENAME(JniScipVarStatus_SCIP_MULTAGGR));
   assert(SCIP_VARSTATUS_NEGATED == JNIPACKAGENAME(JniScipVarStatus_SCIP_NEGATED));
