...
static
SCIP_RETCODE lpiExactStrongbranch(
   SCIP_LPIEXACT*        lpi,                /**< LP interface structure */
   ...
   )
{
    ...
    */
   if( lpi->checkcondition && (SCIPlpiExactIsOptimal(lpi) || SCIPlpiExactIsObjlimExc(lpi)) )
   {
      SCIP_RETCODE retcode;
      SCIP_Real kappa;

      retcode = SCIPlpiExactGetRealSolQuality(lpi, SCIP_LPSOLQUALITY_ESTIMCONDITION, &kappa);
 ...
 */
SCIP_RETCODE SCIPlpiExactGetRealSolQuality(
   SCIP_LPIEXACT*        lpi,                /**< LP interface structure */
...
{
   SCIPdebugMessage("calling SCIPlpiExactGetRealSolQuality()\n");
   ...
   )
{
   ...
   SCIPdebugMessage("calling SCIPlpiExactGetBInvARow()\n");
   ...
   if( binvrow == NULL )
   {
      ...
      SCIP_CALL( SCIPlpiExactGetBInvRow(lpi, r, buf, inds, ninds) );
      binv = buf;
   }
   else
...
{  /*lint --e{715}*/
   ...
   SCIPdebugMessage("calling SCIPlpiExactGetBInvACol()\n");
 ...
 *  columns and rows since the state was stored with SCIPlpiExactGetNorms()
 */
