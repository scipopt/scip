...
struct SCIP_ConsData
{
   ...
   SCIP_Rational*        minactivityEx;        /**< minimal value w.r.t. the variable's local bounds for the constraint's
                                              *   activity, ignoring the coefficients contributing with infinite value */
   SCIP_Rational*        maxactivityEx;        /**< maximal value w.r.t. the variable's local bounds for the constraint's
                                              *   activity, ignoring the coefficients contributing with infinite value */
   SCIP_Rational*        glbminactivityEx;     /**< minimal value w.r.t. the variable's global bounds for the constraint's
                                              *   activity, ignoring the coefficients contributing with infinite value */
   SCIP_Rational*        glbmaxactivityEx;     /**< maximal value w.r.t. the variable's global bounds for the constraint's
                                              *   activity, ignoring the coefficients contributing with infinite value */
   SCIP_Rational*        lastglbminactivityEx; /**< last global minimal activity which was computed by complete summation
                                              *   over all contributing values */
   SCIP_Rational*        lastglbmaxactivityEx; /**< last global maximal activity which was computed by complete summation
                                              *   over all contributing values */
   SCIP_Rational*        maxactdeltaEx;        /**< maximal activity contribution of a single variable, or SCIP_INVALID if invalid */
   SCIP_VAR*             maxactdeltavarEx;     /**< variable with maximal activity contribution, or NULL if invalid */
   ...
   unsigned int computeactivities:1;
   ...
   unsigned int          indexsorted : 1;    /**< are binary variables sorted w.r.t. the
                                                absolute of their coefficient? */
...
/** Uniform way to store the vars and vals of a constraint as they appear in the certificate. */
struct CertificateCons {
   SCIP_Rational*        rhs;
   SCIP_Rational*        lhs;
   SCIP_Real             lhsreal;
   SCIP_Real             rhsreal;
   SCIP_Rational**       vals;
   SCIP_INTERVAL*        valsreal;
   SCIP_Rational*        constantEx;
...
static CERTIFICATE_CONS GetCertificateCons(SCIP* scip, SCIP_CONSDATA* consdata)
{
   ...
   {
      ret.lhs = consdata->lhs;
      ret.rhs = consdata->rhs;
      ret.lhsreal = consdata->lhsreal;
      ret.rhsreal = consdata->rhsreal;
      ret.vals = consdata->vals;
      ret.nvars = consdata->nvars;
      ret.valsreal = consdata->valsreal;
      ret.constantEx = NULL;
      SCIPintervalSet(&ret.constant, 0);
   }
   return ret;
}
...
static SCIP_VAR* GetCertificateVar(SCIP_CONSDATA* consdata, int index)
{
   ...
   if (consdata->rowexact != NULL)
   {
      assert(index < consdata->rowexact->len);
      ret = consdata->rowexact->cols[index]->var;
   }
   else
   {
      assert(index < consdata->nvars);
      ret = consdata->vars[index];
   }
   return ret;
}
   ...
   )
{
   ...
   if( num > consdata->varssize )
   {
      ...
      for( k = consdata->varssize; k < newsize; ++k )
         SCIP_CALL( RatCreateBlock(SCIPblkmem(scip), &consdata->vals[k]) );
...
SCIP_RETCODE linconsupgradeCreate(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_EXLINCONSUPGRADE** linconsupgrade,     /**< pointer to store the linear constraint upgrade */
   SCIP_DECL_EXLINCONSUPGD((*linconsupgd)),    /**< method to call for upgrading linear constraint */
   int                   priority            /**< priority of upgrading method */
   )
{
   ...
   )
{
   ...
   SCIP_CALL( RatCreateBlock(SCIPblkmem(scip), &(*conshdlrdata)->mingainpernmincomp) );
   ...
   )
{
   ...
   RatFreeBlock(SCIPblkmem(scip), &(*conshdlrdata)->maxcardbounddist);
   RatFreeBlock(SCIPblkmem(scip), &(*conshdlrdata)->maxeasyactivitydelta);
   RatFreeBlock(SCIPblkmem(scip), &(*conshdlrdata)->mingainpernmincomp);
   ...
   )
{
   ...
   if( RatIsPositive(val) )
   {
      SCIP_CALL( SCIPlockVarCons(scip, var, cons,
         !RatIsNegInfinity(consdata->lhs), !RatIsInfinity(consdata->rhs)) );
   }
   else
   {
      SCIP_CALL( SCIPlockVarCons(scip, var, cons,
         !RatIsInfinity(consdata->rhs), !RatIsNegInfinity(consdata->lhs)) );
   }
   ...
   )
{
   ...
   if( RatIsPositive(val) )
   {
      SCIP_CALL( SCIPunlockVarCons(scip, var, cons, !RatIsNegInfinity(consdata->lhs),
         !RatIsInfinity(consdata->rhs)) );
   }
   else
   {
      SCIP_CALL( SCIPunlockVarCons(scip, var, cons, !RatIsInfinity(consdata->rhs),
         !RatIsNegInfinity(consdata->lhs)) );
   }
...
/** creates roweventdata for variable at given position in the row, and catches events */
/**! [SnippetDebugAssertions] */
static
SCIP_RETCODE consCatchRowEvent(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONS*            cons,               /**< linear constraint */
   SCIP_EVENTHDLR*       eventhdlr,          /**< event handler to call for the event processing */
   int                   pos                 /**< array position of variable to catch bound change events for */
   )
{
   ...
   assert(0 <= pos && pos < consdata->rowexact->len);
   assert(consdata->rowexact != NULL);
   assert(consdata->rowexact->cols != NULL);
   assert(consdata->rowexact->cols[pos] != NULL);
   assert(SCIPvarIsTransformed(consdata->rowexact->cols[pos]->var));
   assert(consdata->roweventdata[pos] == NULL);

   SCIP_CALL( SCIPallocBlockMemory(scip, &(consdata->roweventdata[pos])) ); /*lint !e866*/
   consdata->roweventdata[pos]->cons = cons;
   consdata->roweventdata[pos]->varpos = pos;
   consdata->roweventdata[pos]->rowvar = true;

   SCIP_CALL( SCIPcatchVarEvent(scip, consdata->rowexact->cols[pos]->var,
         SCIP_EVENTTYPE_BOUNDCHANGED | SCIP_EVENTTYPE_VARFIXED | SCIP_EVENTTYPE_VARUNLOCKED
         | SCIP_EVENTTYPE_GBDCHANGED | SCIP_EVENTTYPE_VARDELETED,
         eventhdlr, consdata->roweventdata[pos], &consdata->roweventdata[pos]->filterpos) );
   ...
   )
{
   ...
   assert(consdata->rowexact->cols != NULL);
   assert(consdata->rowexact->cols[pos] != NULL);
   assert(consdata->roweventdata != NULL);
   assert(consdata->roweventdata[pos] != NULL);
   assert(consdata->roweventdata[pos]->cons == cons);
   assert(consdata->roweventdata[pos]->varpos == pos);

   SCIP_CALL( SCIPdropVarEvent(scip, consdata->rowexact->cols[pos]->var,
         SCIP_EVENTTYPE_BOUNDCHANGED | SCIP_EVENTTYPE_VARFIXED | SCIP_EVENTTYPE_VARUNLOCKED
         | SCIP_EVENTTYPE_GBDCHANGED | SCIP_EVENTTYPE_VARDELETED,
         eventhdlr, consdata->roweventdata[pos], consdata->roweventdata[pos]->filterpos) );

   SCIPfreeBlockMemory(scip, &consdata->roweventdata[pos]); /*lint !e866*/
   ...
   )
{
   ...
   if( nvars > 0 )
   {
      ...
      SCIP_CALL( RatCreateBufferArray(SCIPbuffer(scip), &valsbuffer, nvars) );
      ...
      for( v = 0; v < nvars; ++v )
      {
         ...
         if( !RatIsZero(vals[v]) )
         {
            ...
            if( SCIPisConsCompressionEnabled(scip) && RatIsEqual(SCIPvarGetLbGlobalExact(var), SCIPvarGetUbGlobalExact(var)) )
            {
               RatAddProd(constant, vals[v], SCIPvarGetLbGlobalExact(var));
            }
            else
            {
               varsbuffer[k] = var;
      ...
      RatFreeBufferArray(SCIPbuffer(scip), &valsbuffer, nvars);
   ...
   SCIP_CALL( RatCreateString(SCIPblkmem(scip), &(*consdata)->minabsvalEx, "inf") );
   SCIP_CALL( RatCreateString(SCIPblkmem(scip), &(*consdata)->minactivityEx, "inf") );
   SCIP_CALL( RatCreateString(SCIPblkmem(scip), &(*consdata)->maxactivityEx, "inf") );
   SCIP_CALL( RatCreateString(SCIPblkmem(scip), &(*consdata)->maxactdeltaEx, "inf") );
   (*consdata)->maxactdeltavarEx = NULL;
   ...
   SCIP_CALL( RatCreateString(SCIPblkmem(scip), &(*consdata)->glbminactivityEx, "inf") );
   SCIP_CALL( RatCreateString(SCIPblkmem(scip), &(*consdata)->glbmaxactivityEx, "inf") );
   SCIP_CALL( RatCreateString(SCIPblkmem(scip), &(*consdata)->lastglbminactivityEx, "inf") );
   SCIP_CALL( RatCreateString(SCIPblkmem(scip), &(*consdata)->lastglbmaxactivityEx, "inf") );
   ...
   (*consdata)->computeactivities = FALSE;
   ...
   for( v = 0; v < (*consdata)->nvars; v++ )
   {
      ...
      assert(!RatIsZero((*consdata)->vals[v]));
   ...
   )
{
   ...
   RatFreeBlockArray(SCIPblkmem(scip), &(*consdata)->vals, (*consdata)->varssize);
   ...
   RatFreeBlock(SCIPblkmem(scip), &(*consdata)->lhs);
   RatFreeBlock(SCIPblkmem(scip), &(*consdata)->rhs);
   RatFreeBlock(SCIPblkmem(scip), &(*consdata)->maxabsvalEx);
   RatFreeBlock(SCIPblkmem(scip), &(*consdata)->minabsvalEx);
   RatFreeBlock(SCIPblkmem(scip), &(*consdata)->minactivityEx);
   RatFreeBlock(SCIPblkmem(scip), &(*consdata)->maxactivityEx);
   RatFreeBlock(SCIPblkmem(scip), &(*consdata)->maxactdeltaEx);
   RatFreeBlock(SCIPblkmem(scip), &(*consdata)->glbminactivityEx);
   RatFreeBlock(SCIPblkmem(scip), &(*consdata)->glbmaxactivityEx);
   RatFreeBlock(SCIPblkmem(scip), &(*consdata)->lastglbminactivityEx);
   RatFreeBlock(SCIPblkmem(scip), &(*consdata)->lastglbmaxactivityEx);
   ...
   )
{
   ...
   else
   {
      ...
      for( v = 0; v < consdata->nvars; ++v )
      {
         ...
         {
            if( RatIsEqualReal(consdata->vals[v], 1.0) )
            {
               if( v > 0 )
                  SCIPinfoMessage(scip, file, " +");
            }
            else if( RatIsEqualReal(consdata->vals[v], -1.0) )
               SCIPinfoMessage(scip, file, " -");
            else
            {
   ...
   )
{
   ...
   consdata->maxactdelta = SCIP_INVALID;
   consdata->maxactdeltavar = NULL;
   RatSetString(consdata->minactivityEx, "inf");
   RatSetString(consdata->maxactivityEx, "inf");
   RatSetString(consdata->maxabsvalEx, "inf");
   RatSetString(consdata->minabsvalEx, "inf");
   RatSetString(consdata->maxactdeltaEx, "inf");
   consdata->maxactdeltavarEx = NULL;
   consdata->minactivityneginf = -1;
   consdata->minactivityposinf = -1;
   consdata->maxactivityneginf = -1;
   ...
   consdata->lastglbminactivity = SCIP_INVALID;
   consdata->lastglbmaxactivity = SCIP_INVALID;
   RatSetString(consdata->glbminactivityEx, "inf");
   RatSetString(consdata->glbmaxactivityEx, "inf");
   RatSetString(consdata->lastglbminactivityEx, "inf");
   RatSetString(consdata->lastglbmaxactivityEx, "inf");
   consdata->glbminactivityneginf = -1;
   consdata->glbminactivityposinf = -1;
   consdata->glbmaxactivityneginf = -1;
   consdata->glbmaxactivityposinf = -1;
   ...
   )
{
   ...
   for( i = consdata->nvars - 1; i >= 0; --i )
   {
      ...
      else
      {
         ...
         {
            RatAddProd(pseudoactivity, val, bound);
         }
      }
   }
   ...
   )
{
   ...
   {
      bound = (ccons.valsreal[i].inf > 0.0 ) ? SCIPvarGetLbLocal(GetCertificateVar(consdata, i)) : SCIPvarGetUbLocal(GetCertificateVar(consdata, i));
      if( !SCIPisInfinity(scip, bound) && !SCIPisInfinity(scip, -bound)
         && !SCIPisHugeValue(scip, ccons.valsreal[i].inf * bound) && !SCIPisHugeValue(scip, -ccons.valsreal[i].inf * bound) )
         consdata->minactivity += ccons.valsreal[i].inf * bound;
   }
   ...
   )
{
   ...
   for( i = ccons.nvars - 1; i >= 0; --i )
   {
      bound = (ccons.valsreal[i].sup > 0.0 ) ? SCIPvarGetUbLocal(GetCertificateVar(consdata, i)) : SCIPvarGetLbLocal(GetCertificateVar(consdata, i));
      if( !SCIPisInfinity(scip, bound) && !SCIPisInfinity(scip, -bound)
         && !SCIPisHugeValue(scip, ccons.valsreal[i].sup * bound) && !SCIPisHugeValue(scip, -ccons.valsreal[i].sup * bound) )
         consdata->maxactivity += ccons.valsreal[i].sup * bound;
   }
...
/** recompute the minactivity of a constraint */
static
void consdataRecomputeMinactivityEx(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONSDATA*        consdata            /**< linear constraint data */
   )
{
   ...
   if (ccons.constantEx == NULL)
      ...
      RatSet(consdata->minactivityEx, ccons.constantEx);
   #ifdef SCIP_DEBUG
   int minactivityneginf = 0;
   int minactivityposinf = 0;
   #endif
   for( i = ccons.nvars - 1; i >= 0; --i )
   {
      if (RatIsPositive(ccons.vals[i])) {
         SCIPvarGetLbLocalExactMaximal(GetCertificateVar(consdata, i), bound);
      } else {
         SCIPvarGetUbLocalExactMinimal(GetCertificateVar(consdata, i), bound);
      }
      if( !RatIsAbsInfinity(bound) )
      {
         RatAddProd(consdata->minactivityEx, ccons.vals[i], bound);
...
/** recompute the maxactivity of a constraint */
static
void consdataRecomputeMaxactivityEx(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONSDATA*        consdata            /**< linear constraint data */
   )
{
   ...
   if (ccons.constantEx == NULL)
      ...
      RatSet(consdata->maxactivityEx, ccons.constantEx);
   #ifdef SCIP_DEBUG
   int maxactivityneginf = 0;
   int maxactivityposinf = 0;
   #endif
   for( i = ccons.nvars - 1; i >= 0; --i )
   {
      if (RatIsPositive(ccons.vals[i])) {
      ...
      if( !RatIsAbsInfinity(bound) )
      {
         RatAddProd(consdata->maxactivityEx, ccons.vals[i], bound);
      }
      #ifdef SCIP_DEBUG
      else if ( RatIsPositive(bound) )
      {
   ...
   )
{
   ...
   for( i = ccons.nvars - 1; i >= 0; --i )
   {
      bound = (ccons.valsreal[i].inf > 0.0 ) ? SCIPvarGetLbGlobal(GetCertificateVar(consdata, i)) : SCIPvarGetUbGlobal(GetCertificateVar(consdata, i));
      if( !SCIPisInfinity(scip, bound) && !SCIPisInfinity(scip, -bound)
         && !SCIPisHugeValue(scip, ccons.valsreal[i].inf * bound) && !SCIPisHugeValue(scip, -ccons.valsreal[i].inf * bound) )
         consdata->glbminactivity += ccons.valsreal[i].inf * bound;
   }
   ...
   )
{
   ...
   for( i = ccons.nvars - 1; i >= 0; --i )
   {
      bound = (ccons.valsreal[i].sup > 0.0 ) ? SCIPvarGetUbGlobal(GetCertificateVar(consdata, i)) : SCIPvarGetLbGlobal(GetCertificateVar(consdata, i));
      if( !SCIPisInfinity(scip, bound) && !SCIPisInfinity(scip, -bound)
         && !SCIPisHugeValue(scip, ccons.valsreal[i].sup * bound) && !SCIPisHugeValue(scip, -ccons.valsreal[i].sup * bound) )
         consdata->glbmaxactivity += ccons.valsreal[i].sup * bound;
   }
   ...
   )
{
   ...
   if (ccons.constantEx == NULL)
      ...
      RatSet(consdata->glbminactivityEx, ccons.constantEx);

   for( i = ccons.nvars - 1; i >= 0; --i )
   {
      bound = RatIsPositive(ccons.vals[i]) ? SCIPvarGetLbGlobalExact(GetCertificateVar(consdata, i)) : SCIPvarGetUbGlobalExact(GetCertificateVar(consdata, i));
      if( !RatIsAbsInfinity(bound) )
      {
         RatAddProd(consdata->glbminactivityEx, ccons.vals[i], bound);
...
/** recompute the global maxactivity of a constraint */
static
void consdataRecomputeGlbMaxactivityEx(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONSDATA*        consdata            /**< linear constraint data */
   )
{
   ...
   if (ccons.constantEx == NULL)
      ...
      RatSet(consdata->glbmaxactivityEx, ccons.constantEx);

   for( i = ccons.nvars - 1; i >= 0; --i )
   {
      bound = RatIsPositive(ccons.vals[i]) ? SCIPvarGetUbGlobalExact(GetCertificateVar(consdata, i)) : SCIPvarGetLbGlobalExact(GetCertificateVar(consdata, i));
      if( RatIsAbsInfinity(bound) )
      {
         RatAddProd(consdata->glbmaxactivityEx, ccons.vals[i], bound);
   ...
   )
{
   ...
   RatSetReal(consdata->maxabsvalEx, 0.0);
   for( i = 0; i < ccons.nvars; ++i )
   {
      if( RatIsAbsGT(ccons.vals[i], consdata->maxabsvalEx) )
   ...
   )
{
   ...
   if( ccons.nvars > 0 )
   {
      RatAbs(consdata->minabsvalEx, ccons.vals[0]);
      assert(!RatIsZero(ccons.vals[0]));
   }
   else
      RatSetReal(consdata->minabsvalEx, 0.0);

   for( i = 1; i < ccons.nvars; ++i )
   {
      assert(!RatIsZero(consdata->vals[i]));

      if( RatIsAbsGT(consdata->minabsvalEx, consdata->vals[i]) )
         RatAbs(consdata->minabsvalEx, consdata->vals[i]);
   }
}

/** calculates maximum absolute value of coefficients */
static
void consdataCalcMaxAbsval(SCIP* scip,
   SCIP_CONSDATA*        consdata            /**< linear constraint data */
   )
{
   SCIP_Real absval;
   ...
   for( i = 0; i < ccons.nvars; ++i )
   {
      absval = SCIPintervalAbsMax(ccons.valsreal[i]);
...
/** calculates minimum absolute value of coefficients */
static
void consdataCalcMinAbsval(SCIP* scip,
   SCIP_CONSDATA*        consdata            /**< linear constraint data */
   )
{
   SCIP_Real absval;
   ...
   if( ccons.nvars > 0 )
      consdata->minabsval = SCIPintervalAbsMax(ccons.valsreal[0]);
   else
   ...
   )
{
   ...
   {
      SCIP_Ratoinal* SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &maxactdelta );
      SCIP_Ratoinal* SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &domain );
      SCIP_Ratoinal* SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &delta );
      SCIP_Rational* lb;
      SCIP_Rational* ub;
      int v;
      ...
      for( v = consdata->nvars - 1; v >= 0; --v )
      {
         ...
         {
           RatSet(maxactdelta, delta);
         }
      }
      assert(RatIsEqual(maxactdelta, consdata->maxactdelta));

      RatFreeBuffer(SCIPbuffer(scip), delta);
      RatFreeBuffer(SCIPbuffer(scip), domain);
      RatFreeBuffer(SCIPbuffer(scip), maxactdelta);   }
}
#else
   ...
   )
{
   ...
   {
      for( v = ccons.nvars - 1; v >= 0; --v )
      {
         if( SCIPvarGetLbLocal(GetCertificateVar(consdata, v)) < 0.5 && SCIPvarGetUbLocal(GetCertificateVar(consdata, v)) > 0.5 )
         {
            delta = SCIPintervalAbsMax(ccons.valsreal[v]);

            if( delta > consdata->maxactdelta )
            {
               consdata->maxactdelta = delta;
               consdata->maxactdeltavar = GetCertificateVar(consdata, v);
            }
         }
      }
      return;
   ...
   for( v = ccons.nvars - 1; v >= 0; --v )
   {
      ...
      lb = SCIPvarGetLbLocal(GetCertificateVar(consdata, v));
      ub = SCIPvarGetUbLocal(GetCertificateVar(consdata, v));
      ...
      domain = ub - lb;
      delta = SCIPintervalAbsMax(ccons.valsreal[v]) * domain;
...
/** recompute maximal activity contribution for a single variable */
static
SCIP_RETCODE consdataRecomputeMaxActivityDeltaEx(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONSDATA*        consdata            /**< linear constraint data */
   )
{
   ...
   RatSetReal(consdata->maxactdeltaEx, 0.0);
   ...
   if( !consdata->hasnonbinvar )
   {
      for( v = ccons.nvars - 1; v >= 0; --v )
      {
         if( SCIPvarGetLbLocal(GetCertificateVar(consdata, v)) < 0.5 && SCIPvarGetUbLocal(GetCertificateVar(consdata, v)) > 0.5 )
         {
            RatAbs(delta, consdata->vals[v]);

            if( RatIsGT(delta,consdata->maxactdeltaEx) )
            {
               RatSet(consdata->maxactdeltaEx, delta);
               consdata->maxactdeltavarEx = GetCertificateVar(consdata, v);
            }
         }
      }
   ...
   for( v = ccons.nvars - 1; v >= 0; --v )
   {
      ...
      lb = SCIPvarGetLbLocalExact(GetCertificateVar(consdata, v));
      ub = SCIPvarGetUbLocalExact(GetCertificateVar(consdata, v));

      if( RatIsNegInfinity(lb) || RatIsInfinity(ub) )
      {
         RatSetString(consdata->maxactdeltaEx, "inf");
         consdata->maxactdeltavarEx = GetCertificateVar(consdata, v);
         break;
      ...
      RatDiff(domain, ub, lb);
      RatAbs(delta, ccons.vals[v]);
      RatMult(delta, delta, domain);
...
static
SCIP_RETCODE consdataUpdateActivitiesEx(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONSDATA*        consdata,           /**< linear constraint data */
   SCIP_VAR*             var,                /**< variable that has been changed; can be NULL for global bound changes */
   SCIP_Rational*        oldbound,           /**< old bound of variable */
   SCIP_Rational*        newbound,           /**< new bound of variable */
   SCIP_Rational*        val,                /**< coefficient of constraint entry */
   ...
   )
{
   ...
   SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &oldcontribution) );
   SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &newcontribution) );
   SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &delta) );
   ...
   if( global )
   {
      ...
      if( boundtype == SCIP_BOUNDTYPE_LOWER )
      {
         if( RatIsPositive(val) )
         {
            activity = (consdata->glbminactivityEx);
            lastactivity = (consdata->lastglbminactivityEx);
         ...
         {
            activity = (consdata->glbmaxactivityEx);
            lastactivity = (consdata->lastglbmaxactivityEx);
   ...
   if ( oldbound != NULL )
   {
      RatMult(oldcontribution, val, oldbound);
   }
   else
   {
      RatSetInt(oldcontribution, 0, 1);
   }
   RatMult(newcontribution, val, newbound);
   finitenewbound = !RatIsAbsInfinity(newbound);

   if( oldbound != NULL && RatIsAbsInfinity(oldbound) )
   {
      /* old bound was +infinity */
      if( RatIsPositive(oldbound) )
      {
         assert((*activityposinf) >= 1);

         /* we only have to do something if the new bound is not again +infinity */
         if( finitenewbound || RatIsNegative(newbound) )
         {
            ...
            /* "normal case": just add the contribution to the activity */
               ...
               RatSet(delta, newcontribution);
         }
      }
      /* old bound was -infinity */
      else
      {
         assert(!RatIsPositive(oldbound));
         assert((*activityneginf) >= 1);

         /* we only have to do something ig the new bound is not again -infinity */
         if( finitenewbound || RatIsPositive(newbound) )
         {
   ...
   else
   {
      if( !finitenewbound )
      {
          ...
          */
         if( RatIsPositive(newbound) )
         {
            (*activityposinf)++;
            RatNegate(delta, oldcontribution);
         }
         /* if the new bound is -infinity, the old contribution has to be subtracted
      ...
      /* "normal case": just update the activity */
         ...
         RatDiff(delta, newcontribution, oldcontribution);
   }
   ...
   if( validact && !RatIsZero(delta) && finitenewbound )
   {
       ...
       */
      RatAdd(activity, activity, delta);
      assert(!RatIsAbsInfinity(activity));

      RatSet(lastactivity, activity);
   }

   RatFreeBuffer(SCIPbuffer(scip), &delta);
   RatFreeBuffer(SCIPbuffer(scip), &newcontribution);
   RatFreeBuffer(SCIPbuffer(scip), &oldcontribution);
...
/** updates minimum and maximum activity for a change in lower bound */
static
void consdataUpdateActivitiesLbEx(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONSDATA*        consdata,           /**< linear constraint data */
   SCIP_VAR*             var,                /**< variable that has been changed */
   ...
   )
{
   ...
   if( consdata->validactivities )
   {
      consdataUpdateActivitiesEx(scip, consdata, var, oldlb, newlb, val, SCIP_BOUNDTYPE_LOWER, FALSE, checkreliability);

      assert(!RatIsAbsInfinity(consdata->minactivityEx));
      assert(!RatIsAbsInfinity(consdata->maxactivityEx));
   }
}

/** updates minimum and maximum activity for a change in upper bound */
static
void consdataUpdateActivitiesUbEx(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONSDATA*        consdata,           /**< linear constraint data */
   SCIP_VAR*             var,                /**< variable that has been changed */
   ...
   )
{
   ...
   if( consdata->validactivities )
   {
      consdataUpdateActivitiesEx(scip, consdata, var, oldub, newub, val, SCIP_BOUNDTYPE_UPPER, FALSE, checkreliability);
...
static
void consdataUpdateActivitiesGlbLbEx(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONSDATA*        consdata,           /**< linear constraint data */
   SCIP_Rational*        oldlb,              /**< old lower bound of variable */
   SCIP_Rational*        newlb,              /**< new lower bound of variable */
   ...
   )
{
   ...
   if( consdata->validactivities )
   {
      consdataUpdateActivitiesEx(scip, consdata, NULL, oldlb, newlb, val, SCIP_BOUNDTYPE_LOWER, TRUE, checkreliability);

      assert(!RatIsAbsInfinity(consdata->glbminactivityEx));
      assert(!RatIsAbsInfinity(consdata->glbmaxactivityEx));
   }
}

/** updates minimum and maximum global activity for a change in global upper bound */
static
void consdataUpdateActivitiesGlbUbEx(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONSDATA*        consdata,           /**< linear constraint data */
   SCIP_Rational*        oldub,              /**< old upper bound of variable */
   SCIP_Rational*        newub,              /**< new upper bound of variable */
   ...
   )
{
   ...
   if( consdata->validactivities )
   {
      consdataUpdateActivitiesEx(scip, consdata, NULL, oldub, newub, val, SCIP_BOUNDTYPE_UPPER, TRUE, checkreliability);
   ...
   )
{
   ...
   {
      if( RatIsAbsGT(val, consdata->maxabsvalEx) )
         RatAbs(consdata->maxabsvalEx, val);
   }

   if( consdata->validminabsval )
   {
      if( RatIsAbsGT(consdata->minabsvalEx, val) )
         RatAbs(consdata->minabsvalEx, val);
   }
   ...
   if( consdata->validactivities )
   {
      ...
      consdataUpdateActivitiesLbEx(scip, consdata, var, NULL, tmpBound, val, checkreliability);

      SCIPvarGetUbLocalExactMinimal(var, tmpBound);
      consdataUpdateActivitiesUbEx(scip, consdata, var, NULL, tmpBound, val, checkreliability);

      consdataUpdateActivitiesGlbLbEx(scip, consdata, NULL, SCIPvarGetLbGlobalExact(var), val, checkreliability);
      consdataUpdateActivitiesGlbUbEx(scip, consdata, NULL, SCIPvarGetUbGlobalExact(var), val, checkreliability);
   ...
   )
{
   ...
   if( consdata->validmaxabsval )
   {
      if( RatIsAbsEqual(val, consdata->maxabsvalEx) )
      {
         consdata->validmaxabsval = FALSE;
         RatSetString(consdata->maxabsvalEx, "inf");
      }
   ...
   if( consdata->validminabsval )
   {
      if( RatIsAbsEqual(val, consdata->minabsvalEx) )
      {
         consdata->validminabsval = FALSE;
         RatSetString(consdata->minabsvalEx, "inf");
      }
   ...
   if( consdata->validactivities )
   {
      consdataUpdateActivitiesLbEx(scip, consdata, var, SCIPvarGetLbLocalExact(var), NULL, val, checkreliability);
      consdataUpdateActivitiesUbEx(scip, consdata, var, SCIPvarGetUbLocalExact(var), NULL, val, checkreliability);
      consdataUpdateActivitiesGlbLbEx(scip, consdata, SCIPvarGetLbGlobalExact(var), NULL, val, checkreliability);
      consdataUpdateActivitiesGlbUbEx(scip, consdata, SCIPvarGetUbGlobalExact(var), NULL, val, checkreliability);
   }
}
   ...
   )
{
   ...
   if( consdata->validmaxabsval )
   {
      ...
      absval = MAX(REALABS(val.inf), REALABS(val.sup));
...
static
void consdataUpdateChgCoefEx(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONSDATA*        consdata,           /**< linear constraint data */
   SCIP_VAR*             var,                /**< variable of constraint entry */
   SCIP_Rational*        oldval,             /**< old coefficient of constraint entry */
   SCIP_Rational*        newval,             /**< new coefficient of constraint entry */
   SCIP_Bool             checkreliability    /**< should the reliability of the recalculated activity be checked? */
   )
{
   assert(scip != NULL);
   ...
   assert(!RatIsZero(oldval));

   /* new zero coefficients should be handled by consdataUpdateDelCoef() */
   assert(!RatIsZero(newval));

   /* update maximum absolute value */
   if( consdata->validmaxabsval )
   {
      if( RatIsAbsGT(newval, consdata->maxabsvalEx) )
      {
         RatAbs(consdata->maxabsvalEx, newval);
      }
      else
      {
         /* invalidate maximum absolute value */
         if( RatIsAbsEqual(oldval, consdata->maxabsvalEx) )
         {
            consdata->validmaxabsval = FALSE;
            RatSetString(consdata->maxabsvalEx, "inf");
         }
   ...
   if( consdata->validminabsval )
   {
      if( RatIsAbsGT(consdata->minabsvalEx, newval) )
      {
         RatAbs(consdata->minabsvalEx, newval);
      }
      else
      {
         /* invalidate minimum absolute value */
         if( RatIsAbsEqual(newval, consdata->minabsvalEx) )
         {
            consdata->validminabsval = FALSE;
            RatSetString(consdata->minabsvalEx, "inf");
         }
   ...
   if( !RatIsInfinity(consdata->maxactdeltaEx) )
   {
      SCIP_Rational* domain;
      SCIP_Rational* delta;
      ...
      RatCreateBuffer(SCIPbuffer(scip), &domain);
      RatCreateBuffer(SCIPbuffer(scip), &delta);

      RatDiff(domain, SCIPvarGetUbLocalExact(var), SCIPvarGetLbLocalExact(var));
      RatMult(delta, newval, domain);

      if( RatIsAbsGT(delta, consdata->maxactdeltaEx) )
      {
         RatAbs(consdata->maxactdeltaEx, delta);
         consdata->maxactdeltavarEx = var;
      }
      else
      {
         /* reset maximal activity delta, so that it will be recalculated on the next real propagation */
         if( consdata->maxactdeltavarEx == var )
            RatSetString(consdata->maxactdeltaEx, "inf");
      }
   ...
   consdataUpdateDelCoefEx(scip, consdata, var, oldval, checkreliability);
   consdataUpdateAddCoefEx(scip, consdata, var, newval, checkreliability);
}

/** returns the maximum absolute value of all coefficients in the constraint */
static
SCIP_Rational* consdataGetMaxAbsvalEx(
   SCIP*                 scip,
   SCIP_CONSDATA*        consdata            /**< linear constraint data */
   )
{
   ...
   if( !consdata->validmaxabsval )
      consdataCalcMaxAbsvalEx(scip, consdata);
   assert(consdata->validmaxabsval);

   return consdata->maxabsvalEx;
}
   ...
   )
{
   ...
   if( !consdata->validminabsval )
      consdataCalcMinAbsval(scip, consdata);
   assert(consdata->validminabsval);
   assert(consdata->minabsval < SCIP_INVALID);
   ...
   )
{
   ...
   consdata->validglbmaxact = TRUE;
   RatSetReal(consdata->maxabsvalEx, 0.0);
   RatSetReal(consdata->minabsvalEx, ccons.nvars == 0 ? 0.0 : REALABS(ccons.valsreal[0].inf));
   if (ccons.constantEx == NULL)
   {
      RatSetReal(consdata->minactivityEx, 0);
      RatSetReal(consdata->maxactivityEx, 0);
   ...
   for( i = 0; i < ccons.nvars; ++i )
      consdataUpdateAddCoefEx(scip, consdata, GetCertificateVar(consdata, i), ccons.vals[i], FALSE);

   RatSet(consdata->lastglbminactivityEx, consdata->glbminactivityEx);
   ...
   )
{
   ...
   consdata->validminabsval = TRUE;
   consdata->validactivities = TRUE;
   consdata->computeactivities = TRUE;
   consdata->validminact = TRUE;
   consdata->validmaxact = TRUE;
   consdata->validglbminact = TRUE;
   consdata->validglbmaxact = TRUE;
   consdata->maxabsval = 0.0;
   consdata->minabsval = (ccons.nvars == 0 ? 0.0 : SCIPintervalAbsMax(ccons.valsreal[0]));
   consdata->minactivity = ccons.constant.inf;
   consdata->maxactivity = ccons.constant.sup;
   ...
   consdata->maxactivityneghuge = 0;
   consdata->maxactivityposhuge = 0;
   consdata->glbminactivity = ccons.constant.inf;
   consdata->glbmaxactivity = ccons.constant.inf;
   ...
   for( i = 0; i < ccons.nvars; ++i )
      consdataUpdateAddCoef(scip, consdata, GetCertificateVar(consdata, i), ccons.valsreal[i], FALSE);
   consdata->computeactivities = FALSE;
   consdata->lastminactivity = consdata->minactivity;
   consdata->lastmaxactivity = consdata->maxactivity;
   consdata->lastglbminactivity = consdata->glbminactivity;
...
void getMinActivityEx(
   ...
   SCIP_Rational*        delta,              /**< value to subtract from stored minactivity
   ...
   SCIP_Rational*        minactivity,        /**< pointer to store the minimal activity */
   ...
   )
{
   ...
   else
   {
      ...
      if( global )
      {
         ...
         tmpactivity = consdata->glbminactivityEx;
      }
      else
      {
         ...
         tmpactivity = consdata->minactivityEx;
      }

      RatDiff(minactivity, tmpactivity, delta);
 ...
 */
static
void getMaxActivityEx(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONSDATA*        consdata,           /**< linear constraint */
   int                   posinf,             /**< number of coefficients contributing pos. infinite value */
   int                   neginf,             /**< number of coefficients contributing neg. infinite value */
   SCIP_Rational*        delta,              /**< value to subtract from stored maxactivity
                                              *   (contribution of the variable set to zero when getting residual activity) */
   SCIP_Bool             global,             /**< should the global or local maximal activity be returned? */
   SCIP_Bool             goodrelax,          /**< should a good relaxation be computed or are relaxed acticities ignored, anyway? */
   SCIP_Rational*        maxactivity,        /**< pointer to store the maximal activity */
   SCIP_Bool*            isrelax,            /**< pointer to store whether the activity is a relaxation,
   ...
   )
{
   ...
   else
   {
      ...
      if( global )
      {
         ...
         tmpactivity = consdata->glbmaxactivityEx;
      }
      else
      {
         ...
         tmpactivity = consdata->maxactivityEx;
      }

      RatDiff(maxactivity, tmpactivity, delta);
      *issettoinfinity = FALSE;
      *isrelax = FALSE;
   }
}
   ...
   )
{
   ...
   RatCreateBuffer(SCIPbuffer(scip), &tmpdelta);

   if( !consdata->validactivities )
   {
      consdataCalcActivitiesEx(scip, consdata);
      assert(consdata->validminact);
      assert(consdata->validmaxact);
   }
   assert(consdata->minactivityneginf >= 0);
   ...
   getMinActivityEx(scip, consdata, consdata->minactivityposinf, consdata->minactivityneginf,
      tmpdelta, FALSE, goodrelax,
      minactivity, minisrelax, &issettoinfinity);

   getMaxActivityEx(scip, consdata, consdata->maxactivityposinf, consdata->maxactivityneginf,
      tmpdelta, FALSE, goodrelax,
...
void consdataGetReliableResidualActivity(
   ...
   SCIP_Rational*        resactivity,        /**< pointer to store the residual activity */
...
static
void consdataGetActivityResidualsEx(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONSDATA*        consdata,           /**< linear constraint */
   SCIP_VAR*             var,                /**< variable to calculate activity residual for */
   SCIP_Rational*        val,                /**< coefficient value of variable in linear constraint */
   SCIP_Bool             goodrelax,          /**< if we have huge contributions, do we need a good relaxation or are
                                              *   relaxed acticities ignored, anyway? */
   SCIP_Rational*        minresactivity,     /**< pointer to store the minimal residual activity */
   SCIP_Rational*        maxresactivity,     /**< pointer to store the maximal residual activity */
   ...
   )
{
    ...
    */
   if( RatIsInfinity(minactbound) )
   {
      assert(consdata->minactivityposinf >= 1);

      RatSetReal(tmp, 0.0);
      getMinActivityEx(scip, consdata, consdata->minactivityposinf - 1, consdata->minactivityneginf,
         tmp, FALSE, goodrelax,
         minresactivity, minisrelax, isminsettoinfinity);
   }
   else if( RatIsNegInfinity(minactbound) )
   {
      assert(consdata->minactivityneginf >= 1);

      RatSetReal(tmp, 0.0);
      getMinActivityEx(scip, consdata, consdata->minactivityposinf, consdata->minactivityneginf - 1,
         tmp, FALSE, goodrelax,
         minresactivity, minisrelax, isminsettoinfinity);
   }
   else
   {
      RatMult(tmp, absval, minactbound);
      getMinActivityEx(scip, consdata, consdata->minactivityposinf, consdata->minactivityneginf,
         tmp, FALSE, goodrelax,
         minresactivity, minisrelax, isminsettoinfinity);
   }
    ...
    * and contribution of variable set to zero that has to be subtracted from finite part of activity
    */
   if( RatIsNegInfinity(maxactbound) )
   {
      assert(consdata->maxactivityneginf >= 1);

      RatSetReal(tmp, 0.0);
      getMaxActivityEx(scip, consdata, consdata->maxactivityposinf, consdata->maxactivityneginf - 1,
         tmp, FALSE, goodrelax,
         maxresactivity, maxisrelax, ismaxsettoinfinity);
   }
   else if( RatIsInfinity(maxactbound) )
   {
      assert(consdata->maxactivityposinf >= 1);

      RatSetReal(tmp, 0.0);
      getMaxActivityEx(scip, consdata, consdata->maxactivityposinf - 1, consdata->maxactivityneginf,
         tmp, FALSE, goodrelax,
         maxresactivity, maxisrelax, ismaxsettoinfinity);
   }
   else
   {
      RatMult(tmp, absval, maxactbound);
      getMaxActivityEx(scip, consdata, consdata->maxactivityposinf, consdata->maxactivityneginf,
         tmp, FALSE, goodrelax,
         maxresactivity, maxisrelax, ismaxsettoinfinity);
   }
   ...
   RatFreeBuffer(SCIPbuffer(scip), &tmp);
   RatFreeBuffer(SCIPbuffer(scip), &absval);
...
void consdataGetGlbActivityBounds(
   ...
   SCIP_Bool             goodrelax,          /**< if we have huge contributions, do we need a good relaxation or are
                                              *   relaxed acticities ignored, anyway? */
   SCIP_Rational*        glbminactivity,     /**< pointer to store the minimal activity, or NULL, if not needed */
   SCIP_Rational*        glbmaxactivity,     /**< pointer to store the maximal activity, or NULL, if not needed */
   SCIP_Bool*            minisrelax,         /**< pointer to store whether the returned minactivity is just a relaxation,
                                              *   i.e. <= the exact minactivity (in case of huge contributions),
                                              *   or equal to the exact minimal activity */
   SCIP_Bool*            maxisrelax,         /**< pointer to store whether the returned maxactivity is just a relaxation,
   ...
   )
{
   ...
   RatFreeBuffer(SCIPbuffer(scip), &tmpzero);
}
...
void consdataGetGlbActivityResiduals(
   ...
   SCIP_Bool             goodrelax,          /**< if we have huge contributions, do we need a good relaxation or are
                                              *   relaxed acticities ignored, anyway? */
   SCIP_Rational*        minresactivity,     /**< pointer to store the minimal residual activity, or NULL, if not needed */
   SCIP_Rational*        maxresactivity,     /**< pointer to store the maximal residual activity, or NULL, if not needed */
   SCIP_Bool*            minisrelax,         /**< pointer to store whether the returned residual minactivity is just a
                                              *   relaxation, i.e. <= the exact residual minactivity (in case of huge
                                              *   contributions), or equal to the exact residual minactivity */
   SCIP_Bool*            maxisrelax,         /**< pointer to store whether the returned residual maxactivity is just a
   ...
   )
{
   ...
   if( RatIsPositive(val) )
   {
      RatSet(minactbound, SCIPvarGetLbGlobalExact(var));
      RatSet(maxactbound, SCIPvarGetUbGlobalExact(var));
      RatSet(absval, val);
   }
   else
   {
      RatNegate(minactbound, SCIPvarGetUbGlobalExact(var));
      RatNegate(maxactbound, SCIPvarGetLbGlobalExact(var));
      RatNegate(absval, val);
   }
   ...
   )
{
   ...
   {  SCIPintervalSetRoundingModeUpwards();
      SCIP_Real delta = absval * minactbound;
      SCIPintervalSetRoundingModeDownwards();
      getMinActivity(scip, consdata, consdata->minactivityposinf, consdata->minactivityneginf,
   ...
   {  SCIPintervalSetRoundingModeDownwards();
      SCIP_Real delta = absval * maxactbound;
      SCIPintervalSetRoundingModeUpwards();
      getMaxActivity(scip, consdata, consdata->maxactivityposinf, consdata->maxactivityneginf,
   ...
   )
{
   ...
   else
   {
      ...
      for( v = 0; v < consdata->nvars; ++v )
      {
         ...
         if( RatIsNegative(consdata->vals[v]) )
            negsign = TRUE;
         else
            negsign = FALSE;

         if( (RatIsInfinity(solval) && !negsign) || (RatIsNegInfinity(solval) && negsign) )
            ++nposinf;
         else if( (RatIsInfinity(solval) && negsign) || (RatIsNegInfinity(solval) && !negsign) )
            ++nneginf;
         else
         {
            RatAddProd(activity, solval, consdata->vals[v]);
         }
      }
      assert(nneginf >= 0 && nposinf >= 0);

      SCIPdebugMsg(scip, "activity of linear constraint: %.15g, %d positive infinity values, %d negative infinity values \n", RatApproxReal(activity), nposinf, nneginf);

      /* check for amount of infinity values and correct the activity */
      if( nposinf > 0 && nneginf > 0 )
      {
         RatAdd(activity, consdata->rhs, consdata->lhs);
         RatMultReal(activity, activity, 0.5);
      }
      else if( nposinf > 0 )
   ...
   )
{
   ...
   RatDiff(op2, activity, consdata->lhs);

   RatMIN(ret, op1, op2);

   RatFreeBuffer(SCIPbuffer(scip), &activity);
   RatFreeBuffer(SCIPbuffer(scip), &op1);
   RatFreeBuffer(SCIPbuffer(scip), &op2);
}
   ...
   )
{
   ...
   if( (RatIsPositive(val) && ub > 0.0) || (RatIsNegative(val) && lb < 0.0) )
      consdata->possignature |= varsignature;
   if( (RatIsPositive(val) && lb < 0.0) || (RatIsNegative(val) && ub > 0.0) )
   ...
   )
{
   ...
   switch (variable->varstatus)
   {
      ...
      case SCIP_VARSTATUS_NEGATED:
         ...
         res = SCIPcertificatePrintActivityVarBoundEx(scip, certificate, linename, boundtype == SCIP_BOUNDTYPE_UPPER ? SCIP_BOUNDTYPE_LOWER : SCIP_BOUNDTYPE_UPPER, newbound, ismaxactivity, constraint, variable->negatedvar);
         RatAddReal(newbound, newbound, -1.0);
         RatMultReal(newbound, newbound, -1);
         return res;
         break;
   ...
   val = NULL;
   for( int i = 0; i < ccons.nvars; i++ )
   {
      SCIP_VAR* var;
      var = GetCertificateVar(consdata, i);
      if (var == variable) {
         val = ccons.vals[i];
      }
   }
   ...
   SCIPcertificatePrintProofMessage(certificate, " { lin %d %d -1", ccons.nvars, certificateGetConsIndex(scip, certificate, constraint, !ismaxactivity));
   ...
   for( int i = 0; i < ccons.nvars; i++ )
   {
      ...
      assert(!RatIsAbsInfinity(ccons.vals[i]));
      ivar = GetCertificateVar(consdata, i);
      if ( ivar == variable )
         continue;
      is_upper_bound = upperboundcontribution != RatIsPositive(ccons.vals[i]);
      ...
      SCIPcertificatePrintProofRational(certificate, ccons.vals[i], 10);
   }
...
{
   if (ismaxactivity)
      consdataRecomputeMaxactivityEx(scip, SCIPconsGetData(constraint));
   else
      consdataRecomputeMinactivityEx(scip, SCIPconsGetData(constraint));

   //It would be more efficient if we could do this all in fp artihmetic. However, this is not trivial because the translations between aggregate variables need to be done exactly..
   SCIP_Rational* newboundex;
   SCIP_CALL(RatCreateBuffer(SCIPbuffer(scip), &newboundex));
   RatSetReal(newboundex, newbound);
   SCIPcertificatePrintActivityVarBoundEx(scip, certificate, linename, boundtype, newboundex, ismaxactivity, constraint, variable);
...
unsigned long certificateGetConsIndex(SCIP* scip, SCIP_CERTIFICATE* certificate, SCIP_CONS* cons, SCIP_Bool rhs) {
   ...
   consdata = SCIPconsGetData(cons);
   if (consdata->rowexact != NULL)
      return SCIPcertificateGetRowIndex(certificate, consdata->rowexact, rhs);
...
{  /*lint --e{715}*/
   ...
   else if( SCIPvarIsBinary(var1) )
   {
      ...
      if( RatIsAbsGT(consdata->vals[ind1], consdata->vals[ind2]) )
         return -1;
      else
         return +1;
   }
   ...
   )
{
   ...
   assert(!RatIsInfinity(consdata->lhs));

   /* check whether the side is not changed */
   if( RatIsEqual(consdata->lhs, lhs) )
      return SCIP_OKAY;
   ...
   if( locked )
   {
      if( RatIsNegInfinity(consdata->lhs) && !RatIsNegInfinity(lhs) )
      {
         SCIP_VAR** vars;
         SCIP_Rational** vals;
         int v;
         ...
         {
            assert(vars[v] != NULL);
            assert(!RatIsZero(vals[v]));

            if( RatIsPositive(vals[v]) )
            {
      ...
      else if( !RatIsNegInfinity(consdata->lhs) && RatIsNegInfinity(lhs) )
      {
         SCIP_VAR** vars;
         SCIP_Rational** vals;
         int v;
   ...
   if( !RatIsNegInfinity(lhs) && RatIsGT(lhs, consdata->lhs) )
   {
      consdata->boundstightened = 0;
      consdata->presolved = FALSE;
      consdata->cliquesadded = FALSE;
   ...
   )
{
   ...
   assert(consdata != NULL);
   assert(consdata->nvars == 0 || (consdata->vars != NULL && consdata->vals != NULL));
   assert(!RatIsNegInfinity(consdata->rhs));

   /* check whether the side is not changed */
   if( RatIsEqual(consdata->rhs, rhs) )
      return SCIP_OKAY;
   ...
   if( locked )
   {
      ...
      if( RatIsInfinity(consdata->rhs) && !RatIsInfinity(rhs) )
      {
         SCIP_VAR** vars;
         SCIP_Rational** vals;
         int v;
      ...
      else if( !RatIsInfinity(consdata->rhs) && RatIsInfinity(rhs) )
      {
         SCIP_VAR** vars;
         SCIP_Rational** vals;
         int v;
   ...
   if( !RatIsInfinity(rhs) && RatIsLT(rhs, consdata->rhs) )
   {
      consdata->boundstightened = 0;
      consdata->presolved = FALSE;
      consdata->cliquesadded = FALSE;
   ...
   )
{
   ...
   if( transformed )
   {
      ...
      consdataUpdateAddCoefEx(scip, consdata, var, val, FALSE);
   ...
   )
{
   ...
   if( locked && ((RatIsNegative(newval) && RatIsPositive(val)) || (RatIsNegative(val) && RatIsPositive(newval))) )
   {
      assert(SCIPconsIsTransformed(cons));
   ...
   /* update minimum and maximum activities */
   if( SCIPconsIsTransformed(cons) )
      consdataUpdateChgCoefEx(scip, consdata, var, val, newval, TRUE);
...
SCIP_RETCODE scaleCons(
   ...
   SCIP_Rational*        scalar              /**< value to scale constraint with */
   )
{
   SCIP_CONSDATA* consdata;
   SCIP_Real newval;
   ...
   )
{
   ...
   SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &valsum) );
   ...
   while( v >= 1 )
   {
      ...
      {
         RatSet(valsum, consdata->vals[v]);
         do
         {
            SCIP_CALL( delCoefPos(scip, cons, v) );
            --v;
            RatAdd(valsum, valsum, consdata->vals[v]);
         }
   ...
   RatFreeBuffer(SCIPbuffer(scip), &valsum);
   consdata->merged = TRUE;
...
SCIP_RETCODE addConflictReasonVars(
   ...
   SCIP_Rational*        bound               /**< bound of variable which was tried to apply, or SCIP_INVALID */
   )
{
   int v;
   ...
   )
{
   ...
   case PROPRULE_1_RANGEDROW:
      ...
      assert(!RisInfinity(-consdata->lhs) && !RisInfinity(consdata->rhs));
...
SCIP_RETCODE tightenVarUb(
   ...
   PROPRULE              proprule,           /**< propagation rule that deduced the value */
   SCIP_Rational*        newub,              /**< new upper bound */
   SCIP_Rational*        oldub,              /**< old upper bound */
   ...
   )
{
   ...
   lb = SCIPvarGetLbLocalExact(var);
   //newub = SCIPadjustedUb(scip, var, newub);

   if( RatIsLE(newub, oldub) )
   {
      SCIP_VARTYPE vartype;

      RatDebugMessage("linear constraint <%s>: tighten <%s>, old bds=[%q,%q], val=%q, activity=[%q,%q], sides=[%q,%q] -> newub=%q\n",
      ...
      /* tighten upper bound */
      SCIP_CALL( SCIPinferVarUbConsExact(scip, var, newub, cons, getInferInt(proprule, pos), force, &infeasible, &tightened) );
      ...
      if( infeasible )
      {
         ...
         /* analyze conflict */
         //SCIP_CALL( analyzeConflict(scip, cons, TRUE) ); @TODO add exact conflict analysis

         *cutoff = TRUE;
      }
      else if( tightened )
      {
         assert(RatIsLE(SCIPvarGetUbLocalExact(var), oldub));
...
SCIP_RETCODE tightenVarLb(
   ...
   int                   pos,                /**< variable position */
   PROPRULE              proprule,           /**< propagation rule that deduced the value */
   SCIP_Rational*        newlb,              /**< new lower bound */
   SCIP_Rational*        oldlb,              /**< old lower bound */
   SCIP_Bool*            cutoff,             /**< pointer to store whether the node can be cut off */
   int*                  nchgbds,            /**< pointer to count the total number of tightened bounds */
   SCIP_Bool             force               /**< should a possible bound change be forced even if below bound strengthening tolerance */
   )
   ...
   )
{
   ...
   *cutoff = FALSE;
   ...
   SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &alpha) );
   SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &slack) );
   SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &minactivity) );
   SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &maxactivity) );
   SCIP_Bool minisrelax;
   SCIP_Bool maxisrelax;

   consdataGetActivityBounds(scip, consdata, 0, minactivity, maxactivity, &minisrelax, &maxisrelax);
   if( RatIsPositive(val) )
   {
      ...
      if( !RatIsInfinity(rhs) )
      {
         ...
         if( RatIsLT(rhs, minactivity) )
         {
            RatDebugMessage("linear constraint <%s>: cutoff  <%s>, minactivity=%q > rhs=%q\n",
         ...
         RatDiff(slack, rhs, minactivity);


         /* if the slack is zero in tolerances (or negative, but not enough to make the constraint infeasible), we set
          * it to zero
          */
         assert(!RatIsNegative(slack));




         RatDiff(alpha, ub, lb);
         RatMult(alpha, val, alpha);

         assert(!RatIsNegative(alpha));

         if( RatIsGT(alpha, slack) )
         {
            SCIP_Rational* newub;
      ...
      if( !RatIsNegInfinity(lhs) )
      {
         ...
         if( RatIsLT(maxactivity, lhs) )
         {
            RatDebugMessage("linear constraint <%s>: cutoff  <%s>, maxactivity=%q < lhs=%q\n",
               SCIPconsGetName(cons), SCIPvarGetName(var), maxactivity, lhs);

            *cutoff = TRUE;
            return SCIP_OKAY;
         }

         RatDiff(slack, maxactivity, lhs);

         /* if the slack is zero in tolerances (or negative, but not enough to make the constraint infeasible), we set
          * it to zero
          */
         if( !RatIsPositive(slack) )
            RatSetInt(slack, 0, 1);
         ...
         if( RatIsGT(alpha, slack) )
         {
            ...
            RatDiv(newlb, slack, val);
            RatDiff(newlb, ub, newlb);


            //SCIP_CALL( tightenVarLb(scip, cons, pos, PROPRULE_1_LHS, newlb, lb, cutoff, nchgbds, force) ); //@TODO uncomment after adding tightenVarLb
   ...
   else
   {
      ...
      if( !RatIsInfinity(rhs) )
      {
         ...
         {
            RatDebugMessage("linear constraint <%s>: cutoff  <%s>, minactivity=%q > rhs=%q\n",
               SCIPconsGetName(cons), SCIPvarGetName(var), minactivity, rhs);
         ...
         if( RatIsGT(alpha, slack) )
         {
            RatDebugMessage("alpha (%q) > slack (%q)\n",
                  alpha, slack);
            SCIP_Rational* newlb;
            RatCreateBuffer(SCIPbuffer(scip), &newlb);

            RatDiv(newlb, slack, val);
            RatAdd(newlb, ub, newlb);

            // SCIP_CALL( tightenVarLb(scip, cons, pos, PROPRULE_1_RHS, newlb, lb, cutoff, nchgbds, force) ); //@TODO uncomment after adding tightenVarLb
            RatFreeBuffer(SCIPbuffer(scip), &newlb);
            if( *cutoff )
            {
               RatDebugMessage("linear constraint <%s>: cutoff  <%s>, new bds=[%q,%q]\n",
      ...
      if( !RatIsNegInfinity(lhs) )
      {
         ...
         /* if the maxactivity is smaller than the left hand side by feasibility epsilon, the constraint is infeasible */
         if( RatIsLT(consdata->maxactivity, lhs) )
         {
            RatDebugMessage("linear constraint <%s>: cutoff  <%s>, maxactivity=%q < lhs=%q\n",
         ...
         RatDiff(alpha, lb, ub);
         RatMult(alpha, alpha, val);
         assert(!RatIsNegative(alpha));

         if( RatIsGT(alpha, slack) )
         {
            consPrintConsSol(scip, cons, NULL, 1, NULL);
            RatDebugMessage("LHS=%q\n",
               lhs);
            RatDebugMessage("RHS=%q\n",
               lhs);
            RatDebugMessage("alpha (%q) > slack (%q)\n",
               alpha, slack);
            SCIP_Rational* newub;
...
RETURN_SCIP_OKAY:
   RatFreeBuffer(SCIPbuffer(scip), &minactivity);
   RatFreeBuffer(SCIPbuffer(scip), &maxactivity);
   RatFreeBuffer(SCIPbuffer(scip), &alpha);
   ...
   )
{
   ...
   if( RisInfinity(-consdata->lhs) || RisInfinity(consdata->rhs) )
      return SCIP_OKAY;
   ...
   assert(!RisInfinity(-fixedact) && !RisInfinity(fixedact));
      ...
      if( gcdinfvars >= 1 )
      {
	    ...
	    /* only one variable in the second set, so we can bound this variables */
		  ...
		  if( newub < SCIPvarGetUbLocal(consdata->vars[v]) )
		  {
	       ...
	       if( !RisInfinity(-newlhs) || !RisInfinity(newrhs) )
	       {
		  /* create, add, and release new artificial constraint */
...
static
SCIP_RETCODE tightenVarBoundsEx(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONS*            cons,               /**< linear constraint */
   int                   pos,                /**< position of the variable in the vars array */
   SCIP_Bool*            cutoff,             /**< pointer to store whether the node can be cut off */
   ...
   )
{
   ...
   *cutoff = FALSE;
   ...
   #endif
   consdataCalcActivitiesEx(scip, consdata);
   consdataGetActivityResidualsEx(scip, consdata, var, val, FALSE, minresactivity, maxresactivity,
   ...
   if( RatIsPositive(val) )
   {
      ...
      {
         SCIP_Rational* newub;
         SCIP_Rational* newubproduct;

         RatCreateBuffer(SCIPbuffer(scip), &newub);
         SCIP_CALL(RatCreateBuffer(SCIPbuffer(scip), &newubproduct));
         RatDiff(newubproduct, rhs, minresactivity);
         RatDiv(newub, newubproduct, val);
             ...
             RatIsLT(newub, ub) )
         {
               ...
               {
                  SCIPcertificatePrintActivityVarBoundEx(scip, SCIPgetCertificate(scip), NULL, SCIP_BOUNDTYPE_UPPER, newub, false, cons, var);
               }
               ...
               if( infeasible )
               {
                  ...
                  *cutoff = TRUE;
                  RatFreeBuffer(SCIPbuffer(scip), &newub);
                  RatFreeBuffer(SCIPbuffer(scip), &newubproduct);
                  goto RETURN_SCIP_OKAY;
               }
               if( tightened )
               {
      ...
      {
         SCIP_Rational* newlb;
         SCIP_Rational* newlbproduct;
         SCIP_CALL(RatCreateBuffer(SCIPbuffer(scip), &newlb));
         SCIP_CALL(RatCreateBuffer(SCIPbuffer(scip), &newlbproduct));
         RatDiff(newlbproduct, lhs, maxresactivity);
         RatDiv(newlb, newlbproduct, val);
             ...
             RatIsGT(newlb, lb) )
         {
            ...
            if( SCIPisCertificateActive(scip) && (SCIPgetStage(scip) == SCIP_STAGE_SOLVING) )
               SCIPcertificatePrintActivityVarBoundEx(scip, SCIPgetCertificate(scip), NULL, SCIP_BOUNDTYPE_LOWER, newlb, true, cons, var);
            ...
            if( infeasible )
            {
               ...
               *cutoff = TRUE;
               RatFreeBuffer(SCIPbuffer(scip), &newlb);
               RatFreeBuffer(SCIPbuffer(scip), &newlbproduct);
               goto RETURN_SCIP_OKAY;
            }
            if( tightened )
            {
   ...
   else
   {
      ...
      if( !isminsettoinfinity && !RatIsInfinity(rhs) && !minisrelax )
      {
         ...
         RatCreateBuffer(SCIPbuffer(scip), &newlb);
         SCIP_CALL(RatCreateBuffer(SCIPbuffer(scip), &newlbproduct));
         RatDiff(newlbproduct, rhs, minresactivity);
         RatDiv(newlb, newlbproduct, val);
            ...
            RatIsGT(newlb, lb) )
         {
               ...
               RatDebugMessage("linear constraint <%s>: tighten <%s>, old bds=[%q,%q], val=%.15g, resactivity=[%q,%q], sides=[%q,%q] -> newlb=%q\n",
                  SCIPconsGetName(cons), SCIPvarGetName(var), lb, ub, val, minresactivity, maxresactivity, lhs, rhs, newlb);
               if( SCIPisCertificateActive(scip) && (SCIPgetStage(scip) == SCIP_STAGE_SOLVING) )
                  SCIPcertificatePrintActivityVarBoundEx(scip, SCIPgetCertificate(scip), NULL, SCIP_BOUNDTYPE_LOWER, newlb, false, cons, var);
      ...
      if( !ismaxsettoinfinity && !RatIsNegInfinity(lhs) && !maxisrelax )
      {
         ...
         RatCreateBuffer(SCIPbuffer(scip), &newub);
         SCIP_CALL(RatCreateBuffer(SCIPbuffer(scip), &newubproduct));
         RatDiff(newubproduct, lhs, maxresactivity);
         RatDiv(newub, newubproduct, val);
         ...
         if( !RatIsInfinity(newub) && RatIsLT(newub, ub))
         {
            ...
            RatDebugMessage("linear constraint <%s>: tighten <%s>, old bds=[%q,%q], val=%q, resactivity=[%q,%q], sides=[%q,%q], newub=%q\n",
               SCIPconsGetName(cons), SCIPvarGetName(var), lb, ub, val, minresactivity, maxresactivity, lhs, rhs, newub);
            if( SCIPisCertificateActive(scip) && (SCIPgetStage(scip) == SCIP_STAGE_SOLVING) )
               SCIPcertificatePrintActivityVarBoundEx(scip, SCIPgetCertificate(scip), NULL, SCIP_BOUNDTYPE_UPPER, newub, true, cons, var);
   ...
   RETURN_SCIP_OKAY:
#ifdef SCIP_DEBUG
   SCIPcertificateAssertStateCorrect(scip, var);
#endif
   ...
   )
{
   ...
   *cutoff = FALSE;
   ...
   var = GetCertificateVar(consdata, pos);
   ...
   if( valrange.sup > 0.0 )
   {
      ...
      if( !isminsettoinfinity && !SCIPisInfinity(scip, rhs) && !minisrelax )
      {
            ...
            ((force && SCIPisLT(scip, newub, ub)) || (SCIPvarIsIntegral(var) && SCIPisFeasLT(scip, newub, ub)) || SCIPisUbBetter(scip, newub, lb, ub)) )
         {
            ...
            activityunreliable = false && SCIPisUpdateUnreliable(scip, minresactivity, consdata->lastminactivity);
            ...
            {  assert(false);
               ...
               newub = (rhs - minresactivity)/valrange.sup;
      ...
      if( !ismaxsettoinfinity && !SCIPisInfinity(scip, -lhs) && !maxisrelax )
      {
            ...
            ((force && SCIPisGT(scip, newlb, lb)) || (SCIPvarIsIntegral(var) && SCIPisFeasGT(scip, newlb, lb)) || SCIPisLbBetter(scip, newlb, lb, ub)) )
         {
            ...
            if(false && SCIPisUpdateUnreliable(scip, maxresactivity, consdata->lastmaxactivity) )
            {  assert(false);
               /*
               consdataGetReliableResidualActivity(scip, consdata, var, &maxresactivity, FALSE, FALSE);
               newlb = (lhs - maxresactivity)/valrange.inf;
   ...
   else
   {
      ...
      if( !isminsettoinfinity && !SCIPisInfinity(scip, rhs) && !minisrelax )
      {
            ...
            ((force && SCIPisGT(scip, newlb, lb)) || (SCIPvarIsIntegral(var) && SCIPisFeasGT(scip, newlb, lb)) || SCIPisLbBetter(scip, newlb, lb, ub)) )
         {
            ...
            /*
               consdataGetReliableResidualActivity(scip, consdata, var, &minresactivity, TRUE, FALSE);
               newlb = (rhs - minresactivity)/valrange.sup;
      ...
      if( !ismaxsettoinfinity && !SCIPisInfinity(scip, -lhs) && !maxisrelax )
      {
            ...
            ((force && SCIPisLT(scip, newub, ub)) || (SCIPvarIsIntegral(var) && SCIPisFeasLT(scip, newub, ub)) || SCIPisUbBetter(scip, newub, lb, ub)) )
         {
            ...
            /*
               consdataGetReliableResidualActivity(scip, consdata, var, &maxresactivity, FALSE, FALSE);
               newub = (lhs - maxresactivity)/valrange.sup;
   ...
   )
{
   ...
   *cutoff = FALSE;
   ...
   if( !SCIPisInfinity(scip, consdata->maxactdelta) )
   {
      ...
      slack = (SCIPisInfinity(scip, ccons.rhsreal) || isminsettoinfinity) ? SCIPinfinity(scip) : (ccons.rhsreal - minactivity);
      surplus = (SCIPisInfinity(scip, -ccons.lhsreal) || ismaxsettoinfinity) ? SCIPinfinity(scip) : (maxactivity - ccons.lhsreal);
   ...
   for( nrounds = 0; (force || consdata->boundstightened < tightenmode) && nrounds < MAXTIGHTENROUNDS; ++nrounds ) /*lint !e574*/
   {
      ...
      while( v < nvars && v != lastchange && !(*cutoff) )
      {
         ...
         if( *nchgbds > oldnchgbds )
         {
            ...
            && !SCIPisFeasEQ(scip, SCIPvarGetUbLocal(GetCertificateVar(consdata, v)), SCIPvarGetLbLocal(GetCertificateVar(consdata, v))) )
            v = consdata->nbinvars;
         else
            ++v;
      }
...
#ifndef NDEBUG
   if( force && SCIPisEQ(scip, consdata->lhsreal, consdata->rhsreal) )
      assert(*cutoff || SCIPisFeasEQ(scip, SCIPvarGetLbLocal(GetCertificateVar(consdata, v)), SCIPvarGetUbLocal(GetCertificateVar(consdata, v))));
#endif
   ...
   )
{
   ...
   *cutoff = FALSE;
   //SCIPinfoMessage(scip, "tightenBounds");
   /* we cannot tighten variables' bounds, if the constraint may be not complete */
   if( SCIPconsIsModifiable(cons) )
      return SCIP_OKAY;
    ...
    * thus, ensure here again that variable fixings have been applied
    */
   //SCIP_CALL( applyFixings(scip, cons, cutoff) ); @TODO uncomment
   if( *cutoff )
      return SCIP_OKAY;
   ...
   /* update maximal activity delta if necessary */
   if( RatIsInfinity(consdata->maxactdeltaEx) ) /*lint !e777*/
      consdataRecomputeMaxActivityDelta(scip, consdata);

   assert(consdata->maxactdeltaEx != NULL); /*lint !e777*/
   assert(!RatIsNegative(consdata->maxactdeltaEx));
   checkMaxActivityDelta(scip, consdata);

   /* this may happen if all variables are fixed */
   if( RatIsZero(consdata->maxactdeltaEx) )
      return SCIP_OKAY;

   if( !RatIsInfinity(consdata->maxactdeltaEx) )
   {
      SCIP_Rational* slack;
      SCIP_Rational* surplus;
      SCIP_Rational* minactivity;
      SCIP_Rational* maxactivity;
      SCIP_Bool minisrelax;
      SCIP_Bool maxisrelax;
      ...
      SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &slack) );
      SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &surplus) );
      SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &minactivity) );
      SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &maxactivity) );

      /* use maximal activity delta to skip propagation (cannot deduce anything) */
      consdataGetActivityBoundsEx(scip, consdata, FALSE, minactivity, maxactivity, &minisrelax, &maxisrelax);
      assert(!RatIsInfinity(minactivity));
      assert(!RatIsNegInfinity(maxactivity));

      if (RatIsInfinity(consdata->rhs) || RatIsNegInfinity(minactivity))
      {
         RatSetString(slack, "inf");
      }
      else
      {
         RatDiff(slack, consdata->rhs, minactivity);
      }

      if (RatIsNegInfinity(consdata->lhs) || RatIsInfinity(maxactivity))
      {
         RatSetString(slack, "inf");
      }
      else
      {
         RatDiff(slack, maxactivity, consdata->lhs);
      }


      /* check if the constraint will propagate */
      SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &tmp) );
      RatMIN(tmp, slack, surplus);

      RatFreeBuffer(SCIPbuffer(scip), &surplus);
      RatFreeBuffer(SCIPbuffer(scip), &slack);
      RatFreeBuffer(SCIPbuffer(scip), &maxactivity);
      RatFreeBuffer(SCIPbuffer(scip), &minactivity);
      if( RatIsLE(consdata->maxactdeltaEx, tmp) ) {
         RatFreeBuffer(SCIPbuffer(scip), &tmp);
         return SCIP_OKAY;
      }
      RatFreeBuffer(SCIPbuffer(scip), &tmp);
   ...
   {
      SCIPcertificateAssertStateCorrect(scip, SCIPconsGetData(cons)->vars[i]);
   }
#endif
   for( nrounds = 0; (force || consdata->boundstightened < tightenmode) && nrounds < MAXTIGHTENROUNDS; ++nrounds ) /*lint !e574*/
   {
      ...
      while( v < nvars && v != lastchange && !(*cutoff) )
      {
         ...
         if( *nchgbds > oldnchgbds )
         {
            lastchange = v;
            ++v;
         } else if (consdata->indexsorted && v < consdata->nbinvars - 1 &&
                    !SCIPisFeasEQ(scip, SCIPvarGetUbLocal(consdata->vars[v]),
                                  SCIPvarGetLbLocal(consdata->vars[v])))
           v = consdata->nbinvars;
         else
            ++v;
      }
...
#ifndef NDEBUG
   if( force && RatIsEqual(consdata->lhs, consdata->rhs) )
      assert(*cutoff || RatIsEqual(SCIPvarGetLbLocalExact(consdata->vars[0]), SCIPvarGetUbLocalExact(consdata->vars[0])));
#endif
   ...
   )
{
   ...
   *violated = FALSE;
   ...
   else if( ((!RatIsNegInfinity(consdata->lhs) && RatIsLT(activity, consdata->lhs)) ||
      (!RatIsInfinity(consdata->rhs) && RatIsGT(activity, consdata->rhs))) )
   {
      *violated = TRUE;
   ...
   )
{
   ...
   for (int i = 0; i < consdata->rowexact->len; i++)
   {
      consCatchRowEvent(scip, cons, SCIPconshdlrGetData(SCIPconsGetHdlr(cons))->eventhdlr, i);
   }
   return SCIP_OKAY;
}
   ...
   *cutoff = FALSE;
   ...
   if( !SCIPconsIsModifiable(cons) )
   {
      ...
      if( rangedrowpropagation && tightenbounds && !(*cutoff) )
      {
         ...
         // SCIP_CALL( rangedRowPropagation(scip, cons, cutoff, &nfixedvars, nchgbds, &naddconss) ); @TODO implement ranged row propagation
      ...
      if( !(*cutoff) )
      {
         consdataGetActivityBoundsEx(scip, consdata, TRUE, minactivity, maxactivity, &minactisrelax, &maxactisrelax);
         if( minactivity >consdata->rhs )
         ...
         else if( maxactivity < consdata->lhs )
         {
            RatDebugMessage("linear constraint <%s> is infeasible (lhs): activitybounds=[%q,%q], sides=[%q,%q]\n",
               SCIPconsGetName(cons), minactivity, maxactivity, consdata->lhs, consdata->rhs);
         ...
         else if( RatIsGE(minactivity, consdata->lhs) && RatIsLE(maxactivity, consdata->rhs) )
         {
            RatDebugMessage("linear constraint <%s> is redundant: activitybounds=[%q,%q], sides=[%q,%q]\n",
               SCIPconsGetName(cons), minactivity, maxactivity, consdata->lhs, consdata->rhs);
   ...
   )
{
   ...
   if( !consdata->implsadded )
   {
      ...
      finitelhs = !RisInfinity(-consdata->lhs);
      finiterhs = !RisInfinity(consdata->rhs);
   ...
   )
{
   ...
   *infeasible = FALSE;
   ...
   if( !SCIPisIntegral(scip, consdata->lhs) || !SCIPisIntegral(scip, consdata->rhs) )
   {
      ...
      if( integral )
      {
         if( !RisInfinity(-consdata->lhs) && !SCIPisIntegral(scip, consdata->lhs) )
         {
            newlhs = SCIPfeasCeil(scip, consdata->lhs);
            chglhs = TRUE;
         }
         if( !RisInfinity(consdata->rhs) && !SCIPisIntegral(scip, consdata->rhs) )
         {
   ...
   )
{
      ...
      if( val >= 0.0 )
      {
            ...
            SCIPisGE(scip, minactivity + val, consdata->lhs) && SCIPisLE(scip, maxactivity - val, consdata->rhs) )
         {
            ...
            if( consdata->nvars == 2 )
            {
               ...
               if( !RisInfinity(-consdata->lhs) && consdata->minactivityneginf + consdata->minactivityneginf == 0 )
               {
            ...
            if( !SCIPisSumRelEQ(scip, newval, val) )
            {
               ...
               if( !RisInfinity(-consdata->lhs) && !SCIPisEQ(scip, newlhs, consdata->lhs) )
               {
               ...
               if( !RisInfinity(consdata->rhs) && !SCIPisEQ(scip, newrhs, consdata->rhs) )
               {
         ...
         {
            if( !RisInfinity(-minleftactivity) )
            {
            ...
            if( !RisInfinity(maxleftactivity) )
            {
               assert(!RisInfinity(val));
   ...
   assert(!RisInfinity(-minleftactivity) || RisInfinity(-minactivity));
   assert(!RisInfinity(maxleftactivity) || RisInfinity(maxactivity));
    ...
    */
   assert(!RisInfinity(minactivity));
   if( !RisInfinity(-consdata->lhs) && (RisInfinity(-minactivity) || minactisrelax) )
      return SCIP_OKAY;
    ...
    */
   assert(!RisInfinity(-maxactivity));
   if( !RisInfinity(consdata->rhs) && (RisInfinity(maxactivity) || maxactisrelax) )
      return SCIP_OKAY;
    ...
    */
   if( (RisInfinity(-consdata->lhs) || SCIPisFeasLT(scip, maxleftactivity, aggrlhs))
      && (RisInfinity(consdata->rhs) || SCIPisFeasGT(scip, minleftactivity, aggrrhs)) )
   {
      ...
      while( i < consdata->nvars )
      {
         ...
         if( val >= 0.0 )
         {
               ...
               SCIPisLT(scip, minactivity + val, consdata->lhs) || SCIPisGT(scip, maxactivity - val, consdata->rhs) )
            {
                ...
                * huge contributions
                */
               assert(!minactisrelax || RisInfinity(-consdata->lhs));
               assert(!maxactisrelax || RisInfinity(consdata->rhs));
            }
         }
         else
         {
         ...
         if( !RisInfinity(-consdata->lhs) && !SCIPisFeasEQ(scip, newlhs, consdata->lhs) )
         {
            SCIPdebugMsg(scip, "linear constraint <%s>: change lhs %.15g to %.15g\n", SCIPconsGetName(cons), consdata->lhs, newlhs);
            SCIP_CALL( chgLhs(scip, cons, newlhs) );
            ++(*nchgsides);
         ...
         if( !RisInfinity(consdata->rhs) && !SCIPisFeasEQ(scip, newrhs, consdata->rhs) )
         {
            SCIPdebugMsg(scip, "linear constraint <%s>: change rhs %.15g to %.15g\n", SCIPconsGetName(cons), consdata->rhs, newrhs);
            SCIP_CALL( chgRhs(scip, cons, newrhs) );
            ++(*nchgsides);
...
void getNewSidesAfterAggregation(
   ...
   SCIP_Rational*        slackcoef,          /**< coefficient of variable in constraint */
   ...
   )
{
   ...
   for( v = 0; v < consdata->nvars; ++v )
   {
         ...
         !SCIPdoNotMultaggrVar(scip, var) )
      {
         ...
         if( RisInfinity(varub) || RisInfinity(-varlb) )
         ...
         else
         {
            ...
            assert(!RisInfinity(slackdomrng));
         }
         equal = FALSE;
   ...
   for( v = 0; v < consdata->nvars; ++v )
   {
      if( v != bestslackpos )
      {
         ...
         {
            if( RisInfinity(SCIPvarGetUbGlobal(consdata->vars[v])) )
            {
               ++supinf;
               if( RisInfinity(-SCIPvarGetLbGlobal(consdata->vars[v])) )
               {
                  ++infinf;
                  samevar = TRUE;
               }
            }
            else if( RisInfinity(-SCIPvarGetLbGlobal(consdata->vars[v])) )
               ++infinf;
         }
         else if( SCIPisNegative(scip, consdata->vals[v]) )
         {
            ...
            else if( RisInfinity(SCIPvarGetUbGlobal(consdata->vars[v])) )
               ++infinf;
         }
      }
   }
   ...
   )
{
   ...
   if( applicable )
   {
      SCIP_Bool rhsfinite = !RisInfinity(consdata->rhs);
      SCIP_Bool lhsfinite = !RisInfinity(-consdata->lhs);
...
SCIP_Bool consdataIsResidualIntegral(
   ...
   SCIP_Rational*        val                 /**< value to divide the coefficients by */
   )
{
   int v;
...
void calculateMinvalAndMaxval(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_Rational*        side,               /**< lhs or rhs */
   SCIP_Rational*        val,                /**< coefficient */
   SCIP_Rational*        minresactivity,     /**< minimal residual activity */
   SCIP_Rational*        maxresactivity,     /**< maximal residual activity */
   SCIP_Rational*        minval,             /**< pointer to store calculated minval */
   SCIP_Rational*        maxval              /**< pointer to store calculated maxval */
   )
{
   assert(scip != NULL);
   assert(minval != NULL);
   ...
   if( RatIsPositive(val) )
   {
      if( RatIsAbsInfinity(maxresactivity) )
         RatNegate(minval, maxresactivity);
      else
      {
         RatDiff(minval, side, maxresactivity);
         RatDiv(minval, minval, val);
      }

      if( RatIsAbsInfinity(minresactivity) )
         RatNegate(maxval, minresactivity);
      else
      {
         RatDiff(maxval, side, minresactivity);
         RatDiv(maxval, maxval, val);
      }
   }
   else
   {
      if( RatIsAbsInfinity(minresactivity) )
         RatSet(minval, minresactivity);
      else
      {
         RatDiff(minval, side, minresactivity);
         RatDiv(minval, minval, val);
      }
      if( RatIsAbsInfinity(maxresactivity) )
         RatSet(maxval, maxresactivity);
      else
      {
         RatDiff(maxval, side, maxresactivity);
         RatDiv(maxval, maxval, val);
      }
   }
}
   ...
   )
{
   ...
   lhsexists = !RisInfinity(-consdata->lhs);
   rhsexists = !RisInfinity(consdata->rhs);
   ...
   for( i = 0; i < consdata->nvars && bestisint; ++i )
   {
      ...
      if( agglhs || aggrhs )
      {
         ...
         if( agglhs )
         {
            ...
            if( (!RisInfinity(-minval) && SCIPisFeasGE(scip, minval, lb)) &&
               (!RisInfinity(maxval) && SCIPisFeasLE(scip, maxval, ub)) )
            {
   ...
   if( bestpos >= 0 )
   {
      ...
      for( j = 0; j < consdata->nvars; ++j )
      {
         if( j != bestpos )
         {
            ...
            {
               if( RisInfinity(SCIPvarGetUbGlobal(consdata->vars[j])) )
               {
                  ++supinf;
                  if( RisInfinity(-SCIPvarGetLbGlobal(consdata->vars[j])) )
                  {
                     ++infinf;
                     samevar = TRUE;
                  }
               }
               else if( RisInfinity(-SCIPvarGetLbGlobal(consdata->vars[j])) )
                  ++infinf;
            }
            else if( SCIPisNegative(scip, aggrcoefs[naggrs]) )
            {
               ...
               else if( RisInfinity(SCIPvarGetUbGlobal(consdata->vars[j])) )
                  ++infinf;
            }
   ...
   )
{
   ...
   lhs = consdata->lhs;
   rhs = consdata->rhs;
   assert(!RisInfinity(-lhs) && !RisInfinity(rhs));
   ...
   )
{
   ...
   *infeasible = FALSE;
   ...
   assert(!RisInfinity(-minactsub));
   assert(!RisInfinity(maxactsub));
   ...
   )
{
   ...
   *aggregated = FALSE;
   ...
   if( (bestv != -1 || commonvarlindependent) && SCIPconsGetNUpgradeLocks(cons0) == 0 )
   {
      ...
      assert(!RisInfinity(-consdata1->lhs));
      assert(!RisInfinity(consdata1->rhs));
      if( RisInfinity(-consdata0->lhs) )
         newlhs = -SCIPinfinity(scip);
      else
         newlhs = a * consdata0->lhs + b * consdata1->lhs;
      if( RisInfinity(consdata0->rhs) )
         newrhs = SCIPinfinity(scip);
      else
 ...
 */
static
SCIP_DECL_HASHKEYEQ(hashKeyEqExactLinearcons)
{
   SCIP_CONSDATA* consdata1;
   ...
   assert(consdata1->coefsorted);
   assert(consdata2->coefsorted);
   ...
   for( i = 0; i < consdata1->nvars; ++i )
   {
      if( !RatIsEqual(consdata1->vals[i], consdata2->vals[i]) )
         return FALSE;
   }
...
SCIP_DECL_HASHKEYVAL(hashKeyValLinearcons)
{
   ...
   scale = COPYSIGN(1.0/RatApproxReal(consdata->maxabsval), RatApproxReal(consdata->vals[0]));
      ...
      if( nparallelconss != 0 )
      {
         ...
         for( i = 0; i < nparallelconss; ++i )
         {
            ...
            if( scale > 0.0 )
            {
               ...
               if( ! RisInfinity(-consdatadel->lhs) )
                  lhs = MAX(scale * consdatadel->lhs, lhs);

               if( ! RisInfinity(consdatadel->rhs) )
   ...
   )
{
   ...
   for( c = (cons0changed ? 0 : firstchange); c < chkind && !(*cutoff) && conss[chkind] != NULL; ++c )
   {
       ...
       * redundant
       */
      if( cons1dominateslhs && (!cons0isequality || cons1dominatesrhs || RisInfinity(consdata0->rhs) ) )
      {
      ...
      else if( cons0dominateslhs && (!cons1isequality || cons0dominatesrhs || RisInfinity(consdata1->rhs)) )
      {
      ...
      if( cons1dominatesrhs && (!cons0isequality || cons1dominateslhs || RisInfinity(-consdata0->lhs)) )
      {
      ...
      else if( cons0dominatesrhs && (!cons1isequality || cons0dominateslhs || RisInfinity(-consdata1->lhs)) )
      {
      ...
      if( RisInfinity(-consdata0->lhs) && RisInfinity(consdata0->rhs) )
      {
      ...
      if( RisInfinity(-consdata1->lhs) && RisInfinity(consdata1->rhs) )
      {
   ...
   )
{
   ...
   if( !RisInfinity(consdata->rhs) && !RisInfinity(-consdata->lhs) )
      return SCIP_OKAY;
   ...
   if( v < nvars )
   {
      ...
      for( v = 0; v < nvars; ++v )
      {
         ...
         else
         {
             ...
             */
            if( RisInfinity(-lb) || RisInfinity(ub) )
            {
               tryfixing = FALSE;
               break;
            }
    ...
    */
   if( singlevarstuffing && !RisInfinity(-minactivity) )
   {
   ...
   )
{
   ...
   for( v = 0; v < ncontvars; v++ )
   {
      ...
      if( SCIPisZero(scip, obj) )
         isimplint[v] = (RisInfinity(-lb) || SCIPisIntegral(scip, lb)) && (RisInfinity(ub) || SCIPisIntegral(scip, ub));
      else
      {
         if( SCIPisPositive(scip, obj) )
            isimplint[v] = (RisInfinity(-lb) || SCIPisIntegral(scip, lb));
         else
         {
            assert(SCIPisNegative(scip, obj));
            isimplint[v] = (RisInfinity(ub) || SCIPisIntegral(scip, ub));
         }
      }
   }
   ...
   for( c = 0; c < nconss; ++c )
   {
      ...
      if( SCIPconsIsLocked(conss[c]) )
      {
         ...
         for( i = 0; i < consdata->nvars; ++i )
         {
            ...
            if( SCIPvarIsIntegral(var) )
            {
               if( !RisInfinity(newredlb) )
                  newredlb = SCIPceil(scip, newredlb);
               if( !RisInfinity(-newredub) )
...
SCIP_Bool isFiniteNonnegativeIntegral(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_Rational*        x                   /**< value */
   )
{
   assert(scip != NULL);

   return (!RisInfinity(x) && !SCIPisNegative(scip, x) && SCIPisIntegral(scip, x));
}
#endif
   ...
   )
{
   ...
   for( c = 0; c < nconss; c++ )
   {
      ...
      if( RisInfinity(rhs) && RisInfinity(-lhs) )
      {
      ...
      /* is constraint of type SCIP_CONSTYPE_{SETPARTITION, SETPACKING, SETCOVERING, CARDINALITY, INVKNAPSACK}? */
      {
         ...
         if( !unmatched )
         {
            ...
            else if( !RisInfinity(b) && SCIPisIntegral(scip, b) && !SCIPisNegative(scip, b) )
            {
            ...
            if( RisInfinity(-lhs) && RisInfinity(rhs) )
               continue;
         }
      }
...
{  /*lint --e{715}*/
   ...
   for( c = 0; c < nconss; ++c )
   {
      ...
      if( consdata->rowlhs != NULL )
      {
         ...
         for (int i = 0; i < consdata->rowexact->len; i++)
         {
            SCIP_CALL( consDropRowEvent(scip, conss[c], SCIPconshdlrGetData(conshdlr)->eventhdlr, i) );
         }
...
{  /*lint --e{715}*/
   ...
   if (SCIPisCertificateActive(scip))
   {
      ...
      SCIPhashmapRemove(certificate->rowdatahash, cons);
   }
...
{  /*lint --e{715}*/
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   maxbound = glblowerbound + RatApproxReal(conshdlrdata->maxcardbounddist) * (cutoffbound - glblowerbound);
      ...
      if( violated )
      {
         *result = SCIP_INFEASIBLE;
         ...
         if( printreason )
         {
            ...
            if( RatIsAbsInfinity(activity) ) /*lint !e777*/
...
{  /*lint --e{715}*/
   ...
   haslhs = !RatIsNegInfinity(consdata->lhs);
   hasrhs = !RatIsInfinity(consdata->rhs);
...
{  /*lint --e{715}*/
   ...
   SCIP_CALL( SCIPcopyConsExactLinear(scip, cons, sourcescip, consname, nvars, sourcevars, sourcecoefs,
         RatApproxReal(SCIPgetLhsExactLinear(sourcescip, sourcecons)), RatApproxReal(SCIPgetRhsExactLinear(sourcescip, sourcecons)), varmap, consmap,
...
{  /*lint --e{715}*/
   ...
   if( ((eventtype & SCIP_EVENTTYPE_BOUNDCHANGED) != 0) && updateActivities )
   {
      ...
      assert(var != NULL);
      assert(GetCertificateVar(consdata, varpos) == var);
      valrange = ccons.valsreal[varpos];
      ...
      if( SCIPconsIsActive(cons) )
      {
         ...
         if( (eventtype & SCIP_EVENTTYPE_LBCHANGED) != 0 )
            consdataUpdateActivitiesLb(scip, consdata, var, oldbound, newbound, valrange, TRUE);
         else
         {
            assert((eventtype & SCIP_EVENTTYPE_UBCHANGED) != 0);
            consdataUpdateActivitiesUb(scip, consdata, var, oldbound, newbound, valrange, TRUE);
         }
      }
      else
         consdataInvalidateActivities(consdata);
      ...
      if( (eventtype & SCIP_EVENTTYPE_BOUNDTIGHTENED) != 0 )
      {
         ...
         if( consdata->boundstightened > 0)
         {
            ...
            case SCIP_EVENTTYPE_LBTIGHTENED:
               if( (valrange.sup > 0.0 ? !SCIPisInfinity(scip, ccons.rhsreal) : !SCIPisInfinity(scip, -ccons.lhsreal)) )
                  consdata->boundstightened = 0;
               break;
            case SCIP_EVENTTYPE_UBTIGHTENED:
               if( (valrange.sup > 0.0 ? !SCIPisInfinity(scip, -ccons.lhsreal) : !SCIPisInfinity(scip, ccons.rhsreal)) )
                  consdata->boundstightened = 0;
               break;
            default:
   ...
   else if(!eventdata->rowvar && (eventtype & SCIP_EVENTTYPE_VARUNLOCKED) != 0 )
   {
   ...
   else if( (eventtype & SCIP_EVENTTYPE_GBDCHANGED) != 0 )
   {
      ...
      if (updateActivities) {
         ...
         if( (eventtype & SCIP_EVENTTYPE_GLBCHANGED) != 0 )
            consdataUpdateActivitiesGlbLb(scip, consdata, oldbound, newbound, valrange, TRUE);
         else
         {
            assert((eventtype & SCIP_EVENTTYPE_GUBCHANGED) != 0);
            consdataUpdateActivitiesGlbUb(scip, consdata, oldbound, newbound, valrange, TRUE);
         }
      }
      /* if the variable is binary but not fixed it had to become binary due to this global change */
      if(!eventdata->rowvar && SCIPvarIsBinary(var) && SCIPisGT(scip, SCIPvarGetUbGlobal(var), SCIPvarGetLbGlobal(var)) )
      {
         if( SCIPgetStage(scip) < SCIP_STAGE_INITSOLVE )
            consdata->indexsorted = FALSE;
         else
   ...
   else if( ((eventtype & SCIP_EVENTTYPE_TYPECHANGED) != 0) && !eventdata->rowvar)
   {
   ...
   else if ((eventtype & SCIP_EVENTTYPE_VARDELETED) && !eventdata->rowvar)
   {
   ...
   )
{
   ...
   if( SCIPgetStage(scip) >= SCIP_STAGE_EXITPRESOLVE && nvars > 0 )
   {
      ...
      if( RatIsAbsInfinity(constant) )
      {
         if( RatIsNegative(constant) )
         {
            if( RatIsInfinity(lhs) )
            {
   ...
   )
{
   ...
   SCIP_CALL( RatCreateBuffer(SCIPbuffer(scip), &temp) );

   SCIP_CALL( SCIPchgCoefExactLinear(scip, cons, var, temp) );

   RatFreeBuffer(SCIPbuffer(scip), &temp);
   ...
   )
{
   ...
   if( strcmp(SCIPconshdlrGetName(SCIPconsGetHdlr(cons)), CONSHDLR_NAME) != 0 )
   {
      SCIPerrorMessage("constraint is not linear\n");
      SCIPABORT();
      RatSetString(ret, "inf");  /*lint !e527*/
   }
   ...
   )
{
   ...
   if( consdata->rowexact != NULL )
      SCIPgetRowSolFeasibilityExact(scip, consdata->rowexact, sol, ret);
   else
