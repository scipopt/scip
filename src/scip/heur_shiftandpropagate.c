...
/*                                                                           */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
#pragma ident "@(#) $Id: heur_shiftandpropagate.c,v 1.16.2.5 2011/02/09 14:14:04 bzfhende Exp $"
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   for( c = 0; c < ndiscvars; ++c )
   {
      ...
      if( probing )
      {
         ...
         ndomredsfound = 0;
         SCIP_CALL( SCIPpropagateProbing(scip, DEFAULT_NPROPROUNDS, &cutoff, &ndomredsfound) );
         ++nprobings;
         STATISTIC(
      ...
      if( cutoff )
      {
         ...
         if( ncutoffs >= DEFAULT_CUTOFFBREAKER )
            break;
