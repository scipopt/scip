   ...
   for (c = 0; c < nconss; ++c)
   {
      ...
      if ( SCIPconsIsActive(lincons) )
      {
         ...
         if ( SCIPisEfficacious(scip, cutval - cutrhs) )
         {
            ...
            SCIP_CALL( SCIPcreateEmptyRowConshdlr(scip, &row, SCIPconsGetHdlr(conss[c]), name, -SCIPinfinity(scip), cutrhs, islocal, FALSE, conshdlrdata->removable) );
      ...
      for (c = 0; c < nusefulconss && ncuts < maxsepacuts; ++c)
      {
         ...
         if ( ub <= conshdlrdata->sepacouplingvalue )
         {
            ...
            if ( SCIPisEfficacious(scip, activity) )
            {
               ...
               (void) SCIPsnprintf(name, 50, "couple%d", c);
               SCIP_CALL( SCIPcreateEmptyRowConshdlr(scip, &row, SCIPconsGetHdlr(conss[c]), name, -SCIPinfinity(scip), ub, islocal, FALSE, conshdlrdata->removable) );
#else
               SCIP_CALL( SCIPcreateEmptyRowConshdlr(scip, &row, SCIPconsGetHdlr(conss[c]), "", -SCIPinfinity(scip), ub, islocal, FALSE, conshdlrdata->removable) );
#endif
               SCIP_CALL( SCIPcacheRowExtensions(scip, row) );
   ...
   for (c = 0; c < nconss && !(*infeasible); ++c)
   {
      ...
      if ( ub <= conshdlrdata->maxcouplingvalue )
      {
         ...
         else
         {
            ...
            SCIP_CALL( SCIPcreateEmptyRowConshdlr(scip, &row, conshdlr, name, -SCIPinfinity(scip), ub, FALSE, FALSE, FALSE) );
            SCIP_CALL( SCIPcacheRowExtensions(scip, row) );
