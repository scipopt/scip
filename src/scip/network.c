...
struct SCIP_NetworkDecomposition {
    int numArcs;
    int memArcs;
...
#ifndef NDEBUG
static SCIP_Bool nodeIsRepresentative(const SCIP_NETWORKDECOMP *dec, spqr_node node) {
    assert(dec);
...
static spqr_node findNode(SCIP_NETWORKDECOMP *dec, spqr_node node) {
    assert(dec);
    assert(SPQRnodeIsValid(node));
    assert(node < dec->memNodes);
...
static spqr_node findNodeNoCompression(const SCIP_NETWORKDECOMP *dec, spqr_node node) {
    assert(dec);
    assert(SPQRnodeIsValid(node));
    assert(node < dec->memNodes);
...
static spqr_node findArcTail(SCIP_NETWORKDECOMP *dec, spqr_arc arc) {
    assert(dec);
...
static spqr_node findArcHead(SCIP_NETWORKDECOMP *dec, spqr_arc arc) {
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
...
static spqr_node findArcHeadNoCompression(const SCIP_NETWORKDECOMP *dec, spqr_arc arc) {
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
...
static spqr_node findArcTailNoCompression(const SCIP_NETWORKDECOMP *dec, spqr_arc arc) {
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
...
static spqr_arc getFirstNodeArc(const SCIP_NETWORKDECOMP * dec, spqr_node node){
    assert(dec);
    assert(SPQRnodeIsValid(node));
    assert(node < dec->memNodes);
    return dec->nodes[node].firstArc;
}
static spqr_arc getNextNodeArcNoCompression(const SCIP_NETWORKDECOMP * dec, spqr_arc arc, spqr_node node){
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
...
static spqr_arc getNextNodeArc(SCIP_NETWORKDECOMP * dec, spqr_arc arc, spqr_node node){
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
    assert(nodeIsRepresentative(dec,node));
...
static spqr_arc getPreviousNodeArc(SCIP_NETWORKDECOMP *dec, spqr_arc arc, spqr_node node){
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
    assert(nodeIsRepresentative(dec,node));
...
static void mergeNodeArcList(SCIP_NETWORKDECOMP *dec, spqr_node toMergeInto, spqr_node toRemove){
...
static void arcFlipReversed(SCIP_NETWORKDECOMP *dec, spqr_arc arc){
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
    dec->arcs[arc].reversed = !dec->arcs[arc].reversed;
}
static void arcSetReversed(SCIP_NETWORKDECOMP *dec, spqr_arc arc, SCIP_Bool reversed){
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
    dec->arcs[arc].reversed = reversed;
}
static void arcSetRepresentative(SCIP_NETWORKDECOMP *dec, spqr_arc arc, spqr_arc representative){
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
...
static spqr_node mergeNodes(SCIP_NETWORKDECOMP *dec, spqr_node first, spqr_node second) {
    assert(dec);
...
static SCIP_Bool memberIsRepresentative(const SCIP_NETWORKDECOMP *dec, spqr_member member) {
    assert(dec);
...
static spqr_member findMember(SCIP_NETWORKDECOMP *dec, spqr_member member) {
    assert(dec);
    assert(member < dec->memMembers);
    assert(SPQRmemberIsValid(member));
...
static spqr_member findMemberNoCompression(const SCIP_NETWORKDECOMP *dec, spqr_member member) {
    assert(dec);
    assert(member < dec->memMembers);
    assert(SPQRmemberIsValid(member));
...
static spqr_member mergeMembers(SCIP_NETWORKDECOMP *dec, spqr_member first, spqr_member second) {
    assert(dec);
...
static spqr_member findArcMember(SCIP_NETWORKDECOMP *dec, spqr_arc arc) {
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
...
static spqr_member findArcMemberNoCompression(const SCIP_NETWORKDECOMP *dec, spqr_arc arc) {
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
...
static spqr_member findMemberParent(SCIP_NETWORKDECOMP *dec, spqr_member member) {
    assert(dec);
    assert(member < dec->memMembers);
    assert(SPQRmemberIsValid(member));
...
static spqr_member findMemberParentNoCompression(const SCIP_NETWORKDECOMP *dec, spqr_member member) {
    assert(dec);
    assert(member < dec->memMembers);
    assert(SPQRmemberIsValid(member));
    assert(memberIsRepresentative(dec,member));
...
static spqr_member findArcChildMember(SCIP_NETWORKDECOMP *dec, spqr_arc arc) {
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
...
static spqr_member findArcChildMemberNoCompression(const SCIP_NETWORKDECOMP *dec, spqr_arc arc) {
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
...
static SCIP_Bool arcIsMarker(const SCIP_NETWORKDECOMP *dec, spqr_arc arc) {
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
...
static SCIP_Bool arcIsTree(const SCIP_NETWORKDECOMP *dec, spqr_arc arc) {
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
...
//find
#ifndef NDEBUG
static SCIP_Bool arcIsRepresentative(const SCIP_NETWORKDECOMP *dec, spqr_arc arc) {
    assert(dec);
    assert(arc < dec->memArcs);
    assert(SPQRarcIsValid(arc));
...
#endif
static ArcSign findArcSign(SCIP_NETWORKDECOMP *dec, spqr_arc arc) {
    assert(dec);
    assert(arc < dec->memArcs);
    assert(SPQRarcIsValid(arc));
...
static ArcSign findArcSignNoCompression(const SCIP_NETWORKDECOMP *dec, spqr_arc arc) {
    assert(dec);
    assert(arc < dec->memArcs);
    assert(SPQRarcIsValid(arc));
...
static spqr_node findEffectiveArcHead(SCIP_NETWORKDECOMP *dec, spqr_arc arc){
    assert(dec);
...
static spqr_node findEffectiveArcTail(SCIP_NETWORKDECOMP *dec, spqr_arc arc){
    assert(dec);
    if(findArcSign(dec,arc).reversed){
        return findArcHead(dec,arc);
    }else{
...
static spqr_node findEffectiveArcHeadNoCompression(const SCIP_NETWORKDECOMP *dec, spqr_arc arc){
    assert(dec);
...
static spqr_node findEffectiveArcTailNoCompression(const SCIP_NETWORKDECOMP *dec, spqr_arc arc){
    assert(dec);
    if(findArcSignNoCompression(dec,arc).reversed){
        return findArcHeadNoCompression(dec,arc);
    }else{
...
static spqr_arc mergeArcSigns(SCIP_NETWORKDECOMP *dec, spqr_arc first, spqr_arc second,
...
static SCIP_Bool arcIsReversedNonRigid(const SCIP_NETWORKDECOMP *dec, spqr_arc arc){
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
...
static spqr_element arcGetElement(const SCIP_NETWORKDECOMP * dec, spqr_arc arc){
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
...
SCIP_Bool SCIPNetworkDecompositionContainsRow(const SCIP_NETWORKDECOMP * dec, spqr_row row){
    assert(SPQRrowIsValid(row) && (int) row < dec->memRows);
    assert(dec);
    return SPQRarcIsValid(dec->rowArcs[row]);
}
SCIP_Bool SCIPNetworkDecompositionContainsColumn(const SCIP_NETWORKDECOMP *dec, spqr_col col){
    assert(SPQRcolIsValid(col) && (int) col < dec->memColumns);
    assert(dec);
    return SPQRarcIsValid(dec->columnArcs[col]);
}
static void setDecompositionColumnArc(SCIP_NETWORKDECOMP *dec, spqr_col col, spqr_arc arc){
    assert(SPQRcolIsValid(col) && (int)col < dec->memColumns);
    assert(dec);
    assert(SPQRarcIsValid(arc));
    dec->columnArcs[col] = arc;
}
static void setDecompositionRowArc(SCIP_NETWORKDECOMP *dec, spqr_row row, spqr_arc arc){
    assert(SPQRrowIsValid(row) && (int) row < dec->memRows);
    assert(dec);
    assert(SPQRarcIsValid(arc));
    dec->rowArcs[row] = arc;
}
static spqr_arc getDecompositionColumnArc(const SCIP_NETWORKDECOMP *dec, spqr_col col){
    assert(SPQRcolIsValid(col) && (int) col < dec->memColumns);
    assert(dec);
    return dec->columnArcs[col];
}
static spqr_arc getDecompositionRowArc(const SCIP_NETWORKDECOMP *dec, spqr_row row){
    assert(SPQRrowIsValid(row) && (int) row < dec->memRows);
    assert(dec);
...
SCIP_RETCODE SCIPNetworkDecompositionCreate(SCIP* env, SCIP_NETWORKDECOMP **pDecomposition, int numRows, int numColumns){
    ...
    SCIP_CALL(SCIPallocBlockMemory(env, pDecomposition));
    SCIP_NETWORKDECOMP *dec = *pDecomposition;
    dec->env = env;
    ...
    int initialMemArcs = 8;
    {
        ...
        dec->numArcs = 0;
        SCIP_CALL(SCIPallocBlockMemoryArray(env, &dec->arcs, (size_t) dec->memArcs));
    ...
    int initialMemMembers = 8;
    {
        ...
        dec->numMembers = 0;
        SCIP_CALL(SCIPallocBlockMemoryArray(env, &dec->members, (size_t) dec->memMembers));
    }
    ...
    int initialMemNodes = 8;
    {
        ...
        dec->numNodes = 0;
        SCIP_CALL(SCIPallocBlockMemoryArray(env, &dec->nodes, (size_t) dec->memNodes));
    }

    //Initialize mappings for rows
    {
        dec->memRows = numRows;
        SCIP_CALL(SCIPallocBlockMemoryArray(env, &dec->rowArcs, (size_t) dec->memRows));
    ...
    //Initialize mappings for columns
    {
        ...
        dec->numColumns = 0;
        SCIP_CALL(SCIPallocBlockMemoryArray(env, &dec->columnArcs, (size_t) dec->memColumns));
...
void SCIPNetworkDecompositionFree(SCIP_NETWORKDECOMP **pDec){
    assert(pDec);
    assert(*pDec);

    SCIP_NETWORKDECOMP *dec = *pDec;
...
static spqr_arc getFirstMemberArc(const SCIP_NETWORKDECOMP * dec, spqr_member member){
    assert(dec);
    assert(SPQRmemberIsValid(member));
    assert(member < dec->memMembers);
    return dec->members[member].firstArc;
}
static spqr_arc getNextMemberArc(const SCIP_NETWORKDECOMP * dec, spqr_arc arc){
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
...
static spqr_arc getPreviousMemberArc(const SCIP_NETWORKDECOMP *dec, spqr_arc arc){
    assert(dec);
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
...
static void addArcToMemberArcList(SCIP_NETWORKDECOMP *dec, spqr_arc arc, spqr_member member){
...
static SCIP_RETCODE createArc(SCIP_NETWORKDECOMP *dec, spqr_member member,SCIP_Bool reversed, spqr_arc *pArc) {
    assert(dec);
    assert(pArc);
    ...
    if (SPQRarcIsValid(index)) {
        ...
        SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env, &dec->arcs, dec->memArcs, (size_t) newSize));
...
static SCIP_RETCODE createRowArc(SCIP_NETWORKDECOMP *dec, spqr_member member, spqr_arc *pArc,
                               spqr_row row, SCIP_Bool reversed){
...
static SCIP_RETCODE createColumnArc(SCIP_NETWORKDECOMP *dec, spqr_member member, spqr_arc *pArc,
                                  spqr_col column, SCIP_Bool reversed){
    SCIP_CALL(createArc(dec,member,reversed,pArc));
...
static SCIP_RETCODE createMember(SCIP_NETWORKDECOMP *dec, SPQRMemberType type, spqr_member * pMember){
    assert(dec);
    assert(pMember);
...
static SCIP_RETCODE createNode(SCIP_NETWORKDECOMP *dec, spqr_node * pNode){
...
static void removeArcFromNodeArcList(SCIP_NETWORKDECOMP *dec, spqr_arc arc, spqr_node node, SCIP_Bool nodeIsHead){
    ...
    if(dec->nodes[node].numArcs == 1){
        ...
        SPQRNetworkDecompositionArcListNode * nextListNode = findArcHead(dec, next_arc) == node ? &dec->arcs[next_arc].headArcListNode : &dec->arcs[next_arc].tailArcListNode;
        SPQRNetworkDecompositionArcListNode * prevListNode = findArcHead(dec, prev_arc) == node ? &dec->arcs[prev_arc].headArcListNode : &dec->arcs[prev_arc].tailArcListNode;
...
static void addArcToNodeArcList(SCIP_NETWORKDECOMP *dec, spqr_arc arc, spqr_node node, SCIP_Bool nodeIsHead){
    assert(nodeIsRepresentative(dec,node));
...
static void setArcHeadAndTail(SCIP_NETWORKDECOMP *dec, spqr_arc arc, spqr_node head, spqr_node tail){
    addArcToNodeArcList(dec,arc,head,TRUE);
    addArcToNodeArcList(dec,arc,tail,FALSE);
}
static void clearArcHeadAndTail(SCIP_NETWORKDECOMP *dec, spqr_arc arc){
...
static void changeArcHead(SCIP_NETWORKDECOMP *dec, spqr_arc arc, spqr_node oldHead, spqr_node newHead){
...
static void changeArcTail(SCIP_NETWORKDECOMP *dec, spqr_arc arc, spqr_node oldTail, spqr_node newTail){
...
static int nodeDegree(SCIP_NETWORKDECOMP *dec, spqr_node node){
    assert(dec);
    assert(SPQRnodeIsValid(node));
    assert(node < dec->memNodes);
    return dec->nodes[node].numArcs;
}
static SPQRMemberType getMemberType(const SCIP_NETWORKDECOMP *dec, spqr_member member){
    assert(dec);
    assert(SPQRmemberIsValid(member));
    assert(member < dec->memMembers);
    assert(memberIsRepresentative(dec,member));
...
static void updateMemberType(const SCIP_NETWORKDECOMP *dec, spqr_member member, SPQRMemberType type){
    assert(dec);
    assert(SPQRmemberIsValid(member));
    assert(member < dec->memMembers);
    assert(memberIsRepresentative(dec,member));
...
static spqr_arc markerToParent(const SCIP_NETWORKDECOMP *dec, spqr_member member){
    assert(dec);
    assert(SPQRmemberIsValid(member));
    assert(member < dec->memMembers);
    assert(memberIsRepresentative(dec,member));
...
static void updateMemberParentInformation(SCIP_NETWORKDECOMP *dec, const spqr_member newMember, const spqr_member toRemove){
...
static spqr_arc markerOfParent(const SCIP_NETWORKDECOMP *dec, spqr_member member) {
    assert(dec);
    assert(SPQRmemberIsValid(member));
    assert(member < dec->memMembers);
    assert(memberIsRepresentative(dec,member));
...
static int getNumMemberArcs(const SCIP_NETWORKDECOMP * dec, spqr_member member){
    assert(dec);
    assert(SPQRmemberIsValid(member));
    assert(member < dec->memMembers);
    assert(memberIsRepresentative(dec,member));
...
static int getNumNodes(const SCIP_NETWORKDECOMP *dec){
    assert(dec);
    return dec->numNodes;
}
static int getNumMembers(const SCIP_NETWORKDECOMP *dec){
    assert(dec);
    return dec->numMembers;
}
static SCIP_RETCODE createStandaloneParallel(SCIP_NETWORKDECOMP *dec, spqr_col * columns, SCIP_Bool * reversed,
...
static SCIP_RETCODE createConnectedParallel(SCIP_NETWORKDECOMP *dec, spqr_col * columns, SCIP_Bool * reversed,
                                          int num_columns, spqr_row row, spqr_member * pMember){
    spqr_member member;
...
static SCIP_RETCODE createStandaloneSeries(SCIP_NETWORKDECOMP *dec, spqr_row * rows, SCIP_Bool *reversed,
...
static SCIP_RETCODE createConnectedSeries(SCIP_NETWORKDECOMP *dec, spqr_row * rows, SCIP_Bool *reversed,
                                        int numRows, spqr_col col, spqr_member * pMember){
    spqr_member member;
...
static void removeArcFromMemberArcList(SCIP_NETWORKDECOMP *dec, spqr_arc arc, spqr_member member){
...
static int decompositionGetFundamentalCycleRows(const SCIP_NETWORKDECOMP *dec, spqr_col column, spqr_row * output,
    ...
    FindCycleCall * callStack;
    SCIP_RETCODE result = SCIPallocBlockMemoryArray(dec->env,&callStack,(size_t) dec->memRows);
    if(result != SCIP_OKAY){
        return -1;
    }
    int callStackSize = 1;
    ...
    result = SCIPallocBlockMemoryArray(dec->env,&nodeVisited,(size_t) dec->numNodes);
    if(result != SCIP_OKAY){
        return -1;
    }
    ...
    result = SCIPallocBlockMemoryArray(dec->env,&pathSearchCallStack,(size_t) dec->numNodes);
    if(result != SCIP_OKAY){
        return -1;
    }
            ...
            case SPQR_MEMBERTYPE_RIGID:
            {
                ...
                for (int i = 0; i < pathSearchCallStackSize; ++i) {
                    if(arcIsTree(dec,pathSearchCallStack[i].nodeArc)){
                        SCIP_Bool arcReversedInPath = findEffectiveArcHeadNoCompression(dec,pathSearchCallStack[i].nodeArc) == pathSearchCallStack[i].node;
...
static spqr_member largestMemberID(const SCIP_NETWORKDECOMP *dec){
    return dec->numMembers;
}
static spqr_arc largestArcID(const SCIP_NETWORKDECOMP *dec){
    return dec->numArcs;
}
static spqr_node largestNodeID(const SCIP_NETWORKDECOMP *dec){
    return dec->numNodes;
}
static int numConnectedComponents(const SCIP_NETWORKDECOMP *dec){
    return dec->numConnectedComponents;
}
static SCIP_RETCODE createChildMarker(SCIP_NETWORKDECOMP *dec, spqr_member member, spqr_member child, SCIP_Bool isTree,
                                    spqr_arc * pArc, SCIP_Bool reversed){
    SCIP_CALL(createArc(dec,member,reversed,pArc));
...
static SCIP_RETCODE createParentMarker(SCIP_NETWORKDECOMP *dec, spqr_member member, SCIP_Bool isTree, spqr_member parent, spqr_arc parentMarker
        , spqr_arc * arc,SCIP_Bool reversed){
...
static SCIP_RETCODE createMarkerPair(SCIP_NETWORKDECOMP *dec, spqr_member parentMember, spqr_member childMember,
    ...
    SCIP_CALL(createParentMarker(dec,childMember,!parentIsTree,parentMember,parentToChildMarker,&childToParentMarker,parentMarkerReversed));

    return SCIP_OKAY;
}
static SCIP_RETCODE createMarkerPairWithReferences(SCIP_NETWORKDECOMP *dec, spqr_member parentMember, spqr_member childMember, SCIP_Bool parentIsTree,
                                                 SCIP_Bool childMarkerReversed,
                                                 SCIP_Bool parentMarkerReversed,
    ...
    SCIP_CALL(createParentMarker(dec,childMember,!parentIsTree,parentMember,*parentToChild,childToParent,parentMarkerReversed));

    return SCIP_OKAY;
}

static void moveArcToNewMember(SCIP_NETWORKDECOMP *dec, spqr_arc arc, spqr_member oldMember, spqr_member newMember){
    assert(SPQRarcIsValid(arc));
    assert(arc < dec->memArcs);
    assert(dec);
...
static void mergeMemberArcList(SCIP_NETWORKDECOMP *dec, spqr_member toMergeInto, spqr_member toRemove){
...
static void changeLoopToSeries(SCIP_NETWORKDECOMP * dec, spqr_member member){
    assert(SPQRmemberIsValid(member));
    assert(member < dec->memMembers);
    assert(dec);
...
static void changeLoopToParallel(SCIP_NETWORKDECOMP * dec, spqr_member member){
    assert(SPQRmemberIsValid(member));
    assert(member < dec->memMembers);
    assert(dec);
    assert((getMemberType(dec,member) == SPQR_MEMBERTYPE_PARALLEL || getMemberType(dec, member) == SPQR_MEMBERTYPE_SERIES ||
...
SCIP_Bool SCIPNetworkDecompositionIsMinimal(const SCIP_NETWORKDECOMP * dec){
...
static void decreaseNumConnectedComponents(SCIP_NETWORKDECOMP *dec, int by){
    dec->numConnectedComponents-= by;
    assert(dec->numConnectedComponents >= 1);
}

static void reorderComponent(SCIP_NETWORKDECOMP *dec, spqr_member newRoot){
    assert(dec);
...
static void arcToDot(FILE * stream, const SCIP_NETWORKDECOMP * dec,
...
static void decompositionToDot(FILE * stream, const SCIP_NETWORKDECOMP *dec, SCIP_Bool useElementNames ){
...
static SCIP_RETCODE mergeGivenMemberIntoParent(SCIP_NETWORKDECOMP *dec,
                                             spqr_member member,
                                             spqr_member parent,
...
struct SCIP_NetworkColAddition {
...
static void cleanupPreviousIteration(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION *newCol) {
    assert(dec);
    assert(newCol);
...
SCIP_RETCODE SCIPNetworkColAdditionCreate(SCIP*env, SCIP_NETWORKCOLADDITION **pNewCol) {
    assert(env);

    SCIP_CALL(SCIPallocBlockMemory(env, pNewCol));
    SCIP_NETWORKCOLADDITION *newCol = *pNewCol;
...
void SCIPNetworkColAdditionFree(SCIP * env, SCIP_NETWORKCOLADDITION **pNewCol) {
    assert(env);
    SCIP_NETWORKCOLADDITION *newCol = *pNewCol;
    SCIPfreeBlockMemoryArray(env, &newCol->decompositionRowArcs,newCol->memDecompositionRowArcs);
    SCIPfreeBlockMemoryArray(env, &newCol->decompositionArcReversed,newCol->memDecompositionRowArcs);
    SCIPfreeBlockMemoryArray(env, &newCol->newRowArcs,newCol->memNewRowArcs);
    SCIPfreeBlockMemoryArray(env, &newCol->newRowArcReversed,newCol->memNewRowArcs);
    SCIPfreeBlockMemoryArray(env, &newCol->createReducedMembersCallStack,newCol->memCreateReducedMembersCallStack);
    SCIPfreeBlockMemoryArray(env, &newCol->arcInPath,newCol->memArcsInPath);
    SCIPfreeBlockMemoryArray(env, &newCol->arcInPathReversed,newCol->memArcsInPath);
    SCIPfreeBlockMemoryArray(env, &newCol->nodeInPathDegree,newCol->memNodePathDegree);
    SCIPfreeBlockMemoryArray(env, &newCol->nodeOutPathDegree,newCol->memNodePathDegree);
    SCIPfreeBlockMemoryArray(env, &newCol->pathArcs,newCol->memPathArcs);
    SCIPfreeBlockMemoryArray(env, &newCol->childrenStorage,newCol->memChildrenStorage);
    SCIPfreeBlockMemoryArray(env, &newCol->memberInformation,newCol->memMemberInformation);
    SCIPfreeBlockMemoryArray(env, &newCol->reducedComponents,newCol->memReducedComponents);
    SCIPfreeBlockMemoryArray(env, &newCol->reducedMembers,newCol->memReducedMembers);
    SCIPfreeBlockMemoryArray(env, &newCol->leafMembers,newCol->memLeafMembers);

    SCIPfreeBlockMemory(env, pNewCol);
}


static reduced_member_id createReducedMembersToRoot(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION * newCol, const spqr_member firstMember ){
...
static SCIP_RETCODE constructReducedDecomposition(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION *newCol) {
    assert(dec);
    assert(newCol);
#ifndef NDEBUG
    ...
    if (newSize > newCol->memReducedMembers) {
        ...
        SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env, &newCol->reducedMembers,newCol->memReducedMembers,newArraySize));
    ...
    if (newSize > newCol->memMemberInformation) {
        ...
        SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env, &newCol->memberInformation,newCol->memMemberInformation,updatedSize));
    ...
    if (numComponents > newCol->memReducedComponents) {
        ...
        SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env, &newCol->reducedComponents,newCol->memReducedComponents,updatedSize));
    ...
    if (newCol->memChildrenStorage < numTotalChildren) {
        ...
        SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env, &newCol->childrenStorage, newCol->memChildrenStorage, newMemSize));
...
static void cleanUpMemberInformation(SCIP_NETWORKCOLADDITION * newCol){
...
static void createPathArc(
        SCIP_NETWORKDECOMP * dec, SCIP_NETWORKCOLADDITION * newCol,
        const spqr_arc arc, const reduced_member_id reducedMember, SCIP_Bool reversed){
    assert(dec);
    assert(newCol);
...
static SCIP_RETCODE createPathArcs(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION *newCol){
 ...
 */
static SCIP_RETCODE
newColUpdateColInformation(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION *newCol, spqr_col column,
        ...
        if (SPQRarcIsValid(rowArc)) { //If the arc is the current decomposition: save it in the array
            if (newCol->numDecompositionRowArcs == newCol->memDecompositionRowArcs) {
                ...
                SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env, &newCol->decompositionArcReversed,
                                                (size_t) newCol->memDecompositionRowArcs,newNumArcs));
                newCol->memDecompositionRowArcs = newNumArcs;
            ...
            if (newCol->numNewRowArcs == newCol->memNewRowArcs) {
                ...
                SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env, &newCol->newRowArcs,
                                                (size_t) newCol->memNewRowArcs,newNumArcs));
...
static SCIP_RETCODE computeLeafMembers(const SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION *newCol) {
...
static void determineRigidPath(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION *newCol, SPQRColReducedMember * redMem){
    assert(dec);
    assert(newCol);
    assert(redMem);
...
static void determineSingleRigidType(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION *newCol, reduced_member_id reducedMember,
                                     spqr_member member){
    assert(dec);
    assert(newCol);
...
static void determineSingleComponentType(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION *newCol, reduced_member_id reducedMember){
    assert(dec);
    assert(newCol);
...
static void determinePathSeriesType(SCIP_NETWORKDECOMP * dec, SCIP_NETWORKCOLADDITION *newCol,
...
static void determinePathParallelType(SCIP_NETWORKDECOMP * dec, SCIP_NETWORKCOLADDITION *newCol,
                                      reduced_member_id reducedMember, spqr_member member, MemberPathType previousType,
                                      spqr_arc source, spqr_arc target) {
    assert(dec);
    assert(newCol);
    ...
    if(pathArcIsValid(pathArc)){
        SCIP_Bool pathArcReversed = newCol->pathArcs[pathArc].reversed != arcIsReversedNonRigid(dec,newCol->pathArcs[pathArc].arc);
...
static void determinePathRigidType(SCIP_NETWORKDECOMP * dec, SCIP_NETWORKCOLADDITION *newCol,
                                   reduced_member_id reducedMember, spqr_member member, MemberPathType previousType,
                                   spqr_arc source, spqr_arc target) {
...
static void determinePathMemberType(SCIP_NETWORKDECOMP * dec, SCIP_NETWORKCOLADDITION *newCol,
                                    reduced_member_id reducedMember, spqr_member member, MemberPathType previousType,
                                    spqr_arc source, spqr_arc target){
...
static void determinePathTypes(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION *newCol, SPQRColReducedComponent * component){
    assert(dec);
    assert(newCol);
    assert(component);
...
static void checkRigidLeaf(SCIP_NETWORKDECOMP * dec,SCIP_NETWORKCOLADDITION * newCol, reduced_member_id leaf,
...
static ReducedMemberType checkLeaf(SCIP_NETWORKDECOMP * dec,SCIP_NETWORKCOLADDITION * newCol, reduced_member_id leaf,
                                   spqr_arc toParent, reduced_member_id parent, spqr_arc toChild){
    assert(dec);
    assert(newCol);
...
static void propagateCycles(SCIP_NETWORKDECOMP * dec, SCIP_NETWORKCOLADDITION * newCol){
    assert(dec);
    assert(newCol);
...
static void determineComponentTypes(SCIP_NETWORKDECOMP * dec, SCIP_NETWORKCOLADDITION * newCol, SPQRColReducedComponent * component){
    assert(dec);
    assert(newCol);
    assert(component);
...
SCIP_RETCODE
SCIPNetworkColAdditionCheck(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION *newCol, spqr_col column, const spqr_row * nonzeroRows,
    ...
    //Store call data
    SCIP_CALL(newColUpdateColInformation(dec, newCol, column, nonzeroRows, nonzeroValues, numNonzeros));

    //compute reduced decomposition
    SCIP_CALL(constructReducedDecomposition(dec, newCol));
    //initialize path arcs in reduced decomposition
    SCIP_CALL(createPathArcs(dec,newCol));
    SCIP_CALL(computeLeafMembers(dec,newCol));
...
static SCIP_RETCODE splitParallel(SCIP_NETWORKDECOMP *dec, spqr_member parallel,
...
static SCIP_RETCODE splitSeries(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION * newCol,
...
static SCIP_RETCODE splitSeriesMerging(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION * newCol,
                                     SPQRColReducedMember * reducedMember,
                                     spqr_member member,
...
static SCIP_RETCODE transformFirstPathMember(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION * newCol,
    ...
    SCIP_CALL(splitSeriesMerging(dec,newCol,redMem,member,&pathRepresentative,&nonPathRepresentative,target,SPQR_INVALID_ARC));
...
static SCIP_RETCODE transformAndMergeParallel(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION * newCol,
                                            reduced_member_id current, reduced_member_id next,
                                            spqr_member nextMember, SCIP_Bool nextIsParent,
                                            spqr_arc * representativeArc,
                                            spqr_member * mergedMember){
...
static SCIP_RETCODE transformAndMergeSeries(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION * newCol,
                                          reduced_member_id current, reduced_member_id next,
                                          spqr_member nextMember, SCIP_Bool nextIsParent,
                                          spqr_arc * representativeArc,
                                          spqr_member * mergedMember,
    ...
    spqr_arc pathRepresentative = SPQR_INVALID_ARC;
    spqr_arc nonPathRepresentative = SPQR_INVALID_ARC;
    SCIP_CALL(splitSeriesMerging(dec,newCol,redMem,nextMember,&pathRepresentative,&nonPathRepresentative,source,target));
...
static SCIP_RETCODE transformAndMergeRigid(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION * newCol,
                                         reduced_member_id current, reduced_member_id next,
                                         spqr_member nextMember, SCIP_Bool nextIsParent,
                                         spqr_arc * representativeArc,
                                         spqr_member * mergedMember,
...
static SCIP_RETCODE transformAndMerge(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION * newCol,
                                    reduced_member_id current, reduced_member_id next,
                                    spqr_arc * representativeArc,
                                    spqr_member * mergedMember,
                                    SCIP_Bool nextIsParent, NewColInformation * info){
...
static SCIP_RETCODE transformPath(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION * newCol,
    ...
    while(reducedMemberIsValid(next)){
        SCIP_CALL(transformAndMerge(dec,newCol,current,next,&representativeArc,&mergedMember,nextIsParent,newColInfo));
        current = next;
...
static SCIP_RETCODE columnTransformSingleParallel(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION *newCol,
...
static SCIP_RETCODE columnTransformSingleSeries(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION *newCol,
                                              reduced_member_id reducedMemberId, spqr_member member,
                                              NewColInformation *newColInfo){
...
static SCIP_RETCODE columnTransformSingleRigid(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION *newCol,
                                             reduced_member_id reducedMemberId, spqr_member member,
                                             NewColInformation *newColInfo){
    assert(dec);
    assert(newCol);
        ...
        if(SPQRarcIsValid(existingArcWithPath)){
            ...
            if(SPQRmemberIsValid(adjacentMember) && getMemberType(dec,adjacentMember) == SPQR_MEMBERTYPE_PARALLEL){
                ...
                if( element != MARKER_COLUMN_ELEMENT && element != MARKER_ROW_ELEMENT){
                    ...
                    SCIP_CALL(createChildMarker(dec,adjacentParallel,adjacentMember,arcIsTree(dec,existingArcWithPath),&duplicate,FALSE));
...
static SCIP_RETCODE transformComponent(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION *newCol, SPQRColReducedComponent * component, NewColInformation * newColInfo){
    assert(dec);
    assert(newCol);
    assert(component);
    assert(newColInfo);
    ...
    if(newCol->reducedMembers[component->root].numChildren == newCol->reducedMembers[component->root].numPropagatedChildren){
        ...
        switch(type) {
            ...
            case SPQR_MEMBERTYPE_PARALLEL: {
                SCIP_CALL(columnTransformSingleParallel(dec, newCol, reducedMember, member, newColInfo));
                break;
            }
            case SPQR_MEMBERTYPE_LOOP:
            case SPQR_MEMBERTYPE_SERIES: {
...
SCIP_RETCODE SCIPNetworkColAdditionAdd(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKCOLADDITION *newCol){
    ...
    if(newCol->numReducedComponents == 0){
        ...
        if(newCol->numNewRowArcs == 0){
            ...
            SCIP_CALL(createConnectedSeries(dec,newCol->newRowArcs,newCol->newRowArcReversed,newCol->numNewRowArcs,newCol->newColIndex,&newSeries));
            spqr_arc markerArc = SPQR_INVALID_ARC;
            spqr_arc ignore = SPQR_INVALID_ARC;
            SCIP_CALL(createMarkerPairWithReferences(dec,information.member,newSeries,FALSE,information.reversed,TRUE,&markerArc,&ignore));
            if(SPQRnodeIsValid(information.head)){
                assert(SPQRnodeIsValid(information.tail));
                assert(SPQRarcIsValid(information.representative));
...
typedef struct { //TODO:test if memory overhead of pointers is worth it?
    spqr_arc arc;
    spqr_node arcHead;
    spqr_node arcTail;
...
struct SCIP_NetworkRowAddition {
    SCIP_Bool remainsNetwork;
 ...
 * already part of the decomposition.
 */
static SCIP_RETCODE newRowUpdateRowInformation(const SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
 ...
 * @return
 */
static reduced_member_id createRowReducedMembersToRoot(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION * newRow, const spqr_member firstMember){
    assert(SPQRmemberIsValid(firstMember));
 ...
 * @return
 */
static SCIP_RETCODE constructRowReducedDecomposition(SCIP_NETWORKDECOMP* dec, SCIP_NETWORKROWADDITION* newRow){
#ifndef NDEBUG
    ...
    if(newSize > newRow->memMemberInformation){
        ...
        SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env,&newRow->memberInformation,newRow->memMemberInformation, updatedSize));
    ...
    if(numComponents > newRow->memReducedComponents){
        ...
        SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env,&newRow->reducedComponents,newRow->memReducedComponents,updatedSize));
    ...
    if(newRow->memCreateReducedMembersCallstack < numMembers){
        ...
        SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env,&newRow->createReducedMembersCallstack, newRow->memCreateReducedMembersCallstack,updatedSize));
    ...
    if(newRow->memChildrenStorage < numTotalChildren){
        ...
        SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env,&newRow->childrenStorage,(size_t) newRow->memChildrenStorage,newMemSize));
 ...
 * @param reducedMember
 */
static void createCutArc(SCIP_NETWORKDECOMP  * dec,
                         SCIP_NETWORKROWADDITION* newRow, const spqr_arc arc, const reduced_member_id reducedMember,
                         SCIP_Bool reversed){
 ...
 */
static SCIP_RETCODE createReducedDecompositionCutArcs(SCIP_NETWORKDECOMP* dec, SCIP_NETWORKROWADDITION* newRow){
 ...
 */
static SCIP_RETCODE determineLeafReducedMembers(const SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow) {
 ...
 */
static SCIP_RETCODE allocateRigidSearchMemory(const SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow){
    ...
    if(totalNumNodes > newRow->memArticulationNodes){
        ...
        SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env,&newRow->articulationNodes,newRow->memArticulationNodes, newSize));
    ...
    if(totalNumNodes > newRow->memNodeSearchInfo){
        ...
        SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env,&newRow->articulationNodeSearchInfo,newRow->memNodeSearchInfo, newSize));
    ...
    if(totalNumNodes > newRow->memCrossingPathCount){
        ...
        SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env,&newRow->crossingPathCount,newRow->memCrossingPathCount, newSize));
    ...
    if(largestID > newRow->memIntersectionDFSData){
        ...
        SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env,&newRow->intersectionDFSData,newRow->memIntersectionDFSData, newSize));
    ...
    if(largestID > newRow->memIntersectionPathDepth){
        ...
        SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env, &newRow->intersectionPathDepth,newRow->memIntersectionPathDepth, newSize));
    ...
    if(largestID > newRow->memIntersectionPathParent){
        ...
        SCIP_CALL(SCIPreallocBlockMemoryArray(dec->env, &newRow->intersectionPathParent,newRow->memIntersectionPathParent, newSize));
...
static void zeroOutColors(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow, const spqr_node firstRemoveNode){
...
static void cleanUpPreviousIteration(SCIP_NETWORKDECOMP * dec, SCIP_NETWORKROWADDITION * newRow){
...
static void rigidFindStarNodes(SCIP_NETWORKDECOMP * dec, SCIP_NETWORKROWADDITION * newRow,
...
static SCIP_RETCODE zeroOutColorsExceptNeighbourhood(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
    ...
    SCIP_CALL(SCIPallocBlockMemoryArray(dec->env,&neighbourColors,(size_t) degree));
...
static void intersectionOfAllPaths(SCIP_NETWORKDECOMP * dec, SCIP_NETWORKROWADDITION *newRow,
...
static void addArticulationNode(SCIP_NETWORKROWADDITION *newRow, spqr_node articulationNode){
#ifndef NDEBUG
...
static void articulationPoints(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION * newRow, ArticulationNodeInformation *nodeInfo, reduced_member_id reducedMember){
...
static void rigidConnectedColoringRecursive(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION * newRow, spqr_node articulationNode,
...
static void rigidConnectedColoring(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                   const reduced_member_id reducedMember, const spqr_node node, SCIP_Bool * const isGood){
...
static spqr_node checkNeighbourColoringArticulationNode(SCIP_NETWORKDECOMP * dec, SCIP_NETWORKROWADDITION *newRow,
...
static void rigidGetSplittableArticulationPointsOnPath(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                                       const reduced_member_id toCheck,
...
static void determineParallelType(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
...
static void determineSeriesType(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                const reduced_member_id toCheckMember, const spqr_arc markerToOther,
                                const reduced_member_id otherMember, const spqr_arc markerToCheck){
...
static void determineRigidType(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                               const reduced_member_id toCheckMember, const spqr_arc markerToOther,
                               const reduced_member_id otherMember, const spqr_arc markerToCheck){
...
static RowReducedMemberType determineType(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                          const reduced_member_id toCheckMember, const spqr_arc markerToOther,
                                          const reduced_member_id otherMember, const spqr_arc markerToCheck){
...
static void propagateComponents(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow){
    int leafArrayIndex = 0;
...
static Nodes
rigidDetermineCandidateNodesFromAdjacentComponents(SCIP_NETWORKDECOMP *dec,
                                                   SCIP_NETWORKROWADDITION *newRow, const reduced_member_id toCheck) {
...
static SCIP_RETCODE allocateTreeSearchMemory(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow){
...
static void determineSingleRowRigidType(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
...
static void determineSingleParallelType(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                        reduced_member_id reducedMember, spqr_member member){
...
static void determineSingleSeriesType(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                      reduced_member_id reducedMember, spqr_member member){
    assert(newRow->reducedMembers[reducedMember].numCutArcs == 1);
...
static spqr_node determineAndColorSplitNode(SCIP_NETWORKDECOMP * dec, SCIP_NETWORKROWADDITION * newRow,
...
static void determineSplitTypeFirstLeaf(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
...
static SplitOrientation getRelativeOrientationRigid(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
...
static SplitOrientation getRelativeOrientationParallel(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                                       reduced_member_id reducedId, spqr_arc arcToNext){
    assert(findArcMemberNoCompression(dec,arcToNext) == newRow->reducedMembers[reducedId].member);
...
static SplitOrientation getRelativeOrientationSeries(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                                     reduced_member_id reducedId, spqr_arc arcToNext){
    assert(findArcMemberNoCompression(dec,arcToNext) == newRow->reducedMembers[reducedId].member);
    assert(SPQRarcIsValid(newRow->reducedMembers[reducedId].splitArc) && SPQRarcIsValid(arcToNext));
    SplitOrientation orientation;
...
static SplitOrientation getRelativeOrientation(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                               reduced_member_id reducedId, spqr_arc arcToNext){
...
static void determineSplitTypeSeries(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
...
static void determineSplitTypeParallel(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                       reduced_member_id reducedId, spqr_member member, spqr_arc marker,
                                       SplitOrientation previousOrientation){
    SPQRRowReducedMember * redMember = &newRow->reducedMembers[reducedId];
...
static void determineSplitTypeRigid(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                    reduced_member_id reducedId, spqr_member member, spqr_arc marker,
                                    SplitOrientation previousOrientation){
    assert(dec);
    assert(newRow);
...
static void determineSplitTypeNext(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                   reduced_member_id current, reduced_member_id next, SCIP_Bool currentIsParent){
...
static void determineTypesChildrenNodes(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
...
static void determineMergeableTypes(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow, reduced_member_id root){
...
static void cleanUpRowMemberInformation(SCIP_NETWORKROWADDITION * newRow){
    //This loop is at the end as memberInformation is also used to assign the cut arcs during propagation
    //Clean up the memberInformation array
    for (int i = 0; i < newRow->numReducedMembers; ++i) {
...
static SCIP_RETCODE rigidTransformArcIntoCycle(SCIP_NETWORKDECOMP *dec,
...
static SCIP_RETCODE transformSingleRigid(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
...
static SCIP_RETCODE splitParallelRowAddition(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                           const reduced_member_id reducedMember,
                                           const spqr_member member,
    ...
    if(createCutParallel) {
        if(convertOriginalParallel){
            ...
            SCIP_Bool firstReversed =  newRow->cutArcs[firstCut].arcReversed != arcIsReversedNonRigid(dec,newRow->cutArcs[firstCut].arc);
...
static SCIP_RETCODE transformSingleParallel(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                          const reduced_member_id reducedMember,
                                          const spqr_member member,
...
static SCIP_RETCODE splitSeriesMergingRowAddition(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                                const reduced_member_id reducedMember,
                                                const spqr_member member,
                                                spqr_member * const mergingMember, SCIP_Bool * const isCut,
    ...
    * isCut = newRow->reducedMembers[reducedMember].numCutArcs > 0;
    ...
    if(parentToMergingMember){
        SCIP_CALL(createMarkerPairWithReferences(dec,mergingSeries,member,coTreeToMergingMember,TRUE,FALSE,representativeEdge,&ignoreArc));
    }else{
        SCIP_CALL(createMarkerPairWithReferences(dec,member,mergingSeries,!coTreeToMergingMember,FALSE,TRUE,&ignoreArc,representativeEdge));
    }
...
static SCIP_RETCODE splitParallelMerging(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
    ...
    if(createCutParallel && keepOriginalParallel){
        ...
        spqr_arc ignoreArc = SPQR_INVALID_ARC;
        if(parentCut){
            SCIP_CALL(createMarkerPairWithReferences(dec,cutMember,member,TRUE,FALSE,FALSE,&ignoreArc,cutRepresentative));
        }else{
            SCIP_CALL(createMarkerPairWithReferences(dec,member,cutMember,FALSE,FALSE,FALSE,cutRepresentative,&ignoreArc));
        }
        ...
        *pMergeMember = member;
        ...
        if(parentToMergingMember || parentCut){
            ...
            SCIP_CALL(createMarkerPairWithReferences(dec,member,mergingMember,treeToMergingMember,FALSE,FALSE,&noCutRepresentative,&ignoreArgument));
        }
...
static SCIP_RETCODE splitFirstLeaf(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
...
static SCIP_RETCODE mergeSplitMemberIntoParent(SCIP_NETWORKDECOMP *dec,
                                             SCIP_NETWORKROWADDITION * newRow,
                                             spqr_member member,
                                             spqr_member parent,
                                             spqr_arc parentToChild,
                                             spqr_arc childToParent,
...
static SCIP_RETCODE splitAndMergeSeries(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                      reduced_member_id largeMember, reduced_member_id smallMember,
                                      SCIP_Bool largeIsParent, NewRowInformation * const newRowInformation,
                                      spqr_member member){
...
static SCIP_RETCODE splitAndMergeParallel(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                        reduced_member_id largeMember, reduced_member_id smallMember,
                                        SCIP_Bool largeIsParent, NewRowInformation * const newRowInformation,
                                        spqr_member member){
    spqr_member mergeMember = SPQR_INVALID_MEMBER;
...
static SCIP_RETCODE splitAndMergeRigid(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                     reduced_member_id largeMember, reduced_member_id smallMember,
                                     SCIP_Bool largeIsParent, NewRowInformation * const newRowInformation,
                                     spqr_member member){
    ...
    spqr_node mergeNodeThree;
    if(largeIsParent){
        SCIP_CALL(mergeSplitMemberIntoParent(dec,newRow,smallMemberMember,largeMemberMember,largeMarker,smallMarker,TRUE,
                                             largeOtherNode,smallOtherNode,&mergedMember,
                                             &arcNodeOne,
                                             &arcNodeTwo,
                                             &mergeNodeThree));
    }else{
        SCIP_CALL(mergeSplitMemberIntoParent(dec,newRow,largeMemberMember,smallMemberMember,smallMarker,largeMarker,TRUE,
    ...
    SCIP_Bool adjacentToMarkerHead = (newRowInformation->tail == largeMarkerHead || newRowInformation->head == largeMarkerHead);
...
static SCIP_RETCODE splitAndMerge(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                reduced_member_id largeMember, reduced_member_id smallMember,
                                SCIP_Bool largeIsParent, NewRowInformation * const newRowInformation){
    spqr_member member = newRow->reducedMembers[smallMember].member;
    switch(getMemberType(dec,member)){
        case SPQR_MEMBERTYPE_RIGID:{
            SCIP_CALL(splitAndMergeRigid(dec,newRow,largeMember,smallMember,largeIsParent,newRowInformation,member));
            break;
        }
        case SPQR_MEMBERTYPE_PARALLEL:{
            SCIP_CALL(splitAndMergeParallel(dec,newRow,largeMember,smallMember,largeIsParent,newRowInformation,member));
            break;
        }
        case SPQR_MEMBERTYPE_SERIES:{
            SCIP_CALL(splitAndMergeSeries(dec,newRow,largeMember,smallMember,largeIsParent,newRowInformation,member));
            break;
        }
        default:
            assert(FALSE);
...
static SCIP_RETCODE mergeChildrenNodes(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
                                     reduced_member_id parent, reduced_member_id node,
...
static SCIP_RETCODE mergeTree(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow, reduced_member_id root,
                            NewRowInformation * const newRowInformation){
...
static SCIP_RETCODE transformComponentRowAddition(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow,
...
SCIP_RETCODE SCIPNetworkRowAdditionCreate(SCIP* env, SCIP_NETWORKROWADDITION** pNewRow ){
    assert(env);
    SCIP_CALL(SCIPallocBlockMemory(env,pNewRow));
    SCIP_NETWORKROWADDITION * newRow = *pNewRow;
...
void SCIPNetworkRowAdditionFree(SCIP* env, SCIP_NETWORKROWADDITION ** pNewRow){
    assert(*pNewRow);

    SCIP_NETWORKROWADDITION * newRow = *pNewRow;

    SCIPfreeBlockMemoryArray(env,&newRow->createReducedMembersCallstack,newRow->memCreateReducedMembersCallstack);
    SCIPfreeBlockMemoryArray(env,&newRow->artDFSData,newRow->memArtDFSData);
    SCIPfreeBlockMemoryArray(env,&newRow->colorDFSData,newRow->memColorDFSData);
    SCIPfreeBlockMemoryArray(env,&newRow->mergeTreeCallData,newRow->memMergeTreeCallData);
    SCIPfreeBlockMemoryArray(env,&newRow->intersectionDFSData,newRow->memIntersectionDFSData);
    SCIPfreeBlockMemoryArray(env,&newRow->intersectionPathParent,newRow->memIntersectionPathParent);
    SCIPfreeBlockMemoryArray(env,&newRow->intersectionPathDepth,newRow->memIntersectionPathDepth);
    SCIPfreeBlockMemoryArray(env,&newRow->crossingPathCount,newRow->memCrossingPathCount);
    SCIPfreeBlockMemoryArray(env,&newRow->articulationNodeSearchInfo,newRow->memNodeSearchInfo);
    SCIPfreeBlockMemoryArray(env,&newRow->articulationNodes,newRow->memArticulationNodes);
    SCIPfreeBlockMemoryArray(env,&newRow->nodeColors,newRow->memNodeColors);
    SCIPfreeBlockMemoryArray(env,&newRow->isArcCut,newRow->memIsArcCut);
    SCIPfreeBlockMemoryArray(env,&newRow->isArcCutReversed,newRow->memIsArcCut);
    SCIPfreeBlockMemoryArray(env,&newRow->decompositionColumnArcs,newRow->memDecompositionColumnArcs);
    SCIPfreeBlockMemoryArray(env,&newRow->decompositionColumnArcReversed,newRow->memDecompositionColumnArcs);
    SCIPfreeBlockMemoryArray(env,&newRow->newColumnArcs,newRow->memColumnArcs);
    SCIPfreeBlockMemoryArray(env,&newRow->newColumnReversed,newRow->memColumnArcs);
    SCIPfreeBlockMemoryArray(env,&newRow->childrenStorage,newRow->memChildrenStorage);
    SCIPfreeBlockMemoryArray(env,&newRow->cutArcs,newRow->memCutArcs);
    SCIPfreeBlockMemoryArray(env,&newRow->memberInformation,newRow->memMemberInformation);
    SCIPfreeBlockMemoryArray(env,&newRow->reducedComponents,newRow->memReducedComponents);
    SCIPfreeBlockMemoryArray(env,&newRow->reducedMembers,newRow->memReducedMembers);
    SCIPfreeBlockMemoryArray(env,&newRow->leafMembers, newRow->memLeafMembers);
    SCIPfreeBlockMemory(env,pNewRow);
}

SCIP_RETCODE SCIPNetworkRowAdditionCheck(SCIP_NETWORKDECOMP * dec, SCIP_NETWORKROWADDITION * newRow,
                                       const spqr_row row, const spqr_col * columns, const double * columnValues,
                                       size_t numColumns){
    assert(dec);
    assert(newRow);
    ...
    SCIP_CALL(newRowUpdateRowInformation(dec,newRow,row,columns,columnValues,numColumns));
    SCIP_CALL(constructRowReducedDecomposition(dec,newRow));
    SCIP_CALL(createReducedDecompositionCutArcs(dec,newRow));

    SCIP_CALL(determineLeafReducedMembers(dec,newRow));
    SCIP_CALL(allocateRigidSearchMemory(dec,newRow));
    SCIP_CALL(allocateTreeSearchMemory(dec,newRow));
...
SCIP_RETCODE SCIPNetworkRowAdditionAdd(SCIP_NETWORKDECOMP *dec, SCIP_NETWORKROWADDITION *newRow){
    ...
    if(newRow->numReducedComponents == 0){
        ...
        SCIP_CALL(createStandaloneParallel(dec,newRow->newColumnArcs, newRow->newColumnReversed,
        ...
        SCIP_CALL(transformComponentRowAddition(dec,newRow,&newRow->reducedComponents[0],&information));

        if(newRow->numColumnArcs == 0){
            spqr_arc rowArc = SPQR_INVALID_ARC;
            SCIP_CALL(createRowArc(dec,information.member,&rowArc,newRow->newRowIndex,information.reversed));
            if(SPQRnodeIsValid(information.head)){
                assert(SPQRnodeIsValid(information.tail));
                assert(SPQRarcIsValid(information.representative));
            ...
            SCIP_CALL(createConnectedParallel(dec,newRow->newColumnArcs,newRow->newColumnReversed,newRow->numColumnArcs,
        ...
        for (int i = 0; i < newRow->numReducedComponents; ++i) {
            ...
            SCIP_CALL(transformComponentRowAddition(dec,newRow,&newRow->reducedComponents[i],&information));
            if(getMemberType(dec,information.member) == SPQR_MEMBERTYPE_LOOP){
                assert(getNumMemberArcs(dec,information.member) == 1);
                spqr_arc arc = getFirstMemberArc(dec,information.member);
