...
{  /*lint --e{715}*/
   ...
   if( SCIPisLT(scip, SCIPgetSolOrigObj(scip, bestsol)*SCIPgetObjsense(scip), eventhdlrdata->upperbound) )
   {
      SCIP_CALL( SCIPinterruptSolve(scip) );
   }
   ...
   )
{
    ...
    */
   if( checkint && SCIPsetIsFeasLE(set, SCIPgetPrimalbound(set->scip)*SCIPgetObjsense(set->scip),
         SCIPgetSolOrigObj(set->scip, sol)*SCIPgetObjsense(set->scip)) )
   {
      (*result) = SCIP_DIDNOTRUN;
      return SCIP_OKAY;
   }
   ...
   if( !enhancement )
   {
      ...
      if( solveloop == SCIP_BENDERSSOLVELOOP_CONVEX )
      {
         if( SCIPgetLPSolstat(subproblem) == SCIP_LPSOLSTAT_OPTIMAL )
            SCIPbendersSetSubproblemObjval(benders, probnumber,
               SCIPgetSolOrigObj(subproblem, NULL)*SCIPgetObjsense(subproblem));
      ...
      else if( solveloop == SCIP_BENDERSSOLVELOOP_CIP )
      {
         ...
         if( SCIPgetStatus(subproblem) == SCIP_STATUS_OPTIMAL )
            SCIPbendersSetSubproblemObjval(benders, probnumber,
               SCIPgetSolOrigObj(subproblem, bestsol)*SCIPgetObjsense(subproblem));
   ...
   )
{
   ...
   else
   {
      ...
      if( solvecip && !SCIPbendersSubproblemIsConvex(benders, probnumber) )
      {
         ...
         if( objective != NULL )
            (*objective) = SCIPgetSolOrigObj(subproblem, SCIPgetBestSol(subproblem))*SCIPgetObjsense(subproblem);
      }
      else
      {
         SCIP_Bool success;
         ...
         if( success )
         {
            ...
            if( objective != NULL )
               (*objective) = SCIPgetSolOrigObj(subproblem, NULL)*SCIPgetObjsense(subproblem);
         }
         else
         {
            if( objective != NULL )
