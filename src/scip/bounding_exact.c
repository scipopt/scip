   ...
   )
{
   ...
   if( SCIPrationalIsPositive(alpha) )
   {
      ...
      SCIPrationalSetReal(beta, pow(2, (int) (log(SCIPrationalApproxReal(beta))/log(2))));
   }
   ...
   )
{
   ...
   SCIPrationalPrintf("   objective value=%f (%q) \n", SCIPrationalApproxReal(dualbound), dualbound);
#endif
   ...
   if( !isfeas )
   {
      ...
      for( i = 0; i < nrows+ncols; i++ )
      {
         SCIPrationalPrintf("   i=%d: %f.20 (%q) \n", i, SCIPrationalApproxReal(dualsol[i]), dualsol[i]);
      }
#endif
   }
...
#ifdef PS_OUT
   printf("   common slack=%.20f (", SCIPrationalApproxReal(projshiftdata->commonslack));
   SCIPrationalPrint(projshiftdata->commonslack);
   printf(")\n");

   printf("   max violation=%.20f (", SCIPrationalApproxReal(maxv));
   SCIPrationalPrint(maxv);
   printf(")\n");

   printf("   lambda (use of interior point)=%.20f (", SCIPrationalApproxReal(lambda2));
   SCIPrationalPrint(lambda2);
   printf(")\n");

   printf("   dual objective value=%.20f (", SCIPrationalApproxReal(dualbound));
