...
{  /*lint --e{715}*/
   ...
   *success = TRUE;
   ...
   if( overestimate )
   {
      ...
      addLogLinearization(scip, refpoint[0], SCIPexprGetIntegrality(SCIPexprGetChildren(expr)[0]) != SCIP_EXPRINT_NONE,
   ...
   *nreturned = 0;
   ...
   for( i = 0; i < 4; ++i )
   {
      ...
      if( overest[i] )
      {
         assert(i < 3);
         /* coverity[overrun] */
         addLogLinearization(scip, refpointsover[i], SCIPexprGetIntegrality(child) != SCIP_EXPRINT_NONE,
