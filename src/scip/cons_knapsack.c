   ...
   )
{
   ...
   *redundant = FALSE;
   ...
   do
   {
      ...
      {
         SCIPdebugMsg(scip, " -> cutoff - fixed weight: %" SCIP_LONGINT_FORMAT ", capacity: %" SCIP_LONGINT_FORMAT ", minimum weight sum: %" SCIP_LONGINT_FORMAT " \n",
...
SCIP_DECL_HASHKEYVAL(hashKeyValKnapsackcons)
{
   ...
   return SCIPhashFive(consdata->nvars, minidx, mididx, maxidx, consdata->weights[0]);
}
