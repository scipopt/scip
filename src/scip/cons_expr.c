   ...
   )
{
   ...
   if( SCIPexprGetHdlr(consdata->expr) == SCIPgetExprHdlrSum(conshdlr) )
   {
      ...
      if( counter < 0 || (counter == 0 && SCIPisInfinity(scip, consdata->rhs)) )
      {
         ...
         SCIP_CALL( SCIPcreateExprSum(scip, conshdlr, &expr, nchildren, SCIPexprGetChildren(consdata->expr), newcoefs, -constant) );
   ...
   )
{
   ...
   *newexpr = NULL;
   ...
   if( nexprs > 0 )
   {
      ...
      /* create a new sum expression */
      SCIP_CALL( SCIPcreateExprSum(scip, conshdlr, newexpr, nexprs, exprs, exprcoefs, SCIPexprsumGetConstant(sumexpr)) );
   ...
   )
{
   ...
   else
   {
      ...
      SCIP_CALL( SCIPcreateExprSum(scip, conshdlr, &consdata->expr, 2, children, coefs, 0.0) );
