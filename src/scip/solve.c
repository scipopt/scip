...
/*                                                                           */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
#pragma ident "@(#) $Id: solve.c,v 1.294 2010/05/11 19:25:37 bzfberth Exp $"
   ...
   )
{
   ...
   *cutoff = FALSE;
   ...
   SCIP_CALL( SCIPsepastoreApplyCuts(sepastore, blkmem, set, stat, tree, lp, branchcand, eventqueue, root, cutoff) );
   ...
   )
{
   ...
   *lperror = FALSE;
   ...
   SCIPbranchcandGetLPCands(branchcand, set, stat, lp, NULL, NULL, NULL, &nlpcands, NULL);

   /* call LP separators with nonnegative priority */
   for( i = 0; i < set->nsepas && !(*cutoff) && !(*lperror) && !(*enoughcuts) && lp->flushed && lp->solved && nlpcands > 0
   ...
   )
{
   ...
   *cutoff = FALSE;
   ...
   if( sol == NULL )
   {
      ...
      mustsepa = FALSE;
      mustprice = FALSE;
      SCIP_CALL( separationRoundLP(blkmem, set, stat, prob, lp, branchcand, sepastore, actdepth, 0.0, onlydelayed, delayed, &enoughcuts,
   ...
   while( !(*cutoff) && !(*lperror) && (mustprice || mustsepa || delayedsepa) )
   {
      ...
      while( mustprice && !(*lperror) )
      {
         ...
         SCIP_CALL( SCIPpriceLoop(blkmem, set, stat, prob, primal, tree, lp, pricestore, branchcand, eventqueue,
               root, root, -1, &npricedcolvars, &mustsepa, &pricerlowerbound, lperror, pricingaborted) );
      ...
      if( !(*cutoff) && !(*lperror) && mustsepa )
      {
            ...
            && (SCIPlpGetSolstat(lp) == SCIP_LPSOLSTAT_OPTIMAL || SCIPlpGetSolstat(lp) == SCIP_LPSOLSTAT_UNBOUNDEDRAY) )
         {
            ...
            SCIP_CALL( separationRoundLP(blkmem, set, stat, prob, lp, branchcand, sepastore, actdepth, bounddist, delayedsepa,
         ...
         {
            /* apply found cuts */
            SCIP_CALL( SCIPsepastoreApplyCuts(sepastore, blkmem, set, stat, tree, lp, branchcand, eventqueue, root,
                  cutoff) );
