...
#define HEUR_NAME             "mutation-benders"
...
static
SCIP_DECL_HEURCOPY(heurCopyMutationbenders)
{  /*lint --e{715}*/
   assert(scip != NULL);
   assert(heur != NULL);
...
static
SCIP_DECL_HEURFREE(heurFreeMutationbenders)
{  /*lint --e{715}*/
...
static
SCIP_DECL_HEURINIT(heurInitMutationbenders)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEUREXIT(heurExitMutationbenders)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEUREXEC(heurExecMutationbenders)
{  /*lint --e{715}*/
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   SCIP_CALL( SCIPcopyLargeNeighborhoodSearch(scip, subscip, varmapfw, "mutationbenders", fixedvars, fixedvals, nfixedvars,
   ...
   SCIPdebugMsg(scip, "Solve Mutationbenders subMIP\n");
   ...
   {
      SCIPwarningMessage(scip, "Error while solving subproblem in Mutationbenders heuristic; sub-SCIP terminated with code <%d>\n",retcode);
      SCIPABORT();
   }
   else
   {
   ...
   )
{
   ...
   SCIP_CALL( SCIPincludeHeurBasic(scip, &heur,
         ...
         HEUR_MAXDEPTH, HEUR_TIMING, HEUR_USESSUBSCIP, heurExecMutationbenders, heurdata) );

   assert(heur != NULL);

   /* set non-NULL pointers to callback methods */
   SCIP_CALL( SCIPsetHeurCopy(scip, heur, heurCopyMutationbenders) );
   SCIP_CALL( SCIPsetHeurFree(scip, heur, heurFreeMutationbenders) );
   SCIP_CALL( SCIPsetHeurInit(scip, heur, heurInitMutationbenders) );
   SCIP_CALL( SCIPsetHeurExit(scip, heur, heurExitMutationbenders) );
