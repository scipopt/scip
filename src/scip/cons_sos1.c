...
/*                                                                           */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
#pragma ident "@(#) $Id: cons_sos1.c,v 1.26 2009/09/08 20:41:29 bzfberth Exp $"
   ...
   )
{
   ...
   SCIP_VAR** Vars;
   int nVars;
   SCIP_Real maxWeight = -SCIP_REAL_MAX;
   int c;
   ...
   *result = SCIP_FEASIBLE;
   ...
   for (c = 0; c < nconss; ++c)
   {
	 ...
	 continue;
      ...
      SCIPdebugMessage("propagation in enforcing <%s> (cutoff: %u, domain reductions: %d).\n", SCIPconsGetName(cons), cutoff, nGen);
      if ( cutoff )
      {
	 *result = SCIP_CUTOFF;
	 return SCIP_OKAY;
...
{  /*lint --e{715}*/
   ...
   SCIPdebugMessage("Checking SOS1 constraints <%s>.\n", SCIPconshdlrGetName(conshdlr) );
