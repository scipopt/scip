 ...
 * \f]
 * Now suppose that the no gradient cut at \f$ z \f$ separates \f$ x_0 \f$, ie
 * \f[
 *      g_j(z) + \langle \nabla g_j(z), x_0 - z \rangle \le 0
 * \f]
 * Multiplying each inequality with \f$ \lambda_j \ge \f$ and summing up, we get the following contradiction:
 * \f[
...
#define NLPITERLIM                  250 /**< default NLP iteration limit */
   ...
   )
{
   ...
   /* set parameters in nlpi; iterations limit, tolerance, verbosity */
   SCIP_CALL( SCIPnlpiSetRealPar(sepadata->nlpi, sepadata->nlpiprob, SCIP_NLPPAR_TILIM, NLPITERLIM) );
