...
/*                                                                           */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
#pragma ident "@(#) $Id: sepa_flowcover.c,v 1.19 2008/09/29 20:41:26 bzfheinz Exp $"
   ...
   )
{
   ...
   *nnonflowcovervars = 0;
    ...
    *     
    *    (KP^SNF_rat)  max sum_{j in N1} ( 1 - x*_j ) z起j + sum_{j in N2} x*_j z_j
    *                      sum_{j in N1}          u_j z起j + sum_{j in N2} u_j  z_j < - b + sum_{j in N1} u_j  
    *                                                 z起j in {0,1} for all j in N1 
    ...
    *
    *    (KP^SNF_int)  max sum_{j in N1} ( 1 - x*_j ) z起j + sum_{j in N2} x*_j z_j
    *                      sum_{j in N1}        C u_j z起j + sum_{j in N2} C u_j  z_j <= c 
    *                                                   z起j in {0,1} for all j in N1 
    *                                                    z_j in {0,1} for all j in N2, 
    *    where 
