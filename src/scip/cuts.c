   ...
   )
{
   ...
   /* add up coefficients */
   SCIP_CALL( varVecAddScaledRowCoefsQuadSafely(aggrrow->inds, aggrrow->vals, &aggrrow->nnz, row, weight) );
...
{  /*lint --e{715}*/
   ...
   for( i = 0; i < nrowinds; i++ )
   {
      ...
      {
         SCIP_CALL( varVecAddScaledRowCoefsQuadSafely(cutinds, cutcoefs, nnz, row, mul) );
