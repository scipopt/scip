...
{
   SCIP_Real QUAD(val);
   SCIPquadprecSumDD(val, newcoeff, -oldcoeff);

   if( QUAD_ROUND(val) > QUAD_EPSILON )
   {
   ...
   else if( QUAD_ROUND(val) < -QUAD_EPSILON )
   {
   ...
   )
{
   ...
   SCIPquadprecSumQD(val, -oldcoeff, newcoeff);
   ...
   *redundant = FALSE;
   ...
   for( i = 0; i < *cutnnz; ++i )
   {
      ...
      if( QUAD_ROUND(val) < 0.0 )
      {
         ...
         {
            maxabscontval = MAX(maxabscontval, -QUAD_ROUND(val));
            isintegral = FALSE;
         }
   ...
   if( isintegral )
   {
      ...
      equiscale = 1.0 / MIN((maxact - QUAD_ROUND(*cutrhs)), maxabsintval);
      ...
      for( i = 0; i < *cutnnz; ++i )
      {
         ...
         scaleval = QUAD_ROUND(val) * equiscale;
      ...
      if( success )
      {
         ...
         for( i = 0; i < *cutnnz; ++i )
         {
            ...
            intval = SCIPround(scip, QUAD_ROUND(val));
   ...
   else
   {
      ...
      maxabsval = maxact - QUAD_ROUND(*cutrhs);
   ...
   for( i = 0; i < *cutnnz; )
   {
      ...
      if( QUAD_ROUND(val) < 0.0 && SCIPisLE(scip, maxact + QUAD_ROUND(val), QUAD_ROUND(*cutrhs)) )
      {
         ...
         if( isintegral )
         {
            assert(SCIPisIntegral(scip, QUAD_ROUND(coef)));
            QUAD_ASSIGN(coef, SCIPround(scip, QUAD_ROUND(coef)));
         }
   ...
   )
{
   ...
   for( i = *nnz - 1; i >= 0 && cutinds[i] < firstcontvar; --i )
   {
      ...
      /* calculate the coefficient in the retransformed cut */
      {
         ...
         assert(QUAD_ROUND(fj) >= -SCIPepsilon(scip) && QUAD_ROUND(fj) < 1.0);
   ...
   )
{
   ...
   *cutislocal = aggrrow->local;
   ...
   for( i = 0; i < intstart; ++i )
   {
      ...
      contsqrnorm += QUAD_ROUND(mksetcoef) * QUAD_ROUND(mksetcoef);
   }
   ...
   for( i = 0; i < maxtestdelta; ++i )
   {
      ...
      viol = computeMIREfficacy(scip, tmpcoefs, tmpvalues, QUAD_ROUND(mksetrhs), contactivity, contsqrnorm, deltacands[i], ntmpcoefs, minfrac, maxfrac);
   ...
   for( i = 2; i <= 8 ; i *= 2 )
   {
      ...
      viol = computeMIREfficacy(scip, tmpcoefs, tmpvalues, QUAD_ROUND(mksetrhs), contactivity, contsqrnorm, delta, ntmpcoefs, minfrac, maxfrac);
   ...
   for( i = 0; i < nbounddist; ++i )
   {
      ...
      newviol = computeMIREfficacy(scip, tmpcoefs, tmpvalues, QUAD_ROUND(newrhs), contactivity, contsqrnorm, bestdelta, ntmpcoefs, minfrac, maxfrac);
