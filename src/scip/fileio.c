...
{
#ifndef NDEBUG
   int nbytesread = gzread((gzFile)stream, ptr, (unsigned) size * nmemb);
   assert(nbytesread >= 0);

   return (size_t) nbytesread;
#else
   return (size_t) gzread((gzFile)stream, ptr, (unsigned) size * nmemb);
#endif
}

size_t SCIPfwrite(const void *ptr, size_t size, size_t nmemb, SCIP_FILE *stream)
{
   return (size_t) gzwrite((gzFile)stream, ptr, (unsigned) size * nmemb);
}
