...
/*                                                                           */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
#pragma ident "@(#) $Id: sepa_strongcg.c,v 1.13 2005/08/22 18:35:48 bzfpfend Exp $"
...
{  /*lint --e{715}*/
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   SCIPdebugMessage("searching strong CG cuts: %d cols, %d rows, maxdnom=%lld, maxscale=%g, maxcuts=%d\n",
   ...
   for( i = 0; i < nrows && ncuts < maxsepacuts; ++i )
   {
      ...
      if( c >= 0 )
      {
         ...
         if( SCIPvarGetType(var) != SCIP_VARTYPE_CONTINUOUS )
         {
            ...
            {
               SCIPdebugMessage("trying strong CG cut for <%s> [%g]\n", SCIPvarGetName(var), primsol);

               /* get the row of B^-1 for this basic integer variable with fractional solution value */
               SCIP_CALL( SCIPgetLPBInvRow(scip, i, binvrow) );

               /* create a strong CG cut out of the weighted SCIP_LP rows using the B^-1 row as weights */
