...
#define DEFAULT_MINIMPROVE    0.01      /* factor by which DINSBENDERS should at least improve the incumbent          */
...
#define EVENTHDLR_NAME         "Dinsbenders"
...
/** DINSBENDERS primal heuristic data */
struct SCIP_HeurData
{
   ...
   SCIP_Real             bestsolobj;         /**< the objective function value of the best found solution */
   SCIP_Real             prevdualbound;      /**< the previous dual bound found at the last LP solve */
   int                   stalllpcount;       /**< the number of LPs since the last improvement to the best solution */
   ...
   )
{
   ...
   (void) SCIPsnprintf(consname, SCIP_MAXSTRLEN, "%s_dinsbendersLBcons", SCIPgetProbName(scip));
   ...
   )
{
   ...
   if( *success )
   {
      SCIPdebugMsg(scip, "DINSBENDERS successfully found new solution\n");
   }
 ...
 */
static
SCIP_DECL_EVENTEXEC(eventExecDinsbenders)
{
...
static
SCIP_DECL_HEURCOPY(heurCopyDinsbenders)
{  /*lint --e{715}*/
   assert(scip != NULL);
   assert(heur != NULL);
...
static
SCIP_DECL_HEURFREE(heurFreeDinsbenders)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEURINITSOL(heurInitsolDinsbenders)
{
   SCIP_HEURDATA* heurdata;
   int i;
...
static
SCIP_DECL_HEUREXITSOL(heurExitsolDinsbenders)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEUREXEC(heurExecDinsbenders)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   SCIP_CALL( SCIPcopyLargeNeighborhoodSearch(scip, subscip, varmapfw, "dinsbenders", fixedvars, fixedvals, binfixings + intfixings,
         heurdata->uselprows, heurdata->copycuts, &success, NULL) );

   /* create event handler for LP events */
   SCIP_CALL( SCIPincludeEventhdlrBasic(subscip, &eventhdlr, EVENTHDLR_NAME, EVENTHDLR_DESC, eventExecDinsbenders, NULL) );
   if( eventhdlr == NULL )
   {
   ...
   SCIPdebugMsg(scip, "DINSBENDERS subproblem: %d vars (%d binvars & %d intvars), %d cons\n",
   ...
   if( !heurdata->uselprows )
   {
      ...
      /* setting the data for the stall lp limit */
      heurdata->bestsolobj = SCIPinfinity(scip);
      heurdata->prevdualbound = -SCIPinfinity(scip);
      heurdata->stalllpcount = 0;
   }

   /* solve the subproblem */
   SCIPdebugMsg(scip, "solving DINSBENDERS sub-MIP with neighborhoodsize %d and maxnodes %" SCIP_LONGINT_FORMAT "\n", heurdata->neighborhoodsize, nsubnodes);
   ...
   SCIPdebugMsg(scip, "DINSBENDERS used %" SCIP_LONGINT_FORMAT "/%" SCIP_LONGINT_FORMAT " nodes and found %d solutions\n", SCIPgetNNodes(subscip), nsubnodes, nsubsols);
   ...
   )
{
   ...
   SCIP_CALL( SCIPincludeHeurBasic(scip, &heur,
         ...
         HEUR_MAXDEPTH, HEUR_TIMING, HEUR_USESSUBSCIP, heurExecDinsbenders, heurdata) );

   assert(heur != NULL);

   /* set non-NULL pointers to callback methods */
   SCIP_CALL( SCIPsetHeurCopy(scip, heur, heurCopyDinsbenders) );
   SCIP_CALL( SCIPsetHeurFree(scip, heur, heurFreeDinsbenders) );
   SCIP_CALL( SCIPsetHeurInitsol(scip, heur, heurInitsolDinsbenders) );
   SCIP_CALL( SCIPsetHeurExitsol(scip, heur, heurExitsolDinsbenders) );
