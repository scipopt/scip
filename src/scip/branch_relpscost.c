...
#define DEFAULT_GMIEFFWEIGHT     0.05        /**< weight in score calculations of GMI cut efficacies */
...
struct SCIP_BranchruleData
{
   ...
   SCIP_Real             gmieffweight;       /**< weight in score calculations of GMI cut efficacies */
   ...
   else
   {
      ...
      if( branchruledata->usehyptestforreliability )
      {
         for( c = 0; c < nbranchcands; ++c )
         {
            ...
            gmieffscore = SCIPgetVarLastGMIScore(scip, branchcands[c]);
              ...
              && (i < (int) maxlookahead || SCIPgetNStrongbranchLPIterations(scip) < maxnsblpiterations); ++i )
      {
         ...
         else
         {
            ...
            gmieffscore = branchruledata->usesblocalinfo ? 0.0 : SCIPgetVarLastGMIScore(scip, branchcands[c]);
            pscostscore = SCIPgetBranchScore(scip, branchcands[c], downgain, upgain);
   ...
   )
{
   ...
   SCIP_CALL( SCIPaddRealParam(scip,
          "branching/relpscost/gmieffweight",
          "weight in score calculations for last GMI cuts normalised efficacy",
          &branchruledata->gmieffweight, TRUE, DEFAULT_GMIEFFWEIGHT, SCIP_REAL_MIN, SCIP_REAL_MAX, NULL, NULL) );
   SCIP_CALL( SCIPaddRealParam(scip,
