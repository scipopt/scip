   ...
   )
{
  int izero = 0;
   ...
   CHECK_ZERO( XPRSsetintcontrol((*lpi)->xprslp, XPRS_SOLUTIONFILE, 0) );

   /* We need to create an empty LP in this prob since SCIP might */
   /* attempt to add rows or columns to it. */
   CHECK_ZERO( XPRSloadlp((*lpi)->xprslp, "temp", 0, 0, NULL, NULL, NULL, NULL, &izero, NULL, NULL, NULL, NULL, NULL) );
   ...
   )
{
   ...
   /* free LP */
   CHECK_ZERO( XPRSdestroyprob(((*lpi)->xprslp)) );
   ...
   )
{
   ...
   if (colnames) {
     for (c = 0; c < ncols; c++) {
       ...
       cnamesize += isize+1;
       if (namelength < isize) namelength = isize;
     }
   }
   rnamesize = 16;
   if (rownames) {
   ...
   if (colnames) {
     ...
     CHECK_ZERO( XPRSaddnames(lpi->xprslp, 2, cnamestore, 0, ncols-1) );
   ...
   if (rownames) {
     ...
     CHECK_ZERO( XPRSaddnames(lpi->xprslp, 1, rnamestore, 0, nrows-1) );
   ...
   )
{
  ...
  int imaxnamelength;
   ...
   for (c = 0; c < ncols; c++) mstart[c] = beg[c];
   mstart[ncols] = nnonz;
   ...
   if (colnames) {
     ...
     for (c = 0; c < ncols; c++) {
       ...
       if (isize > imaxnamelength) isize = imaxnamelength;
       cnamesize += isize+1;
     }
     SCIP_ALLOC( BMSallocMemoryArray(&cnamestore, cnamesize) );
     isize = 0;
     for (c = 0; c < ncols; c++) {
       int i;
       for (i = 0; (i < imaxnamelength) && (colnames[c][i]); i++) cnamestore[isize++] = colnames[c][i];
       cnamestore[isize++] = '\0';
     }
     assert(isize == cnamesize);
     CHECK_ZERO( XPRSaddnames(lpi->xprslp, 2, cnamestore, lp_ncols-ncols, lp_ncols-1) );
     BMSfreeMemoryArray(&cnamestore);
   }
   ...
   )
{
   ...
   for (c = firstcol; c <= lastcol; c++) mind[c-firstcol] = c;
   ...
   )
{
   ...
   CHECK_ZERO( XPRSgetintattrib(lpi->xprslp, XPRS_COLS, &ncols) );
   ndel = 0;
   for (c = 0; c < ncols; c++) if (dstat[c]) ndel++;
   ...
   )
{
   ...
   for (r = 0; r < nrows; r++) mstart[r] = beg[r];
   mstart[nrows] = nnonz;
   ...
   if (rownames) {
     ...
     for (r = 0; r < nrows; r++) {
       int i;
       for (i = 0; (i < imaxnamelength) && (rownames[r][i]); i++) rnamestore[isize++] = rownames[r][i];
       rnamestore[isize++] = '\0';
     }
     assert(isize == rnamesize);
     CHECK_ZERO( XPRSaddnames(lpi->xprslp, 1, rnamestore, lp_nrows, lp_nrows+nrows-1) );
     BMSfreeMemoryArray(&rnamestore);
   }
   ...
   )
{
   ...
   for (r = firstrow; r <= lastrow; r++) mind[r-firstrow] = r;
   ...
   )
{
   ...
   CHECK_ZERO( XPRSgetintattrib(lpi->xprslp, XPRS_ROWS, &nrows) );
   ndel = 0;
   for (r = 0; r < nrows; r++) if (dstat[r]) ndel++;
   SCIP_ALLOC( BMSallocMemoryArray(&mind, ndel) );
   r_new = 0;
   ndel = 0;
   for (r = 0; r < nrows; r++) {
   ...
   )
{
   ...
   if( nnonz != NULL )
   {
      ...
      for (c = 0; c < lastcol-firstcol+1; c++) beg[c] = mstart[c];
      BMSfreeMemoryArray(&mstart);
   }
   else
   {
   ...
   )
{
   ...
   if( nnonz != NULL )
   {
      ...
      for (r = 0; r < lastrow-firstrow+1; r++) beg[r] = mstart[r];
      BMSfreeMemoryArray(&mstart);
   }
   else
   {
   ...
   )
{
   ...
   CHECK_ZERO( XPRSgetintattrib(lpi->xprslp, XPRS_DUALINFEAS, &dinfeas) );
   *primalfeasible = (SCIP_Bool) (pinfeas == 0);
   *dualfeasible = (SCIP_Bool) (dinfeas == 0);
   ...
   )
{
    ...
    * but not necessarily a feasible primal solution. If pinfeas is non-zero, we cannot conclude,
    * that the problem is unbounded
    */
   return (!pinfeas && (lpi->solstat == XPRS_LP_UNBOUNDED));
}
...
#ifndef XPRS_INDUAL
#define XPRS_INDUAL 1047
#endif
   CHECK_ZERO( XPRSgetintattrib(lpi->xprslp, XPRS_INDUAL, &indual) );
   ...
   )
{
   ...
   return (!dinfeas && (lpi->solstat == XPRS_LP_UNBOUNDED));
}
   ...
   )
{
   ...
   return (lpi->solstat == XPRS_LP_OPTIMAL);
}
   ...
   )
{
   ...
   if( activity != NULL )
   {
     ...
     for (r = 0; r < nrows; r++) activity[r] = lpi->rhsarray[r] - activity[r];
   }
...
#define XPRS_INDUAL 1047
   ...
   SCIP_CALL( getBase(lpi) );
   if (indual == 1) {
      /* Unboundedness found by dual - nothing we can do about it. */
      return SCIP_ERROR;
   }
   ...
   if (lpi->unbvec >= cfirst) {
      ...
      /* ... and unpack it. */
      assert(nnonz > 0);
      for (i = 0; i < nnonz; i++) bvec[mcolind[i]] = dmult*dcolval[i];
      ray[icol] = dmult;
   ...
   for (irow = 0; irow < nrows; irow++) {
      if (bind[irow] >= cfirst) ray[bind[irow]-cfirst] = bvec[irow];
   }
