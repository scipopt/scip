...
#define HEUR_NAME             "crossover-benders"
...
#define EVENTHDLR_NAME         "Crossoverbenders"
 ...
 */
static
SCIP_DECL_EVENTEXEC(eventExecCrossoverbenders)
{
...
static
SCIP_DECL_HEURCOPY(heurCopyCrossoverbenders)
{  /*lint --e{715}*/
   assert(scip != NULL);
   assert(heur != NULL);
...
static
SCIP_DECL_HEURFREE(heurFreeCrossoverbenders)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEURINIT(heurInitCrossoverbenders)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEUREXIT(heurExitCrossoverbenders)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
   SOLTUPLE* soltuple;
...
static
SCIP_DECL_HEUREXEC(heurExecCrossoverbenders)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;                  /* primal heuristic data                               */
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   SCIP_CALL( SCIPcopyLargeNeighborhoodSearch(scip, subscip, varmapfw, "crossoverbenders", fixedvars, fixedvals, nfixedvars,
         heurdata->uselprows, heurdata->copycuts, &success, NULL) );

   eventhdlr = NULL;
   /* create event handler for LP events */
   SCIP_CALL( SCIPincludeEventhdlrBasic(subscip, &eventhdlr, EVENTHDLR_NAME, EVENTHDLR_DESC, eventExecCrossoverbenders, NULL) );
   if( eventhdlr == NULL )
   {
   ...
   SCIPdebug( SCIP_CALL( SCIPwriteOrigProblem(subscip, "crossoverbendersprob.cip", "cip", FALSE) ) );
#endif
   /* solve the subproblem */
   SCIPdebugMsg(scip, "Solve Crossoverbenders subMIP\n");
   ...
   )
{
   ...
   SCIP_CALL( SCIPincludeHeurBasic(scip, &heur,
         ...
         HEUR_MAXDEPTH, HEUR_TIMING, HEUR_USESSUBSCIP, heurExecCrossoverbenders, heurdata) );

   assert(heur != NULL);

   /* set non-NULL pointers to callback methods */
   SCIP_CALL( SCIPsetHeurCopy(scip, heur, heurCopyCrossoverbenders) );
   SCIP_CALL( SCIPsetHeurFree(scip, heur, heurFreeCrossoverbenders) );
   SCIP_CALL( SCIPsetHeurInit(scip, heur, heurInitCrossoverbenders) );
   SCIP_CALL( SCIPsetHeurExit(scip, heur, heurExitCrossoverbenders) );
