...
#include "scip/heur_proximity-benders.h"
#include "scip/cons_linear.h"

#define HEUR_NAME             "proximity-benders"
...
#define EVENTHDLR_NAME         "Proximitybenders"
   ...
   )
{
   ...
   {
#ifdef NDEBUG
      SCIPwarningMessage(scip, "Error while solving LP in Proximitybenders heuristic; LP solve terminated with code <%d>\n",retstat);
#else
      SCIP_CALL( retstat );
#endif
   }
   ...
   )
{
   ...
   if( heurdata->subscip != NULL )
   {
      ...
      SCIPdebugMsg(scip, "Freeing subproblem of proximitybenders heuristic\n");
 ...
 */
static
SCIP_DECL_EVENTEXEC(eventExecProximitybenders)
{
...
static
SCIP_DECL_HEURCOPY(heurCopyProximitybenders)
{  /*lint --e{715}*/
   assert(scip != NULL);
   assert(heur != NULL);
   ...
   SCIP_CALL( SCIPincludeHeurProximitybenders(scip) );
...
static
SCIP_DECL_HEURFREE(heurFreeProximitybenders)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEURINIT(heurInitProximitybenders)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEUREXITSOL(heurExitsolProximitybenders)
{
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEUREXEC(heurExecProximitybenders)
{  /*lint --e{715}*/
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   {
      SCIPdebugMsg(scip, "skipping proximitybenders: nnodes=%" SCIP_LONGINT_FORMAT ", minnodes=%" SCIP_LONGINT_FORMAT "\n", nnodes, heurdata->minnodes);
      return SCIP_OKAY;
   }
   ...
   {
      SCIPdebugMsg(scip, "skipping proximitybenders: pure feasibility problem anyway\n");
      return SCIP_OKAY;
   }
   ...
   do
   {
      ...
      SCIP_CALL( SCIPapplyProximitybenders(scip, heur, result, heurdata->minimprove, nnodes, nlpiters, &nusednodes, &nusedlpiters, FALSE) );
...
#ifndef NDEBUG
      SCIP_CALL( SCIPdeleteSubproblemProximitybenders(scip) );
#else
      SCIP_CALL( deleteSubproblem(scip, heurdata) );
#endif
   }
   ...
   )
{
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   if( heurdata->subscip == NULL )
   {
      ...
      eventhdlr = NULL;
      SCIP_CALL( SCIPincludeEventhdlrBasic(subscip, &eventhdlr, EVENTHDLR_NAME, EVENTHDLR_DESC, eventExecProximitybenders, NULL) );
      if( eventhdlr == NULL )
      {
   ...
   )
{
   ...
   SCIP_CALL( SCIPincludeHeurBasic(scip, &heur,
         ...
         HEUR_MAXDEPTH, HEUR_TIMING, HEUR_USESSUBSCIP, heurExecProximitybenders, heurdata) );
   assert(heur != NULL);

   /* set non-NULL pointers to callback methods */
   SCIP_CALL( SCIPsetHeurCopy(scip, heur, heurCopyProximitybenders) );
   SCIP_CALL( SCIPsetHeurFree(scip, heur, heurFreeProximitybenders) );
   SCIP_CALL( SCIPsetHeurInit(scip, heur, heurInitProximitybenders) );
   SCIP_CALL( SCIPsetHeurExitsol(scip, heur, heurExitsolProximitybenders) );
