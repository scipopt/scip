...
/*                                                                           */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
#pragma ident "@(#) $Id: heur_actconsdiving.c,v 1.1 2006/09/22 01:30:29 bzfpfend Exp $"
...
#define heurInitsolActconsdiving NULL


/** solving process deinitialization method of primal heuristic (called before branch and bound process data is freed) */
#define heurExitsolActconsdiving NULL
...
{  /*lint --e{715}*/
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   if( SCIPisObjIntegral(scip) ) /*?????????????? do this also in other diving heuristics! */
   ...
   *result = SCIP_DIDNOTFIND;
         ...
         || (divedepth < maxdivedepth && heurdata->nlpiterations < maxnlpiterations && objval < searchbound)) )
   {
      ...
      do
      {
         /* if we have backtracked, select the same variable but with opposite direction */
         if( !backtrack )
         {
         ...
         if( cutoff && !backtrack ) /*?????????????? do this also in other diving heuristics! */
         {
   ...
   SCIPdebugMessage("(node %"SCIP_LONGINT_FORMAT") finished actconsdiving heuristic: %d fractionals, dive %d/%d, LP iter %"SCIP_LONGINT_FORMAT"/%"SCIP_LONGINT_FORMAT", objval=%g/%g, lpsolstat=%d, cutoff=%d\n", 
   ...
   )
{
   ...
   SCIP_CALL( SCIPincludeHeur(scip, HEUR_NAME, HEUR_DESC, HEUR_DISPCHAR, HEUR_PRIORITY, HEUR_FREQ, HEUR_FREQOFS,
         ...
         heurFreeActconsdiving, heurInitActconsdiving, heurExitActconsdiving, 
         heurInitsolActconsdiving, heurExitsolActconsdiving, heurExecActconsdiving,
         heurdata) );
