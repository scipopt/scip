   ...
   )
{
   ...
   SCIPdebugMessage("%p eval jacobian for row %d\n", (void*)oracle, rowidx);
   ...
   if( cons->expr == NULL )
   {
      ...
      if( cons->nlinidxs > 0 )
      {
         ...
         if( convals != NULL )
            for( l = 0; l < cons->nlinidxs; ++l )
               convals[rowidx] += cons->lincoefs[l] * x[cons->linidxs[l]];
      }
      assert(j == oracle->jacrowoffsets[rowidx+1]);
   }
   else
   {
      /* eval grad for nonlinear and add to jacobi */
      ...
      SCIPdebug( printf("g ="); for( l = oracle->jacoffsets[rowidx]; l < oracle->jacoffsets[rowidx+1]; ++l)
            printf(" %g", grad[oracle->jaccols[l]]); printf("\n"); )

      if( !SCIPisFinite(nlval) || SCIPisInfinity(scip, ABS(nlval)) )
      {
         SCIPdebugMessage("gradient evaluation for row %d yield invalid function value %g\n", rowidx, nlval);
         retcode = SCIP_INVALIDDATA; /* indicate that the function could not be evaluated at given point */
         goto TERMINATE;
      }
      if( convals != NULL )
         convals[rowidx] = nlval;
       ...
       * use the already evaluated sparsity pattern to pick only elements from grad that could have been set
       */
      assert(j == oracle->jacrowoffsets[rowidx]);
      for( ; j < oracle->jacrowoffsets[rowidx+1]; ++j )
      {
         if( !SCIPisFinite(grad[oracle->jaccols[j]]) )
         {
            SCIPdebugMessage("gradient evaluation yield invalid gradient value %g\n", grad[l]);
