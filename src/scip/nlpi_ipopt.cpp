   ...
   )
{
   ...
   if( param.iterlimit < 0 )
   {
      ...
      else
      {
         ...
         const int* offset;
         SCIP_CALL( SCIPnlpiOracleGetJacobianRowSparsity(scip, nlpiproblem->oracle, &offset, NULL) );
         jacnnz = offset[m];
   ...
   )
{
   ...
   const int* offset;
   const int* col;
   SCIP_CALL_ABORT( SCIPnlpiOracleGetJacobianRowSparsity(scip, problem->oracle, &offset, NULL) );
   jacnnz = offset[m];
