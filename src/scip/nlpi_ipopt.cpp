...
/* Script used to fit iteration guess (see handleNlpParam()) to recorded iteration numbers (see collectStatistic()):
...
sys.path.insert(0,'/path/to/ipyopt/build/lib.linux-x86_64-3.9')
...
for l in open(sys.argv[1]) :
   if not l.startswith('IPOPTSTAT ') :
      continue
   l = l[10:]
   if l.startswith('status') and seenheader :
      continue
   file_str.write(l)
   seenheader = True
file_str.seek(0)
...
print(df['expectinfeas'].value_counts())

# throw away bad status and other settings
df = df.loc[(df['status'] == 0) & (df['expectinfeas'] == 1),:]

# throw away small number of iter
df = df.loc[df['iter'] > 20,:]

# fix integer overflow in data
df['linsys11density'] = df['linsys11nz'] / (df['nvars'] + df['nlincons'] + df['nnlcons']).pow(2)
df['linsys13density'] = df['linsys13nz'] / (df['nvars'] + df['nlincons'] + df['nnlcons']).pow(2)

print('Iter mean:', df['iter'].mean(), 'stddev:', df['iter'].std())

#drop outliers w.r.t. niter (iterations that are more than x*stddev away from mean)
outliers = stats.zscore(df['iter'])>10
print('Outliers:', outliers.sum(), 100.0 * outliers.sum() / len(df.index), '%')
print(sorted(df.loc[outliers,'iter']))
df = df.loc[~outliers,:]
print('Iter mean:', df['iter'].mean(), 'stddev:', df['iter'].std())

niter = df['iter']

# drop columns that are not problem characteristics
# drop objnl because always 0 (for subnlp nlps, as scip objective is always linear)
param = df.drop(columns = ['status','iter','time','expectinfeas','objnl'])
# linsys13nz is linear dependent on other cols (=linsys11nz + m)
param.drop(columns = ['linsys13nz','linsys13density'], inplace = True)

# linsys11nz also seems linear dependent on other cols
param.drop(columns = ['linsys11nz'], inplace = True)

# drop columns that requires hessian
param.drop(columns = ['hesnnz','linsys11density'], inplace = True)

# merge nvarlb and nvarub into one column
param['nvarbnd'] = param['nvarlb'] + param['nvarub']
param.drop(columns = ['nvarlb','nvarub'], inplace = True)

# additional data columns derived from existing columns
for c in param.columns :
   if c == 'linsys11density' :
      param[c+'_sqr'] = param[c].pow(2.0)
      continue
   #param[c+'_sqr'] = param[c].pow(2.0)
   param[c+'_sqrt'] = param[c].pow(0.5)
   #param[c+'_log1p'] = np.log1p(param[c])

# check for nan
for c in param.columns:
   n = param[c].isna()
   if n.sum() > 0 :
      print('nan in column', c)
      print(param.loc[n].to_string(max_cols=100))

param['intercept'] = 1

param.sort_index(axis = 1, inplace = True)

niterfull = niter
paramfull = param
param = param.sample(frac = 0.5, random_state = 26051979)
niter = niter.loc[param.index]
paramalt = paramfull.drop(param.index)
niteralt = niterfull.loc[paramalt.index]

print(param.to_string(max_cols=30,max_rows=10))

#https://numpy.org/doc/stable/reference/generated/numpy.linalg.lstsq.html
A = param.to_numpy()
ret = np.linalg.lstsq(A,niter,rcond=None)
print('Coefs:', ret[0])
print('Residuals:', ret[1],'E/n:',np.sqrt(ret[1])/len(niter))
print('Rank:', ret[2], '#Param:', len(param.columns))

for c in range(len(param.columns)) :
   print('%-20s %+.10f' % (param.columns[c], ret[0][c]))

def eval(A, coefs, niter) :
   over = 0
   nover = 0
   under = 0
   nunder = 0
   for r in range(len(A)) :
      if r < 0 :
         print(np.dot(A[r],coefs), niter.iloc[r])
      diff = np.dot(A[r],coefs) - niter.iloc[r]
      if diff > 0 :  # estimate larger than data
         over += diff
         nover += 1
      else :  # estimate lower-equal than data
         under += -diff
         nunder += 1
   print('arith mean  overestimate:', over/len(A), '#:', nover)
   print('arith mean underestimate:', under/len(A), '#:', nunder)

eval(A, ret[0], niter)
print('Control set:')
eval(paramalt.to_numpy(), ret[0], niteralt)


####  Now instead of least-square, penalize overestimate differently from underestimate

'''
min sppenalty * s+^2 + smpenalty * (s- or s-^2)
s.t. Ax + s+ - s- = iter
    s+, s- >= 0

s+ is about underestimating
s- is about overestimating
'''
sppenalty = 2.0
smpenalty = 1.0
smquad = True

nparam  = len(param.columns)
nsample = len(param.index)

nvars = nparam + 2*nsample

def eval_f(x) :
   assert(len(x) == nvars)
   obj = 0.0
   for i in range(nsample) :
      obj += sppenalty * x[nparam + i] * x[nparam + i] + smpenalty * x[nparam + nsample + i] * (x[nparam + nsample + i] if smquad else 1.0)
   return obj

def eval_grad_f(x, out) :
   assert(len(x) == nvars)
   assert(len(out) == nvars)
   out[0:nparam] = 0.0   # x
   for i in range(nsample) :
      out[nparam+i] = 2.0 * sppenalty * x[nparam+i]
   if smquad :
      for i in range(nsample) :
         out[nparam+nsample+i] = 2.0 * smpenalty * x[nparam+nsample+i]
   else :
      out[nparam+nsample:nparam+2*nsample] = smpenalty
   return out

def eval_g(x, out) :
   assert(len(x) == nvars)
   assert(len(out) == nsample)
   for i in range(nsample) :
      out[i] = np.dot(A[i],x[0:nparam]) + x[nparam + i] - x[nparam + nsample + i]  # Ax + s+ - s-
   return out

jac_g_sparsity_indices = ( list(), list() )
jac_g = ( list() )
for i in range(nsample) :
   for j in range(nparam) :
      jac_g_sparsity_indices[0].append(i)
      jac_g_sparsity_indices[1].append(j)
      jac_g.append(A[i][j])
   jac_g_sparsity_indices[0].append(i)
   jac_g_sparsity_indices[1].append(nparam + i)  # s+
   jac_g.append(1)
   jac_g_sparsity_indices[0].append(i)
   jac_g_sparsity_indices[1].append(nparam + nsample + i)  # -s-
   jac_g.append(-1)

def eval_jac_g(x, out) :
   for i in range(len(jac_g)) : out[i] = jac_g[i]
   return out

h_sparsity_indices = ( list(), list() )
for i in range(2*nsample if smquad else nsample) :
   h_sparsity_indices[0].append(nparam + i)
   h_sparsity_indices[1].append(nparam + i)

def eval_h(x, lagrange, obj_factor, out) :
   for i in range(nsample) :
      out[i] = 2.0 * sppenalty * obj_factor
      if smquad :
         out[nsample + i] = 2.0 * smpenalty * obj_factor
   return out

x_L = np.zeros(nvars, dtype=float)
x_L[0:nparam] = -np.inf

x_U = np.ones(nvars, dtype=float) * np.inf

g_L = niter.to_numpy(dtype=float)
g_U = g_L

ipopt_options = {'bound_push' : 1e-9, 'bound_frac' : 1e-9, 'bound_relax_factor' : 0.0, 'jac_c_constant' : 'yes', 'hessian_constant' : 'yes'}
...
x0 = np.zeros(nvars)
for i in range(nsample) :
   x0[nparam + i] = niter.iloc[i]

fullret, _ , _ = nlp.solve(x0)
ret = fullret[0:nparam]
for c in range(len(param.columns)) :
   print('%-20s %+.10f' % (param.columns[c], ret[c]))

eval(A, ret, niter)
print('Control set:')
eval(paramalt.to_numpy(), ret, niteralt)

plt.scatter(niter, param.dot(ret), marker = '.')
plt.scatter(niteralt, paramalt.dot(ret), marker = '.', color = 'darkblue')
plt.plot([0, 1], [0, 1], transform=plt.gca().transAxes)
plt.gca().set_yscale("log");
plt.gca().set_xscale("log");
plt.xlabel('Data');
plt.ylabel('Estimate');
plt.savefig('iterestimate.png')
plt.show()

####  Now compare with simple iter=150 guess
sys.exit(0)
ret = np.zeros(nparam)
ret[param.columns.get_loc('intercept')] = 150.0
for c in range(len(param.columns)) :
   print('%-20s %+.10f' % (param.columns[c], ret[c]))
