...
/*                                                                           */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
#pragma ident "@(#) $Id: lpi_qso.c,v 1.1 2008/10/09 19:41:39 bzfpfets Exp $"
   ...
   )
{
   ...
   for ( i = ncols ; --i ; )
   {
      lpi->iccnt[i-1] = beg[i] - beg[i-1];
      assert(lpi->iccnt[i-1] >= 0);
   }
   lpi->iccnt[ncols-1] = nnonz - (ncols>1 ? beg[ncols-2] : 0);
   ...
   )
{
   ...
   double* colval=0, *lb=0, *ub=0, *obj=0;
   int rval = 0;
   ...
   /* ending */
 CLEANUP:
   if (colcnt) QSfree(colcnt);
   if (colbeg) QSfree(colbeg);
   if (colind) QSfree(colind);
   if (colval) QSfree(colval);
   if (obj) QSfree(obj);
   if (lb) QSfree(lb);
   if (ub) QSfree(ub);
   QS_RETURN(rval);
   ...
   )
{
   ...
   /* get data from qsopt */
   rval = QSget_columns_list(lpi->prob, len, lpi->iccnt, nnonz ? (&lcnt) : NULL, nnonz ? (&lbeg) : NULL, nnonz ? (&lind) : NULL,
      nnonz ? (&lval) : NULL, NULL, lb ? (&llb) : NULL, lb ? (&lub) : NULL, NULL);  /*lint !e826*/
   ...
   )
{
   ...
   /* get data from qsopt */
   rval = QSget_ranged_rows_list(lpi->prob, len, lpi->ircnt, nnonz ? (&lcnt) : NULL, nnonz ? (&lbeg) : NULL, nnonz ? (&lind) : NULL,
      nnonz ? (&lval) : NULL, rhs ? (&lrhs) : NULL, rhs ? (&lsense) : NULL, rhs ? (&lrng) : NULL, NULL);  /*lint !e826*/
   QS_TESTG(rval, CLEANUP, " ");
   ...
   )
{
   ...
   SCIPdebugMessage("calling QSopt primal simplex: %d cols, %d rows, %d nz\n",
      QSget_colcount(lpi->prob), QSget_rowcount(lpi->prob), QSget_nzcount(lpi->prob));
   ...
   )
{
   ...
   SCIPdebugMessage("calling QSopt dual simplex: %d cols, %d rows, %d nz\n",
      QSget_colcount(lpi->prob), QSget_rowcount(lpi->prob),
      QSget_nzcount(lpi->prob));
   ...
   )
{
   ...
   SCIPdebugMessage("calling QSopt strongbranching on variable %d (%d it lim)\n", col, itlim);
   ...
   )
{
   ...
   /* the current version of QSopt can not give a primal certificate of unboundness */
   return FALSE;
}
   ...
   )
{
   ...
   QS_ERROR(rval, "Could not load basis from file.");
   ...
   )
{
   ...
   QS_TESTG((rval = !bas), CLEANUP, "Could not get basis from problem.");   /*lint !e820*/
   rval = QSwrite_basis(lpi->prob, bas, fname);
   QS_TESTG(rval, CLEANUP, "Could not write basis to file.");
   ...
   )
{
   ...
   rval =((lpi->prob = QSread_prob(fname, "LP"))!=0);
   ...
   )
{
   ...
   rval = QSwrite_prob (lpi->prob, fname, "LP");
