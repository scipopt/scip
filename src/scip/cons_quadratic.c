...
struct SCIP_ConshdlrData
{
   ...
   SCIP_Bool             addkkt;             /**< if TRUE then try to add the KKT conditions as additional (redundant) constraints in presolving */
   SCIP_Bool             addkktbinary;       /**< if TRUE then allow binary variables for KKT update (for this method, conshdlrdata->addkkt has to be set to TRUE) */
...
/** tries to add the KKT conditions as additional (redundant) constraints to the quadratic program */
static
SCIP_RETCODE presolveAddKKT(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONSHDLR*        conshdlr,           /**< constraint handler data structure */
   SCIP_CONS*            cons,               /**< quadratic constraint */
   SCIP_Bool             updatequadbinary,   /**< if TRUE then also apply the update to mixed binary QPs */
...
{  /*lint --e{715,788}*/
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   /* if there is only one quadratic constraint, then try to add the KKT conditions as additional (redundant) constraints */
   if ( nconss == 1 && conshdlrdata->addkkt )
   {
      SCIP_CONS* cons;
      ...
      /* only apply KKT update to nonconvex and nonconcave quadratic constraints */
      if ( ! SCIPisConvexQuadratic(scip, cons) && ! SCIPisConcaveQuadratic(scip, cons) )
      {
         SCIP_CALL( presolveAddKKT(scip, conshdlr, cons, conshdlrdata->addkktbinary, TRUE, naddconss, ndelconss) );
      }
   }
   ...
   )
{
   ...
   SCIP_CALL( SCIPaddBoolParam(scip, "constraints/" CONSHDLR_NAME "/addkkt",
         "if TRUE then try to add the KKT conditions as additional (redundant) constraints in presolving",
         &conshdlrdata->addkkt, TRUE, TRUE, NULL, NULL) );

   SCIP_CALL( SCIPaddBoolParam(scip, "constraints/" CONSHDLR_NAME "/addkktbinary",
         "if TRUE then allow binary variables for KKT update (for this method, conshdlrdata->addkkt has to be set to TRUE)",
         &conshdlrdata->addkktbinary, TRUE, FALSE, NULL, NULL) );
