   ...
   )
{
   ...
   if( !iis->irreducible && minimal && !(timelim - SCIPclockGetTime(iis->iistime) <= 0 || (nodelim != -1 && iis->nnodes > nodelim)) && !trivial )
   {
      ...
      /*                                 iis, timelim, nodelim, removebounds, silent, timelimperiter, nodelimperiter, additive, conservative, delafteradd, dynamicreordering, initbatchsize, initrelbatchsize, maxbatchsize, maxrelbatchsize, batchingfactor, batchingoffset, result */
      SCIP_CALL( SCIPexecIISfinderGreedy(iis, timelim, nodelim, removebounds, silent, 1e+20,          -1L,            FALSE,    TRUE,         TRUE,        TRUE,              1,             -1.0,             1,            1.0,             1.0,            0,              &result) );
      assert( result == SCIP_SUCCESS || result == SCIP_DIDNOTFIND || result == SCIP_DIDNOTRUN );
   }
