   ...
   )
{
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   if( propdata->nlpiprob == NULL )
   {
      ...
      {
         SCIP_CALL( SCIPaddConvexNlpRowsNlobbt(scip, propdata->nlpi, propdata->nlpiprob, propdata->var2nlpiidx, SCIPgetLPRows(scip),
               SCIPgetNLPRows(scip)) );
      }
   }
   else
   {
      SCIP_CALL( SCIPupdateConvexNlpNlobbt(scip, propdata->nlpi, propdata->nlpiprob, propdata->var2nlpiidx,
            propdata->nlpivars, propdata->nlpinvars) );
   }
