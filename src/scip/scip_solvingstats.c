...
{
   SCIP_CALL_ABORT( SCIPcheckStage(scip, "SCIPprintSummaryStatistics", FALSE, FALSE, FALSE, FALSE, FALSE, TRUE, TRUE, TRUE, FALSE, TRUE, TRUE, FALSE, FALSE, FALSE) );

   printf("STATS Problem %s \n", SCIPprobGetName(scip->origprob));
   // printf("STATS RESOLUTION HEADER: (Time, Calls, Success, Conflicts, LargeCoefs, LongConfs, WeakPer, Length, Fails, Unresolvable)\n");
   printf("STATS ResTime %.2f\n", SCIPconflictGetResTime(scip->conflict));
   printf("STATS ResCalls %lld\n", SCIPconflictGetNResCalls(scip->conflict));
   printf("STATS ResSuccess %lld\n", SCIPconflictGetNResSuccess(scip->conflict));
   printf("STATS ResConflicts %lld\n", SCIPconflictGetNResConflictConss(scip->conflict));
   printf("STATS ResLargeCoefs %lld\n", SCIPconflictGetNResLargeCoefs(scip->conflict));
   printf("STATS ResLongConfs %lld\n", SCIPconflictGetNResLongConflicts(scip->conflict));
   printf("STATS ResWeakPer %.2f\n", SCIPconflictGetWeakeningPercentage(scip->conflict));
   printf("STATS ResLength %.2f\n", SCIPconflictGetNResConflictConss(scip->conflict) > 0
         ? (SCIP_Real)SCIPconflictGetNResConflictVars(scip->conflict)
         / (SCIP_Real)SCIPconflictGetNResConflictConss(scip->conflict) : 0);
   printf("STATS ResFails %lld\n", SCIPconflictGetNResUnkTerm(scip->conflict));
   printf("STATS ResUnresolvable %lld\n", SCIPconflictGetNUnresolvable(scip->conflict));
   // printf("STATS RESOLUTION: %.2f %lld %lld %lld %lld %lld %.2f %.2f %lld %lld\n",
   //       SCIPconflictGetResTime(scip->conflict),
   //       SCIPconflictGetNResCalls(scip->conflict),
   //       SCIPconflictGetNResSuccess(scip->conflict),
   //       SCIPconflictGetNResConflictConss(scip->conflict),
   ...
   //       SCIPconflictGetNResUnkTerm(scip->conflict),
   //       SCIPconflictGetNUnresolvable(scip->conflict));
   // printf("STATS GRAPH HEADER: (Time, Calls, Success, Conflicts, LargeCoefs, LongConfs, WeakPer, Length, Fails, Unresolvable)\n");
   printf("STATS GraphTime %.2f\n", SCIPconflictGetPropTime(scip->conflict));
   printf("STATS GraphCalls %lld\n", SCIPconflictGetNPropCalls(scip->conflict));
   printf("STATS GraphSuccess %lld\n", SCIPconflictGetNPropSuccess(scip->conflict));
   printf("STATS GraphConflicts %lld\n", SCIPconflictGetNPropConflictConss(scip->conflict));
   printf("STATS GraphLargeCoefs %lld\n",(long long) 0);
   printf("STATS GraphLongConfs %lld\n",(long long) 0);
   printf("STATS GraphWeakPer %.2f\n", 100.0);
   printf("STATS GraphLength %.2f\n", SCIPconflictGetNPropConflictConss(scip->conflict) > 0
         ? (SCIP_Real)SCIPconflictGetNPropConflictLiterals(scip->conflict)
         / (SCIP_Real)SCIPconflictGetNPropConflictConss(scip->conflict) : 0);
   printf("STATS GraphFails %lld\n",(long long) 0);
   printf("STATS GraphUnresolvable %lld\n",(long long) 0);
   printf("STATS Status %d\n", SCIPgetStatus(scip));
   printf("STATS Nodes %lld\n", SCIPgetNNodes(scip));
   printf("STATS Time %.2f\n", SCIPgetSolvingTime(scip));
   printf("STATS DualLPIterations %lld\n", SCIPgetNDualLPIterations(scip));


   // Conflict statistics
   printf("STATS ResUseful %lld \n", scip->stat->nusefulresconflicts);
   printf("STATS ResUseless %lld \n", SCIPconflictGetNResConflictConss(scip->conflict) - scip->stat->nusefulresconflicts);
   printf("STATS GraphUseful %lld \n", scip->stat->nusefulpropconflicts);
   printf("STATS GraphUseless %lld \n", SCIPconflictGetNPropConflictConss(scip->conflict) - scip->stat->nusefulpropconflicts);

   printf("STATS FinalStatus %d \n", SCIPgetStatus(scip));
   printf("STATS FinalTime %.2f \n", SCIPgetSolvingTime(scip));
   printf("STATS FinalGap %.2f \n", SCIPgetGap(scip));
   printf("STATS FinalNodes %" SCIP_LONGINT_FORMAT "\n", SCIPgetNNodes(scip));
   printf("STATS FinalDualLPIterations %" SCIP_LONGINT_FORMAT" \n", SCIPgetNDualLPIterations(scip));
   printf("STATS FinalInfeasLPLength %.2f \n", SCIPconflictGetNDualproofsInfSuccess(scip->conflict) > 0
      ? (SCIP_Real)SCIPconflictGetNDualproofsInfNonzeros(scip->conflict)
      / (SCIP_Real)SCIPconflictGetNDualproofsInfSuccess(scip->conflict) : 0);
   printf("STATS FinalBoundexceedingLPLength %.2f \n", SCIPconflictGetNDualproofsBndSuccess(scip->conflict) > 0
      ? (SCIP_Real)SCIPconflictGetNDualproofsBndNonzeros(scip->conflict)
      / (SCIP_Real)SCIPconflictGetNDualproofsBndSuccess(scip->conflict) : 0);

   /* this is not counted here */
   printf("STATS ConflictCutoffs %d \n", 0);
   printf("STATS ConflictBdChgs %d \n", 0);

   SCIP_Real proptime;

   for( int i = 0; i < scip->set->nconshdlrs; ++i )
   {
      SCIP_CONSHDLR* conshdlr;
      int maxnactiveconss;
