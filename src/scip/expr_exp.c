...
SCIP_DECL_EXPRSIMPLIFY(simplifyExp)
{
   ...
   {
      SCIP_CALL( SCIPcreateExprValue(scip, simplifiedexpr, exp(SCIPgetValueExprValue(child)), ownerdatacreate, ownerdatacreatedata) );
   }
   else
   {
      *simplifiedexpr = expr;
...
{  /*lint --e{715}*/
   ...
   SCIP_CALL( SCIPcreateExprExp(scip, expr, childexpr, ownerdatacreate, ownerdatacreatedata) );
   assert(*expr != NULL);
