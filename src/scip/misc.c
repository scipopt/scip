...
/*                                                                           */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
#pragma ident "@(#) $Id: misc.c,v 1.66 2008/02/28 10:19:15 bzfwinkm Exp $"
  ...
  )
{
  ...
  assert(dataptr != NULL || (len == 0));
  assert(indcomp != NULL);
  assert(indarray != NULL || (len == 0));

  for( i = len - 1; i > 0; i--)
    if ( indcomp(dataptr, indarray[i], indarray[i-1]) < 0 )
      return FALSE;
  ...
  )
{
  ...
  for( i = len - 1; i > 0; i--)
    if ( ptrcomp(ptrarray[i], ptrarray[i-1]) < 0 )
      return FALSE;
  ...
  )
{
    ...
    assert(dataptr != NULL || (start == 0 && end == 0));
    assert(indcomp != NULL);
    assert(indarray != NULL || (start == 0 && end == 0));
    ...
    for (k = 3; k >= 0; --k)
      for (h = incs[k], i = h + start; i <= end; ++i)
        { 
          temp = indarray[i];
          j = i;
          while (j >= h && indcomp(dataptr, temp, indarray[j-h]) < 0)
          {
              indarray[j] = indarray[j-h];
              j -= h;
          }
  ...
  )
{
    ...
    assert(dataptr != NULL || end - start == 0);
    assert(indcomp != NULL);
    assert(indarray != NULL || end - start == 0);
    assert(start <= end);
    ...
    for (;;)
    {
      while ( lo < end && indcomp(dataptr, indarray[lo], pivot) <= 0) ++lo;
      while ( hi > start && indcomp(dataptr, indarray[hi], pivot) >= 0) --hi;
      
      if (lo >= hi)
        break;

      temp = indarray[hi];
      indarray[hi] = indarray[lo];
      indarray[lo] = temp;
      ++lo;
      --hi;
    }
    if (lo < mid)
    {
      indarray[mid] = indarray[lo];
      indarray[lo] = pivot;
      ++lo;
    }
    else if (hi > mid)
    {
      indarray[mid] = indarray[hi];
      indarray[hi] = pivot;
      --hi;
    }

    if ( hi-start < 25)
      shellSort(dataptr, start, hi, indcomp, indarray);
    else
      qSort(dataptr, start, hi, indcomp, indarray);
    if ( end-lo < 25)
      shellSort(dataptr, lo, end, indcomp, indarray);
    else
      qSort(dataptr, lo, end, indcomp, indarray);
}
  ...
  )
{
    ...
    qSort(dataptr, 0, len, indcomp, indarray);
    
#ifdef DEBUG
    if ( !checkSorting( dataptr, indcomp,, indarray, len))
      SCIPABORT();
#endif
  ...
  )
{
    ...
    assert(ptrarray != NULL || ( start == 0 && end == 0));
    assert(ptrcomp != NULL);
    ...
    for (k = 3; k >= 0; --k)
      for (h = incs[k], i = h + start; i <= end; ++i)
        { 
          temp = ptrarray[i];
          j = i;
          while (j >= h && ptrcomp(temp, ptrarray[j-h]) < 0)
          {
              ptrarray[j] = ptrarray[j-h];
              j -= h;
          }
  ...
  )
{
    ...
    for (;;)
    {
      while ( lo < end && ptrcomp( ptrarray[lo], pivot) <= 0) ++lo;
      while ( hi > start && ptrcomp(ptrarray[hi], pivot) >= 0) --hi;
      
      if (lo >= hi)
        break;

      temp = ptrarray[hi];
      ptrarray[hi] = ptrarray[lo];
      ptrarray[lo] = temp;
      ++lo;
      --hi;
    }
    if (lo < mid)
    {
      ptrarray[mid] = ptrarray[lo];
      ptrarray[lo] = pivot;
      ++lo;
    }
    else if (hi > mid)
    {
      ptrarray[mid] = ptrarray[hi];
      ptrarray[hi] = pivot;
      --hi;
    }

    if ( hi-start < 25)
      shellSortPtr(ptrarray, start, hi, ptrcomp);
    else
      qSortPtr(ptrarray, start, hi, ptrcomp);
    if ( end-lo < 25)
      shellSortPtr(ptrarray, lo, end, ptrcomp);
    else
      qSortPtr(ptrarray, lo, end, ptrcomp);
   ...
   )
{
   ...
   if (len < 1) return;
   qSortPtr(ptrarray, 0, len, ptrcomp);

#ifdef DEBUG
    if ( !checkSortingPtr(ptrarray, ptrcomp, len))
      SCIPABORT();
#endif
}
  ...
  )
{
    ...
    assert(ptrarray != NULL || ( start == 0 && end == 0));
    assert(ptrcomp != NULL);
    assert(realarray != NULL || ( start == 0 && end == 0));

    for (k = 3; k >= 0; --k)
      for (h = incs[k], i = h + start; i <= end; ++i)
        { 
          temp = ptrarray[i];
          tempreal = realarray[i];
          j = i;
          while (j >= h && ptrcomp(temp, ptrarray[j-h]) < 0)
          {
              ptrarray[j] = ptrarray[j-h];
              realarray[j] = realarray[j-h];
              j -= h;
          }
  ...
  )
{
    ...
    for (;;)
    {
      ...
      ptrarray[hi] = ptrarray[lo];
      ptrarray[lo] = temp;
      tempreal = realarray[hi];
      realarray[hi] = realarray[lo];
      realarray[lo] = tempreal;
      ++lo;
      --hi;
    }
    if (lo < mid)
    {
      ptrarray[mid] = ptrarray[lo];
      ptrarray[lo] = pivot;
      tempreal = realarray[mid];
      realarray[mid] = realarray[lo];
      realarray[lo] = tempreal;
      ++lo;
    }
    else if (hi > mid)
    {
      ...
      ptrarray[hi] = pivot;
      tempreal = realarray[mid];
      realarray[mid] = realarray[hi];
      realarray[hi] = tempreal;
      --hi;
    }

    if ( hi-start < 25)
      shellSortPtrReal(ptrarray, realarray, start, hi, ptrcomp);
    else
      qSortPtrReal(ptrarray, realarray, start, hi, ptrcomp);
    if ( end-lo < 25)
      shellSortPtrReal(ptrarray, realarray, lo, end, ptrcomp);
    else
      qSortPtrReal(ptrarray, realarray, lo, end, ptrcomp);
   ...
   )
{
   ...
   if (len < 1) return;
   qSortPtrReal(ptrarray, realarray, 0, len, ptrcomp);
  ...
  )
{
    ...
    assert(ptrarray != NULL || ( start == 0 && end == 0));
    assert(intarray != NULL || ( start == 0 && end == 0));
    assert(ptrcomp != NULL);
  ...
  )
{
    ...
    if (lo < mid)
    {
      ptrarray[mid] = ptrarray[lo];
      ptrarray[lo] = pivot;
      tempint = intarray[mid];
      intarray[mid] = intarray[lo];
      intarray[lo] = tempint;
      ++lo;
    }
    else if (hi > mid)
    {
      ...
      ptrarray[hi] = pivot;
      tempint = intarray[mid];
      intarray[mid] = intarray[hi];
      intarray[hi] = tempint;
      --hi;
    }

    if ( hi-start < 25)
      shellSortPtrInt(ptrarray, intarray, start, hi, ptrcomp);
    else
      qSortPtrInt(ptrarray, intarray, start, hi, ptrcomp);
    if ( end-lo < 25)
      shellSortPtrInt(ptrarray, intarray, lo, end, ptrcomp);
    else
      qSortPtrInt(ptrarray, intarray, lo, end, ptrcomp);
   ...
   )
{
   ...
   if (len < 1) return;
   qSortPtrInt(ptrarray, intarray, 0, len, ptrcomp);
  ...
  )
{
    ...
    assert(ptrarray != NULL || ( start == 0 && end == 0));
    assert(intarray1 != NULL || ( start == 0 && end == 0));
    assert(intarray2 != NULL || ( start == 0 && end == 0));
    assert(ptrcomp != NULL);
    ...
    for (k = 3; k >= 0; --k)
      for (h = incs[k], i = h + start; i <= end; ++i)
        { 
          temp = ptrarray[i];
          tempint1 = intarray1[i];
          tempint2 = intarray2[i];
          j = i;
          while (j >= h && ptrcomp(temp, ptrarray[j-h]) < 0)
          {
              ptrarray[j] = ptrarray[j-h];
              intarray1[j] = intarray1[j-h];
              intarray2[j] = intarray2[j-h];
              j -= h;
          }
          
          ptrarray[j] = temp; 
          intarray1[j] = tempint1; 
          intarray2[j] = tempint2; 
        }
}
  ...
  )
{
    ...
    for (;;)
    {
      ...
      ptrarray[hi] = ptrarray[lo];
      ptrarray[lo] = temp;
      tempint1 = intarray1[hi];
      intarray1[hi] = intarray1[lo];
      intarray1[lo] = tempint1;
      tempint2 = intarray2[hi];
      intarray2[hi] = intarray2[lo];
      intarray2[lo] = tempint2;
      ++lo;
      --hi;
    }
    if (lo < mid)
    {
      ptrarray[mid] = ptrarray[lo];
      ptrarray[lo] = pivot;
      tempint1 = intarray1[mid];
      intarray1[mid] = intarray1[lo];
      intarray1[lo] = tempint1;
      tempint2 = intarray2[mid];
      intarray2[mid] = intarray2[lo];
      intarray2[lo] = tempint2;
      ++lo;
    }
    else if (hi > mid)
    {
      ...
      ptrarray[hi] = pivot;
      tempint1 = intarray1[mid];
      intarray1[mid] = intarray1[hi];
      intarray1[hi] = tempint1;
      tempint2 = intarray2[mid];
      intarray2[mid] = intarray2[hi];
      intarray2[hi] = tempint2;
      --hi;
    }

    if ( hi-start < 25)
      shellSortPtrIntInt(ptrarray, intarray1, intarray2, start, hi, ptrcomp);
    else
      qSortPtrIntInt(ptrarray, intarray1, intarray2, start, hi, ptrcomp);
    if ( end-lo < 25)
      shellSortPtrIntInt(ptrarray, intarray1, intarray2, lo, end, ptrcomp);
    else
      qSortPtrIntInt(ptrarray, intarray1, intarray2, lo, end, ptrcomp);
   ...
   )
{
   ...
   if (len < 1) return;
   qSortPtrIntInt(ptrarray, intarray1, intarray1, 0, len, ptrcomp);
  ...
  )
{
    ...
    if ( hi-start < 25)
      shellSortPtrRealInt(ptrarray, realarray, intarray, start, hi, ptrcomp);
    else
      qSortPtrRealInt(ptrarray, realarray, intarray, start, hi, ptrcomp);
    if ( end-lo < 25)
      shellSortPtrRealInt(ptrarray, realarray, intarray, lo, end, ptrcomp);
    else
      qSortPtrRealInt(ptrarray, realarray, intarray, lo, end, ptrcomp);
   ...
   )
{
   ...
   if (len < 1) return;
   qSortPtrRealInt(ptrarray, realarray, intarray, 0, len, ptrcomp);
  ...
  )
{
    ...
    if ( hi-start < 25)
      shellSortPtrRealIntInt(ptrarray, realarray, intarray1, intarray2, start, hi, ptrcomp);
    else
      qSortPtrRealIntInt(ptrarray, realarray, intarray1, intarray2, start, hi, ptrcomp);
    if ( end-lo < 25)
      shellSortPtrRealIntInt(ptrarray, realarray, intarray1, intarray2, lo, end, ptrcomp);
    else
      qSortPtrRealIntInt(ptrarray, realarray, intarray1, intarray2, lo, end, ptrcomp);
   ...
   )
{
   ...
   if (len < 1) return;
   qSortPtrRealIntInt(ptrarray, realarray, intarray1, intarray2, 0, len, ptrcomp);
    ...
    for (k = 3; k >= 0; --k)
      for (h = incs[k], i = h + start; i <= end; ++i)
        { 
          ...
          tempreal = realarray[i];
          j = i;
          while (j >= h && tempreal < realarray[j-h])
          {
              ptrarray[j] = ptrarray[j-h];
              realarray[j] = realarray[j-h];
              j -= h;
  ...
  )
{
    ...
    for (;;)
    {
      while ( lo < end && realarray[lo] <= pivot) ++lo;
      while ( hi > start && realarray[hi] >= pivot) --hi;
    ...
    if ( hi-start < 25)
      shellSortRealPtr(realarray, ptrarray, start, hi);
    else
      qSortRealPtr(realarray, ptrarray, start, hi);
    if ( end-lo < 25)
      shellSortRealPtr(realarray, ptrarray, lo, end);
    else
      qSortRealPtr(realarray, ptrarray, lo, end);
   ...
   )
{
   ...
   if (len < 1) return;
   qSortRealPtr(realarray, ptrarray, 0, len);

#ifdef DEBUG
    if ( !checkSortingReal(realarray, len))
      SCIPABORT();
#endif
