...
#else
   SCIP_CALL( cutsLpCalcMIR2(scip->lp, scip->set, scip->stat, scip->transprob, sol,
         boundswitch, usevbds, allowlocal, fixintegralrhs, boundsfortrans, boundtypesfortrans, maxmksetcoefs,
         maxweightrange, minfrac, maxfrac, weights, maxweight, weightinds, nweightinds, rowlensum, sidetypes,
         scale, mksetcoefs, mksetcoefsvalid, mircoef, mirrhs, cutactivity, success, cutislocal, cutrank) );
#endif
   ...
   *rowtoolong = FALSE;
   ...
   while( i < *nrowinds )
   {
      ...
      if( !row->modifiable && (allowlocal || !row->local)
         && absweight * maxweightrange >= maxweight && !SCIPsetIsSumZero(set, weight) )
      {
         int sidetype = sidetypes != NULL ? sidetypes[r] : 0;

         SCIP_CALL( SCIPcutRowAddRow(set->scip, cutrow, row, weight, sidetype) );
   ...
   if( SCIPcutRowGetNNz(cutrow) > 0 )
      *emptyrow = FALSE;
   /* check if the total number of non-zeros is too large */
   if( SCIPcutRowGetNNz(cutrow) > maxmksetcoefs )
      *rowtoolong = TRUE;
   ...
   )
{
   ...
   *success = FALSE;
   ...
   SCIP_CALL( SCIPcutRowCreate(set->scip, &cutrow) );

   /* calculate the row summation */
   SCIP_CALL( cutsSumMIRRow2(set, prob, lp, weights, maxweight, sidetypes, scale, allowlocal,
      maxmksetcoefs, maxweightrange, compress, cutrow, rowinds, &nrowinds,
      &emptyrow, &rowtoolong) );
   assert(allowlocal);
   *cutislocal = SCIPcutRowIsLocal(cutrow);
...

   {
      int i;
      int* mksetinds2;
      SCIP_Real* mksetcoefs2;
      int nnz;
      int* inds;
      SCIP_Real* vals;
      int nmksetcoefs;

      vals = SCIPcutRowGetVals(cutrow);
      inds = SCIPcutRowGetInds(cutrow);
      nnz = SCIPcutRowGetNNz(cutrow);

      if( mksetcoefs != NULL )
      {
         SCIPsetAllocBufferArray(set, &mksetcoefs2, 2*nnz);
         SCIPsetAllocBufferArray(set, &mksetinds2, 2*nnz);
      }
      else
      {
         mksetinds2 = NULL;
      ...
      SCIP_CALL( SCIPcutRowCalcMIR(set->scip, sol, boundswitch, usevbds, allowlocal, fixintegralrhs, boundsfortrans, boundtypesfortrans, minfrac, maxfrac, 1.0, cutrow, NULL, mksetcoefs2, mksetinds2, &nmksetcoefs, success) );
      ...
      if(*success)
      {
         *mirrhs = SCIPcutRowGetRhs(cutrow);

         BMSclearMemoryArray(mircoef, prob->nvars);

         for( i = 0; i < nnz; ++i )
            mircoef[inds[i]] = vals[i];

         if( cutactivity != NULL )
            *cutactivity = SCIPcutRowCalcSolActivity(set->scip, cutrow, sol);
         if( cutrank != NULL )
            *cutrank = SCIPcutRowGetRank(cutrow);
      }
      if( mksetcoefsvalid != NULL )
         *mksetcoefsvalid = (nmksetcoefs != -1);
      if( mksetcoefs != NULL && *mksetcoefsvalid )
      {
         BMSclearMemoryArray(mksetcoefs, prob->nvars);
         for( i = 0; i < nmksetcoefs; ++i  )
         {
            mksetcoefs[mksetinds2[i]] = mksetcoefs2[i];
         }
      }

      if( mksetcoefs2 != NULL )
      {
         SCIPsetFreeBufferArray(set, &mksetinds2);
         SCIPsetFreeBufferArray(set, &mksetcoefs2);
      }

   }

 TERMINATE:
   SCIPcutRowFree(set->scip, &cutrow);
   /* free temporary memory */
   SCIPsetFreeBufferArray(set, &rowinds);
