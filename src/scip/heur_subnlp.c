...
/*                                                                           */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
#pragma ident "@(#) $Id: heur_subnlp.c,v 1.51 2010/12/16 18:35:46 bzfberth Exp $"
   ...
   )
{
   ...
   if( !success )
   {
      SCIPverbMessage(scip, SCIP_VERBLEVEL_FULL, NULL, "In heur_subnlp: failed to copy some plugins to subSCIP, continue anyway\n");
   }
   ...
   for( i = 0; i < heurdata->nvars; i++ )
   {
      ...
      subvar = (SCIP_VAR*) SCIPhashmapGetImage(varsmap, var);
