...
#include <stdio.h>
#include "scip/scip.h"
#include "scip/heur_rens-benders.h"
...
#define EVENTHDLR_NAME         "Rensbenders"
...
struct SCIP_HeurData
{
   ...
   int                   nfails;             /**< the number of consecutive fails */
   int                   maxfails;           /**< the number of permitted fails */
};
 ...
 */
static
SCIP_DECL_EVENTEXEC(eventExecRensbenders)
{
...
SCIP_RETCODE SCIPapplyRensbenders(
   SCIP*                 scip,               /**< original SCIP data structure                                        */
   ...
   )
{
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   if( !success )
   {
      SCIPstatisticPrintf("RENSBENDERS statistic: fixed only %5.2f integer variables --> abort \n", intfixingrate);
      goto TERMINATE;
   }
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   SCIP_CALL( SCIPcopyLargeNeighborhoodSearch(scip, subscip, varmapfw, "rensbenders", fixedvars, fixedvals, nfixedvars, uselprows,
         heurdata->copycuts, &success, NULL) );

   eventhdlr = NULL;
   /* create event handler for LP events */
   SCIP_CALL( SCIPincludeEventhdlrBasic(subscip, &eventhdlr, EVENTHDLR_NAME, EVENTHDLR_DESC, eventExecRensbenders, NULL) );
   if( eventhdlr == NULL )
   {
   ...
   SCIPdebugMsg(scip, "RENSBENDERS subproblem: %d vars, %d cons\n", SCIPgetNVars(subscip), SCIPgetNConss(subscip));
   ...
   {
      SCIPwarningMessage(scip, "Error while presolving subproblem in RENSBENDERS heuristic; sub-SCIP terminated with code <%d>\n", retcode);
   ...
   SCIPdebugMsg(scip, "RENSBENDERS presolved subproblem: %d vars, %d cons, success=%u\n", SCIPgetNVars(subscip), SCIPgetNConss(subscip), success);
    ...
    */
   if( allfixingrate >= minfixingrate / 2.0 || TRUE )
   {
      SCIP_SOL** subsols;
      int nsubsols;
      ...
      if( retcode != SCIP_OKAY )
      {
         SCIPwarningMessage(scip, "Error while solving subproblem in RENSBENDERS heuristic; sub-SCIP terminated with code <%d>\n", retcode);
         SCIPABORT();
      }
      else
      {
      ...
      /* if the upperbound is not improved, then the heuristic is considered as failed */
      if( nsubsols > 0 && SCIPisGE(subscip, SCIPgetSolOrigObj(subscip, SCIPgetBestSol(subscip)), cutoff) )
         heurdata->nfails++;
      else
         heurdata->nfails = 0;

      SCIPstatisticPrintf("RENSBENDERS statistic: fixed %6.3f integer variables, %6.3f all variables, needed %6.1f seconds, %" SCIP_LONGINT_FORMAT " nodes, solution %10.4f found at node %" SCIP_LONGINT_FORMAT "\n",
   ...
   {
      SCIPstatisticPrintf("RENSBENDERS statistic: fixed only %6.3f integer variables, %6.3f all variables --> abort \n", intfixingrate, allfixingrate);
   }
...
static
SCIP_DECL_HEURCOPY(heurCopyRensbenders)
{  /*lint --e{715}*/
   assert(scip != NULL);
   assert(heur != NULL);
...
static
SCIP_DECL_HEURFREE(heurFreeRensbenders)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEURINIT(heurInitRensbenders)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEUREXEC(heurExecRensbenders)
{  /*lint --e{715}*/
   ...
   *result = SCIP_DELAYED;
   ...
   /* if the number of fails exceeds the limit */
   if( heurdata->nfails > heurdata->maxfails )
   {
      if( heurdata->nfails / 2 > heurdata->maxfails )
         heurdata->nfails = heurdata->maxfails - 1;
      else
         heurdata->nfails++;
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   {
      SCIPdebugMsg(scip, "skipping RENSBENDERS: nstallnodes=%" SCIP_LONGINT_FORMAT ", minnodes=%" SCIP_LONGINT_FORMAT "\n", nstallnodes, heurdata->minnodes);
      return SCIP_OKAY;
   }
   ...
   SCIP_CALL( SCIPapplyRensbenders(scip, heur, result, heurdata->minfixingrate, heurdata->minimprove,
...
SCIP_RETCODE SCIPincludeHeurRensbenders(
   SCIP*                 scip                /**< SCIP data structure */
   )
{
   SCIP_HEURDATA* heurdata;
   ...
   SCIP_CALL( SCIPincludeHeurBasic(scip, &heur,
         ...
         HEUR_MAXDEPTH, HEUR_TIMING, HEUR_USESSUBSCIP, heurExecRensbenders, heurdata) );

   assert(heur != NULL);

   /* set non-NULL pointers to callback methods */
   SCIP_CALL( SCIPsetHeurCopy(scip, heur, heurCopyRensbenders) );
   SCIP_CALL( SCIPsetHeurFree(scip, heur, heurFreeRensbenders) );
   SCIP_CALL( SCIPsetHeurInit(scip, heur, heurInitRensbenders) );
   ...
   SCIP_CALL( SCIPaddRealParam(scip, "heuristics/" HEUR_NAME "/minimprove",
         "factor by which RENSBENDERS should at least improve the incumbent",
   ...
   SCIP_CALL( SCIPaddBoolParam(scip, "heuristics/" HEUR_NAME "/extratime",
         "should the RENSBENDERS sub-CIP get its own full time limit? This is only for tesing and not recommended!",
   ...
   SCIP_CALL( SCIPaddBoolParam(scip, "heuristics/" HEUR_NAME "/fullscale",
         "should the RENSBENDERS sub-CIP be solved with cuts, conflicts, strong branching,... This is only for tesing and not recommended!",
