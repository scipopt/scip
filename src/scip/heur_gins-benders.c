...
#include "scip/heur_gins-benders.h"
#include "scip/pub_misc.h"

#define HEUR_NAME             "gins-benders"
   ...
   )
{
   ...
   *nfixings = 0;
   ...
   else
   {
      ...
      {
         SCIPdebugMsg(scip, "Fixed %d < %d variables in ginsbenders heuristic, stopping", *nfixings, fixthreshold);
         *success = FALSE;
      }
   }
   ...
   )
{
   ...
   SCIPverbMessage(scip, SCIP_VERBLEVEL_HIGH, NULL, "Ginsbenders: %s", name);
...
static
SCIP_DECL_HEURCOPY(heurCopyGinsbenders)
{  /*lint --e{715}*/
   assert(scip != NULL);
   assert(heur != NULL);
...
static
SCIP_DECL_HEURFREE(heurFreeGinsbenders)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEURINIT(heurInitGinsbenders)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
/** initialization method of primal heuristic (called after problem was transformed) */
static
SCIP_DECL_HEUREXIT(heurExitGinsbenders)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
   ...
   SCIPstatistic(
      SCIPverbMessage(scip, SCIP_VERBLEVEL_HIGH, NULL, "Ginsbenders: Avg Neighborhood size: %.1f Avg. discrete neighboorhood vars: %.1f\n",
            heurdataAvgNeighborhoodSize(heurdata), heurdataAvgDiscreteNeighborhoodSize(heurdata));
      SCIPverbMessage(scip, SCIP_VERBLEVEL_HIGH, NULL, "Ginsbenders: Max seen distance %d\n", heurdata->maxseendistance);
...
static
SCIP_DECL_HEUREXEC(heurExecGinsbenders)
{  /*lint --e{715}*/
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   do
   {
      ...
      SCIP_CALL( SCIPcopyLargeNeighborhoodSearch(scip, subscip, varmapfw, "ginsbenders", fixedvars, fixedvals, nfixedvars,
      ...
      SCIPdebugMsg(scip, "Solve Ginsbenders subMIP\n");
...
SCIP_RETCODE SCIPincludeHeurGinsbenders(
   SCIP*                 scip                /**< SCIP data structure */
   )
{
   SCIP_HEURDATA* heurdata;
   ...
   SCIP_CALL( SCIPincludeHeurBasic(scip, &heur,
         ...
         HEUR_MAXDEPTH, HEUR_TIMING, HEUR_USESSUBSCIP, heurExecGinsbenders, heurdata) );

   assert(heur != NULL);

   /* set non-NULL pointers to callback methods */
   SCIP_CALL( SCIPsetHeurCopy(scip, heur, heurCopyGinsbenders) );
   SCIP_CALL( SCIPsetHeurFree(scip, heur, heurFreeGinsbenders) );
   SCIP_CALL( SCIPsetHeurInit(scip, heur, heurInitGinsbenders) );
   SCIP_CALL( SCIPsetHeurExit(scip, heur, heurExitGinsbenders) );
