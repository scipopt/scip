...
#include "scip/struct_compress.h"
   ...
   )
{
   ...
   if( set->misc_resetstat && (!set->reopt_enable || !compr->initialized) )
   {
   ...
   )
{
   ...
   /* call solving process initialization method of tree compression */
   if( compr->comprinitsol != NULL )
   {
      /* start timing */
      SCIPclockStart(compr->setuptime, set);

      SCIP_CALL( compr->comprinitsol(set->scip, compr) );
   ...
   )
{
   ...
   /* call solving process deinitialization method of tree compression */
   if( compr->comprexitsol != NULL )
   {
      /* start timing */
      SCIPclockStart(compr->setuptime, set);

      SCIP_CALL( compr->comprexitsol(set->scip, compr) );
   ...
   )
{
   ...
   return (depth >= compr->mindepth && nnodes >= compr->minnnodes);
}
...
/** gets minimal depth level for calling tree compression (returns -1, if no depth threshold exists) */
