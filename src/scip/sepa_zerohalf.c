   ...
   )
{
   ...
   for( i = 0; i < nrows; ++i )
   {
      ...
      if( SCIProwIsModifiable(rows[i]) || !SCIProwIsIntegral(rows[i]) || SCIPgetRowNumImpliedIntCols(scip, rows[i]) > 0 ||
   ...
   )
{
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   if( sepadata->ncuts > 0  )
   {
      ...
      SCIP_CALL( SCIPselectCutsHybrid(scip, sepadata->cuts, NULL, sepadata->randnumgen, sepadata->goodscore, sepadata->badscore,
