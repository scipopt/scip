   ...
   )
{
   ...
   if( parent != NULL )
   {
      ...
      if( singleChild && SCIPnodeGetDepth(parent) == tree->effectiverootdepth && FALSE ) /* ????????????? */
      {
...
{
   SCIP_CALL( SCIPnodeAddBoundinfer(node, blkmem, set, stat, prob, tree, lp, branchcand, eventqueue, var, newbound,
   ...
   )
{
   ...
   for( i = 0; i < npendingbdchgs; ++i )
   {
      ...
      SCIP_CALL( SCIPnodeAddBoundinfer(tree->pendingbdchgs[i].node, blkmem, set, stat, prob, tree, lp, branchcand,
   ...
   for( i = 0; i < nboundchgs && !(*cutoff); ++i )
   {
      ...
      if( SCIPvarIsBinary(var) )
      {
         ...
         for( j = 0; j < nimpls; ++j )
         {
            ...
            SCIP_CALL( SCIPnodeAddBoundinfer(node, blkmem, set, stat, prob, tree, lp, branchcand, eventqueue,
   ...
   )
{
   ...
   *cutoff = FALSE;
   ...
   SCIP_CALL( treeApplyPendingBdchgs(tree, blkmem, set, stat, prob, lp, branchcand, eventqueue) );
   ...
   )
{
   ...
   /* create fork data */
   SCIP_CALL( forkCreate(&fork, blkmem, set, prob, tree, lp) );
   ...
   )
{
   ...
   /* create subroot data */
   SCIP_CALL( subrootCreate(&subroot, blkmem, set, prob, tree, lp) );
   ...
   if( tree->nchildren > 0 )
   {
      ...
      if( tree->focusnodehaslp && lp->isrelax )
      {
         ...
         if( tree->focusnode->depth > 0 && tree->focusnode->depth % 25 == 0 )
         {
            ...
            SCIP_CALL( focusnodeToSubroot(blkmem, set, messagehdlr, stat, eventqueue, eventfilter, prob, tree, lp, branchcand) );
   ...
   )
{
   ...
   if( SCIPvarGetType(var) == SCIP_VARTYPE_CONTINUOUS )
   {
      if( SCIPsetIsRelEQ(set, SCIPvarGetLbLocal(var), SCIPvarGetUbLocal(var)) )
      {
         ...
         {
            SCIP_CALL( SCIPnodeAddBoundchg(tree->focusnode, blkmem, set, stat, prob, tree, lp, branchcand, eventqueue,
   ...
   )
{
   ...
   SCIP_CALL( treeBacktrackProbing(tree, blkmem, set, stat, prob, lp, branchcand, eventqueue, eventfilter, probingdepth) );
   ...
   )
{
   ...
   SCIP_CALL( treeBacktrackProbing(tree, blkmem, set, stat, prob, lp, branchcand, eventqueue, eventfilter, -1) );
   ...
   if(node->domchg != NULL)
   {
      for(i = 0; i < node->domchg->domchgbound.nboundchgs; i++ )
      {
         if( node->domchg->domchgbound.boundchgs[i].boundchgtype == SCIP_BOUNDCHGTYPE_BRANCHING
          && branching )
            ndomchgs++;
         else if( node->domchg->domchgbound.boundchgs[i].boundchgtype == SCIP_BOUNDCHGTYPE_CONSINFER
               && consinfer )
            ndomchgs++;
   ...
   if( branchvarssize >= *nbranchvars )
   {
      ...
      for(i = 0, p = 0; i < nboundchgs; i++)
      {
          ...
          || (boundchgs[i].boundchgtype == SCIP_BOUNDCHGTYPE_CONSINFER && boundchgs[i].data.inferencedata.reason.cons != NULL) )
         {
            if( boundchgs[i].var->vartype == SCIP_VARTYPE_BINARY )
            {
               branchvars[p] = boundchgs[i].var;
               boundtypes[p] = (SCIP_BOUNDTYPE) boundchgs[i].boundtype;
               branchbounds[p] = boundchgs[i].newbound;
               p++;
            }
         }
      }
...
)
{
   ...
   if( varssize >= *nvars )
   {
      for(i = nboundchgs-1, v = 0; i >= 0; i--)
      {
         if( boundchgs[i].boundchgtype != SCIP_BOUNDCHGTYPE_BRANCHING ) /*lint !e641*/
         {
            if( boundchgs[i].var->vartype == SCIP_VARTYPE_BINARY )
            {
               vars[v] = boundchgs[i].var;
               boundtypes[v] = (SCIP_BOUNDTYPE) boundchgs[i].boundtype;
               bounds[v] = boundchgs[i].newbound;
               v++;
            }
         }
         else
...
)
{
   ...
   while( SCIPnodeGetNumber(node) != SCIPnodeGetNumber(parent) )
   {
      ...
      SCIPnodeGetParentConsAndPropReopt(node, &branchvars[start], &branchbounds[start], &boundtypes[start], &nodenbranchvars, size);
      *nbranchvars += nodenbranchvars;
