...
SCIP_DECL_HASHKEYEQ(hashdataKeyEqCons)
{
   ...
   assert(scip != NULL);
#endif
   assert(hashdata1->vars != NULL);
   assert(hashdata2->vars != NULL);
   ...
   )
{
   ...
   if( !presoldata->usefulsetppcexist )
   {
      ...
      for (c = 0; c < nsetppcs; ++c)
      {
         ...
         if( SCIPgetTypeSetppc(scip, setppcs[c]) == SCIP_SETPPCTYPE_PACKING && SCIPgetNVarsSetppc(scip, setppcs[c]) == 2
               && !SCIPconsIsModifiable(setppcs[c]) )
         {
      ...
      if( nusefulconss > 0 )
      {
         ...
         for (c = 0; c < nusefulconss; ++c)
         {
            ...
            SCIP_CALL(
                  SCIPduplicateBlockMemoryArray(scip, &(presoldata->setppchashdatas[h]->vars), SCIPgetVarsSetppc(scip, usefulconss[c]), 2));

            SCIP_CALL(
                  SCIPgetBinvarRepresentative(scip, presoldata->setppchashdatas[h]->vars[0],
                        &(presoldata->setppchashdatas[h]->vars[0]), &(negated[0])));
            SCIP_CALL(
                  SCIPgetBinvarRepresentative(scip, presoldata->setppchashdatas[h]->vars[1],
                        &(presoldata->setppchashdatas[h]->vars[1]), &(negated[1])));

            if( SCIPvarGetStatus(presoldata->setppchashdatas[h]->vars[0]) == SCIP_VARSTATUS_FIXED
                  || SCIPvarGetStatus(presoldata->setppchashdatas[h]->vars[0]) == SCIP_VARSTATUS_MULTAGGR
                  || SCIPvarGetStatus(presoldata->setppchashdatas[h]->vars[1]) == SCIP_VARSTATUS_FIXED
                  || SCIPvarGetStatus(presoldata->setppchashdatas[h]->vars[1]) == SCIP_VARSTATUS_MULTAGGR )
            {
            ...
            if( SCIPvarGetIndex(presoldata->setppchashdatas[h]->vars[0])
                  > SCIPvarGetIndex(presoldata->setppchashdatas[h]->vars[1]) )
            {
   ...
   if( !presoldata->usefullogicorexist )
   {
      ...
      if( nusefulconss > 0 )
      {
         ...
         SCIP_CALL(
               SCIPduplicateBlockMemoryArray(scip, &presoldata->usefullogicor, usefulconss, presoldata->susefullogicor));
      }
   }
   ...
   if( presoldata->usefulsetppcexist )
   {
      ...
      for (c = presoldata->nsetppchashdatas - 1; c >= 0; --c)
      {
         ...
         else
         {
            ...
            SCIP_CALL(
                  SCIPgetBinvarRepresentative(scip, presoldata->setppchashdatas[c]->vars[0], &(vars[0]), &(negated[0])));
            SCIP_CALL(
                  SCIPgetBinvarRepresentative(scip, presoldata->setppchashdatas[c]->vars[1], &(vars[1]), &(negated[1])));

            if( SCIPvarGetStatus(vars[0]) == SCIP_VARSTATUS_FIXED || SCIPvarGetStatus(vars[0]) == SCIP_VARSTATUS_MULTAGGR
                  || SCIPvarGetStatus(vars[1]) == SCIP_VARSTATUS_FIXED
                  || SCIPvarGetStatus(vars[1]) == SCIP_VARSTATUS_MULTAGGR
                  || presoldata->setppchashdatas[c]->vars[0] != vars[0]
                  || presoldata->setppchashdatas[c]->vars[1] != vars[1] )
            {
               removeentry = TRUE;
            }
         }
         ...
         if( removeentry )
         {
            ...
            if( c < presoldata->nsetppchashdatas - 1 )
            {
               ...
               SCIP_CALL(
                     SCIPhashtableRemove(presoldata->hashdatatable,
                           (void* ) presoldata->setppchashdatas[presoldata->nsetppchashdatas - 1]));
            }
   ...
   )
{
      ...
      if( SCIPgetTypeSetppc(scip, setppcs[c]) == SCIP_SETPPCTYPE_PACKING && SCIPgetNVarsSetppc(scip, setppcs[c]) == 2 && !SCIPconsIsModifiable(setppcs[c]) )
      {
         ...
         if( !SCIPhashtableExists(presoldata->setppchashtable, (void*) setppcs[c]) )
         {
            ...
            if( presoldata->nsetppchashdatas == presoldata->ssetppchashdatas )
            {
               ...
               SCIP_CALL(
                     SCIPreallocBlockMemoryArray(scip, &(presoldata->setppchashdatas), presoldata->ssetppchashdatas, newsize));
               SCIP_CALL(
                     SCIPreallocBlockMemoryArray(scip, &(presoldata->setppchashdatastore), presoldata->ssetppchashdatas, newsize));
            ...
            presoldata->setppchashdatas[presoldata->nsetppchashdatas] =
                  &(presoldata->setppchashdatastore[presoldata->nsetppchashdatas]);

            SCIP_CALL(
                  SCIPduplicateBlockMemoryArray(scip, &(presoldata->setppchashdatas[presoldata->nsetppchashdatas]->vars), SCIPgetVarsSetppc(scip, setppcs[c]), 2));
            SCIP_CALL(
                  SCIPgetBinvarRepresentative(scip, presoldata->setppchashdatas[presoldata->nsetppchashdatas]->vars[0],
                        &(presoldata->setppchashdatas[presoldata->nsetppchashdatas]->vars[0]), &(negated[0])));
            SCIP_CALL(
                  SCIPgetBinvarRepresentative(scip, presoldata->setppchashdatas[presoldata->nsetppchashdatas]->vars[1],
                        &(presoldata->setppchashdatas[presoldata->nsetppchashdatas]->vars[1]), &(negated[1])));

            if( SCIPvarGetStatus(presoldata->setppchashdatas[presoldata->nsetppchashdatas]->vars[0]) == SCIP_VARSTATUS_FIXED
                  || SCIPvarGetStatus(presoldata->setppchashdatas[presoldata->nsetppchashdatas]->vars[0])
                        == SCIP_VARSTATUS_MULTAGGR
                  || SCIPvarGetStatus(presoldata->setppchashdatas[presoldata->nsetppchashdatas]->vars[1])
            ...
            /* order the variables after their index */
            if( SCIPvarGetIndex(presoldata->setppchashdatas[presoldata->nsetppchashdatas]->vars[0])
                  > SCIPvarGetIndex(presoldata->setppchashdatas[presoldata->nsetppchashdatas]->vars[1]) )
            {
               ...
               presoldata->setppchashdatas[presoldata->nsetppchashdatas]->vars[0] =
                     presoldata->setppchashdatas[presoldata->nsetppchashdatas]->vars[1];
            ...
            SCIP_CALL(
                  SCIPhashtableInsert(presoldata->hashdatatable,
                        (void* ) presoldata->setppchashdatas[presoldata->nsetppchashdatas]));
   ...
   {
      SCIP_CALL(
            SCIPhashtableRemove(presoldata->logicorhashtable,
                  (void* ) presoldata->usefullogicor[presoldata->nusefullogicor - 1]));
      ...
      if( !SCIPconsIsModifiable(logicors[c]) && SCIPgetNVarsLogicor(scip, logicors[c]) >= 3 )
      {
         ...
         if( !SCIPhashtableExists(presoldata->logicorhashtable, (void*) logicors[c]) )
         {
            ...
            if( presoldata->nusefullogicor == presoldata->susefullogicor )
            {
               ...
               SCIP_CALL(
                     SCIPreallocBlockMemoryArray(scip, &(presoldata->usefullogicor), presoldata->susefullogicor, newsize));
   ...
   )
{
   ...
   if( ngateconss == nlogicorvars || (ngateconss >= nlogicorvars - 1 && !presoldata->onlysetpart) )
   {
      ...
      if( ngateconss == nlogicorvars - 1 )
      {
         ...
         SCIP_CALL(
               SCIPcreateConsAnd(scip, &newcons, name, posresultants[d], ngateconss, consvars, initial, separate, enforce,
      ...
      else
      {
         ...
         SCIP_CALL(
               SCIPcreateConsSetpart(scip, &newcons, name, nlogicorvars, activevars, initial, separate, enforce, check,
...
{  /*lint --e{715}*/
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   if( conshdlrand == NULL )
   {
      ...
      {
         /* make sure that we correct the parameter for only extracting set-partitioning constraints */
   ...
   if( conshdlrand != NULL && SCIPgetBoolParam(scip, "constraints/and/linearize", &paramvalue) == SCIP_OKAY )
   {
      if( paramvalue )
      {
         SCIPwarningMessage(scip, "Gate-presolving is the 'counterpart' of linearizing all and-constraints,"
               " so enabling both presolving steps at once does not make sense.\n");
      }
   }
   *result = SCIP_DIDNOTFIND;
   ...
   /* move the biggate extraction to front or back by sorting the logicors after number of variables */
   ...
   if( presoldata->searchequations && !SCIPisStopped(scip) )
   {
      ...
      SCIP_CALL( SCIPhashtableCreate(&setppchashdatatable, SCIPblkmem(scip), nsetppcconss, SCIPhashGetKeyStandard,
            setppcHashdataKeyEqCons, setppcHashdataKeyValCons, (void*) scip) );
      ...
      for (d = nsetppcconss - 1; d >= 0; --d)
      {
         ...
         SCIP_CALL(
               SCIPduplicateBlockMemoryArray(scip, &(setppchashdatas[nsetppchashdatas]->vars), activevarssetppc, nsetppcvars));
         ...
         if( v == -1 )
         {
            ...
            if( hashmaphashdata != NULL && !SCIPconsIsDeleted(hashmaphashdata->cons) )
            {
               ...
               else
               {
                  ...
                  (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "setpart_%d", presoldata->ngates);
                  SCIP_CALL(
                        SCIPcreateConsSetpart(scip, &newcons, name, nlogicorvars, activevarslogicor, initial, separate,
   ...
   if( presoldata->nsetppchashdatas == 0
         || (presoldata->firstchangedlogicor == presoldata->nusefullogicor && !presoldata->newsetppchashdatas) )
   {
   ...
   if( presoldata->nsetppchashdatas > 0 && !SCIPisStopped(scip) )
   {
      ...
      for (c = presoldata->nusefullogicor - 1; c >= endloop && !SCIPisStopped(scip); --c)
      {
          ...
          */
         SCIP_CALL(
               extractGates(scip, presoldata, c, varmap, gateconss, activevars, posresultants, hashdata, ndelconss,
                     naddconss));
      }
      SCIPfreeBuffer(scip, &hashdata);
