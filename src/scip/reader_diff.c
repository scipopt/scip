...
static
SCIP_DECL_READERWRITE(readerWriteDiff)
{  /*lint --e{715}*/
   assert(reader != NULL);
   assert(strcmp(SCIPreaderGetName(reader), READER_NAME) == 0);

   SCIP_CALL( SCIPwriteDiff(scip, file, name, transformed, objsense, objscale, objoffset, vars,
   ...
   )
{
   ...
   SCIP_CALL( SCIPsetReaderWrite(scip, reader, readerWriteDiff) );
