...
public:
   ...
   void EndInput()
   {
      ...
      if( objexpr != NULL )
      {
         ...
         SCIP_CALL_THROW( SCIPaddLinearTermNonlinear(scip, objcons, -1.0, objvar) );
      ...
      for( size_t i = 0; i < nlconslin.size(); ++i )
      {
         ...
         {
            SCIP_CALL_THROW( SCIPaddLinearTermNonlinear(scip, probdata->conss[i], nlconslin[i][j].first, nlconslin[i][j].second) );
         }
      }
