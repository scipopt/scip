...
/*                                                                           */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
#pragma ident "@(#) $Id: cons_sos2.h,v 1.4 2008/04/17 17:49:05 bzfpfets Exp $"

/**@file   cons_sos2.h
 * @brief  constraint handler for SOS type 2 constraints
 * @author Marc Pfetsch
 *
 * A specially ordered set of type 2 (SOS2) is a sequence of variables
 * such that at most two variables are nonzero and if two variables
 * are nonzero they must be adjacent in the specified sequence. Note
 * that it is in principle allowed that a variables appears twice, but
 * it then can be fixed to 0 if it is at least two appart in the
 * squence.
 *
 * This constraint is usefull when considering a piecewise affine
 * approximation of a univariate (nonlinear) function \f$: [a,b]
 * \rightarrow R\f$: Let \f$x_1 < \ldots < x_n\f$ be points in
 * \f$[a,b]\f$ and introduce variables \f$\lambda_1, \ldots,
 * \lambda_n\f$. To evaluate \f$f(x')\f$ at some point \f$x' \in
 * [a,b]\f$ one can use the following constraints:
 * \f[
 ...
 *
 * - If the constraint is created with SCIPcreateConsSOS2() and
 *   weights are given, the weights determine the order (decreasing
 *   weights). Additional variables can be added with SCIPaddVarSOS2(),
 *   which adds a variable with given weight.
 *
 * - If an empty constraint is created and then variables are added
 *   with SCIPaddVarSOS2(), weights are needed and stored.
 *
