...
#include "scip/sepa_liftedflowcover.h"
#include "scip/cons_knapsack.h"
#include "scip/pub_misc.h"


#define SEPA_NAME              "liftedflowcover"
#define SEPA_DESC              "flow cover cuts separator (Gu's approach)"
...
#define DEFAULT_MAXROUNDSROOT       150 /**< maximal number of separation rounds in the root node (-1: unlimited) */
...
#define DEFAULT_MAXFAILS           0.05 /**< maximal number of consecutive fails to generate a cut per separation round
                                        *   (-1: unlimited) */
#define DEFAULT_MAXFAILSROOT        0.1 /**< maximal number of consecutive fails to generate a cut per separation round
                                         *   in the root (-1: unlimited) */
#define DEFAULT_MAXSEPACUTS         100 /**< maximal number of flow cover cuts separated per separation round */
#define DEFAULT_MAXSEPACUTSROOT     500 /**< maximal number of flow cover cuts separated per separation round in the root */
...
#define DEFAULT_GENCMIRFCI        FALSE /**< should c-MIRFCIs be generated additionally? */
...
struct SCIP_SepaData
{
   ...
   int                   maxtriesroot;       /**< maximal number of rows to separate flow cover cuts for per separation round
                                              *   in the root (-1: unlimited) */
   SCIP_Real             maxfails;           /**< maximal number of consecutive fails to generate a cut per separation round
                                              *   (-1: unlimited) */
   SCIP_Real             maxfailsroot;       /**< maximal number of consecutive fails to generate a cut per separation round
                                              *   in the root (-1: unlimited) */
   ...
   SCIP_Bool             gencmirfci;         /**< should c-MIRFCIs be generated additionally? */
...
struct DenseRow
{
   ...
   SCIP_Real             rowrhs;
   int*                  nonzcols;
   SCIP_Bool*            nonzcolisbin;
   int                   nnonzcols;
} DENSEROW;

typedef enum {
   SLACKTYPE_NONE,
   SLACKTYPE_RHSMINUSROW,
   SLACKTYPE_ROWMINUSLHS
} SLACKTYPE;
...
struct SNF_Relaxation
{
   ...
   int*                  assoctransvars;     /**< assosicated var in relaxed set for all vars of row */
   int*                  boundsfortrans;     /**< bound used for all non-binary vars of row */
   SCIP_BOUNDTYPE*       boundtypesfortrans; /**< type of bound used for all non-binary vars of row */
   SLACKTYPE             slack;              /**< type of slack variable that was added */
} SNF_RELAXATION;
...
SCIP_RETCODE getClosestVlb(
   ...
   SCIP_Real             scale,              /** scale for binary coefficients */
   ...
   *closestvlb = -SCIPinfinity(scip);
   if( nvlbs > 0 )
   {
      ...
      for( i = 0; i < nvlbs; i++ )
      {
         ...
         rowcoefbinvar = scale * rowcoefsbinary[probidxbinvar];
...
SCIP_RETCODE constructSNFRelaxation(
   ...
   DENSEROW*             denserow,           /**< given row */
   SCIP_Real             scale,              /**< additional scaling factor for given row */
   SNF_RELAXATION*       snf,                /**< pointer to snf relaxation to be constructed */
   ...
   )
{
   ...
   *success = FALSE;
   ...
   for( c = 0; c < nvars; c++ )
   {
      snf->assoctransvars[c] = -1;
      snf->boundsfortrans[c] = -3;
   }
   snf->ntransvars = 0;

   /* initialize right hand side and slacktype of constraint in 0-1 single node flow relaxation */
   if( scale == 1.0 && !SCIPisInfinity(scip, denserow->rowrhs) )
   {
      snf->transrhs = denserow->rowrhs;
      snf->slack = SLACKTYPE_NONE;
   }
   else if( scale == 1.0 && SCIPisInfinity(scip, denserow->rowrhs) )
   {
      snf->transrhs = denserow->rowlhs;
      snf->slack = SLACKTYPE_ROWMINUSLHS;
   }
   else if( scale == -1.0 && !SCIPisInfinity(scip, -denserow->rowlhs) )
   {
      snf->transrhs = -denserow->rowlhs;
      snf->slack = SLACKTYPE_NONE;
   }
   ...
   {
      assert(scale == -1.0 && SCIPisInfinity(scip, -denserow->rowlhs));
      snf->transrhs = -denserow->rowrhs;
      snf->slack = SLACKTYPE_RHSMINUSROW;
   }
    ...
    */
   SCIPdebugMsg(scip, "transformation for NONBINARY variables:\n");

   /* non-binary variables and binary variables contained in used variable bounds */
   for( c = 0; c < denserow->nnonzcols; c++ )
   {
      SCIP_VAR* var;
      SCIP_Real bestlb;
      SCIP_Real bestub;
      ...
      probidx = denserow->nonzcols[c];
      var = vars[probidx];
      rowcoef = scale * denserow->rowcoefs[probidx];

      assert(snf->assoctransvars[probidx] == -1);
      assert(snf->boundsfortrans[probidx] == -3);
      assert(!SCIPisZero(scip, rowcoef));
      ...
      if( !SCIPisInfinity(scip, bestsub) )
      {
         ...
         if( SCIPvarGetType(var) == SCIP_VARTYPE_CONTINUOUS )
         {
            ...
            SCIP_CALL( getClosestVlb(scip, var, bestsub, rowcoef, denserow->rowcoefs, scale, varsolvals, snf->assoctransvars, &bestvlb, &bestvlbidx) );
      ...
      if( !SCIPisInfinity(scip, -bestslb) )
      {
         ...
         if( SCIPvarGetType(var) == SCIP_VARTYPE_CONTINUOUS )
         {
            ...
            SCIP_CALL( getClosestVub(scip, var, bestslb, rowcoef, denserow->rowcoefs, scale, varsolvals, snf->assoctransvars, &bestvub, &bestvubidx) );
      ...
      if( uselb )
      {
          ...
          * in the relaxed set
          */
         snf->assoctransvars[probidx] = snf->ntransvars;
         snf->boundsfortrans[probidx] = bestlbtype;
         snf->boundtypesfortrans[probidx] = SCIP_BOUNDTYPE_LOWER;
         ...
         if( bestlbtype < 0 )
         {
            ...
            if( SCIPisPositive(scip, rowcoef) )
            {
               ...
               snf->aggrconstants[snf->ntransvars] = (rowcoef * bestsub);
            ...
            SCIPdebugMsg(scip, "    --> bestlb used for trans: ... %s y'_%d + ..., y'_%d <= %g x_%d (=1), rhs=%g-(%g*%g)=%g\n",
               ...
               snf->ntransvars, snf->transrhs + (rowcoef * bestsub), rowcoef, bestsub, snf->transrhs);
         }
         else
         {
            ...
            rowcoefbinary = scale * denserow->rowcoefs[SCIPvarGetProbindex(vlbvars[bestlbtype])];
            ...
            /* store aggregation information for y'_j for transforming cuts for the SNF relaxation back to the problem variables later */
            snf->origbinvars[snf->ntransvars] = SCIPvarGetProbindex(vlbvars[bestlbtype]);
            ...
            if( SCIPisPositive(scip, rowcoef) )
            {
               ...
               snf->aggrcoefscont[snf->ntransvars] = - rowcoef;
               snf->aggrconstants[snf->ntransvars] = (rowcoef * vlbconsts[bestlbtype]);
            }
            else
            {
               assert(SCIPisNegative(scip, rowcoef));
            ...
            snf->transrhs -= (rowcoef * vlbconsts[bestlbtype]);

            /* store for x_j that y'_j is the associated real variable in the 0-1 single node flow relaxation */
            snf->assoctransvars[snf->origbinvars[snf->ntransvars]] = snf->ntransvars;

            SCIPdebugMsg(scip, "    --> bestlb used for trans: ... %s y'_%d + ..., y'_%d <= %g x_%d (=%s), rhs=%g-(%g*%g)=%g\n",
               snf->transvarcoefs[snf->ntransvars] == 1 ? "+" : "-", snf->ntransvars, snf->ntransvars, snf->transvarvubcoefs[snf->ntransvars],
               snf->ntransvars, SCIPvarGetName(vlbvars[bestlbtype]), snf->transrhs + (rowcoef * vlbconsts[bestlbtype]), rowcoef,
               vlbconsts[bestlbtype], snf->transrhs );
         }
      }
      else
      {
          ...
          */
         snf->assoctransvars[probidx] = snf->ntransvars;
         snf->boundsfortrans[probidx] = bestubtype;
         snf->boundtypesfortrans[probidx] = SCIP_BOUNDTYPE_UPPER;
         snf->origcontvars[snf->ntransvars] = probidx;
         ...
         if( bestubtype < 0 )
         {
            ...
            if( SCIPisPositive(scip, rowcoef) )
            {
               ...
               /* aggregation information for y'_j */
               snf->aggrcoefscont[snf->ntransvars] = rowcoef;
               snf->aggrconstants[snf->ntransvars] = - (rowcoef * bestslb);
            }
            else
            {
               assert(SCIPisNegative(scip, rowcoef));
            ...
            snf->transrhs -= (rowcoef * bestslb);

            SCIPdebugMsg(scip, "    --> bestub used for trans: ... %s y'_%d + ..., Y'_%d <= %g x_%d (=1), rhs=%g-(%g*%g)=%g\n",
               snf->transvarcoefs[snf->ntransvars] == 1 ? "+" : "-", snf->ntransvars, snf->ntransvars, snf->transvarvubcoefs[snf->ntransvars],
               snf->ntransvars, snf->transrhs + (rowcoef * bestslb), rowcoef, bestslb, snf->transrhs);
         }
         else
         {
            SCIP_Real rowcoefbinary;
            ...
            rowcoefbinary = scale * denserow->rowcoefs[SCIPvarGetProbindex(vubvars[bestubtype])];
            ...
            /* store aggregation information for y'_j for transforming cuts for the SNF relaxation back to the problem variables later */
            snf->origbinvars[snf->ntransvars] = SCIPvarGetProbindex(vubvars[bestubtype]);
            ...
            if( SCIPisPositive(scip, rowcoef) )
            {
               ...
               snf->aggrcoefsbin[snf->ntransvars] = rowcoefbinary;
               snf->aggrcoefscont[snf->ntransvars] = rowcoef;
               snf->aggrconstants[snf->ntransvars] = - (rowcoef * vubconsts[bestubtype]);
            }
            else
            {
               assert(SCIPisNegative(scip, rowcoef));
            ...
            snf->transrhs -= (rowcoef * vubconsts[bestubtype]);
            ...
            SCIPdebugMsg(scip, "    --> bestub used for trans: ... %s y'_%d + ..., y'_%d <= %g x_%d (=%s), rhs=%g-(%g*%g)=%g\n",
               snf->transvarcoefs[snf->ntransvars] == 1 ? "+" : "-", snf->ntransvars, snf->ntransvars, snf->transvarvubcoefs[snf->ntransvars],
               snf->ntransvars, SCIPvarGetName(vubvars[bestubtype]), snf->transrhs + (rowcoef * vubconsts[bestubtype]), rowcoef,
               vubconsts[bestubtype], snf->transrhs);
         }
      }
       ...
       */
      if( SCIPisInfinity(scip, snf->transvarvubcoefs[snf->ntransvars]) )
      {
         assert(!(*success));
         return SCIP_OKAY;
      }
      ...
      assert(snf->boundsfortrans[probidx] > -3);
      assert(snf->assoctransvars[probidx] >= 0 && snf->assoctransvars[probidx] == snf->ntransvars);
   ...
   SCIPdebugMsg(scip, "transformation for BINARY variables:\n");
   ...
   for( c = 0; c < denserow->nnonzcols; c++ )
   {
      ...
      if( snf->assoctransvars[probidx] > -1 )
      {
         assert(snf->assoctransvars[probidx] >= 0 && snf->assoctransvars[probidx] <= denserow->nnonzcols);
         assert(snf->boundsfortrans[probidx] == -3);
      ...
      if( SCIPisPositive(scip, rowcoef) )
      {
         snf->transvarcoefs[snf->ntransvars] = 1;
         snf->transvarvubcoefs[snf->ntransvars] = val;
         snf->transbinvarsolvals[snf->ntransvars] = varsolvals[probidx];
         snf->transcontvarsolvals[snf->ntransvars] = contsolval;
   ...
   assert(snf->ntransvars >= 0 && snf->ntransvars <= denserow->nnonzcols);
   ...
   *success = TRUE;
   ...
   for( c = 0; c < snf->ntransvars; c++ )
   {
      SCIPdebugMsgPrint(scip, "%s y'_%d ", snf->transvarcoefs[c] == 1 ? "+" : "-", c);
   }
...
{
   SCIP_CALL( SCIPallocBufferArray(scip, &snf->assoctransvars, nvars) );
   SCIP_CALL( SCIPallocBufferArray(scip, &snf->boundsfortrans, nvars) );
   SCIP_CALL( SCIPallocBufferArray(scip, &snf->boundtypesfortrans, nvars) );
   ...
   )
{
   ...
   SCIPfreeBufferArray(scip, &snf->boundtypesfortrans);
   SCIPfreeBufferArray(scip, &snf->boundsfortrans);
   SCIPfreeBufferArray(scip, &snf->assoctransvars);
}

static
SCIP_RETCODE allocDenseRow(
   SCIP*                 scip,               /**< SCIP data structure */
   DENSEROW*             denserow,           /**< pointer to snf relaxation to be destroyed */
   int                   nvars               /**< number of active problem variables */
   )
{
   SCIP_CALL( SCIPallocBufferArray(scip, &denserow->rowcoefs, nvars) );
   SCIP_CALL( SCIPallocBufferArray(scip, &denserow->nonzcols, nvars) );
   SCIP_CALL( SCIPallocBufferArray(scip, &denserow->nonzcolisbin, nvars) );

   return SCIP_OKAY;
}

static
void destroyDenseRow(
   SCIP*                 scip,               /**< SCIP data structure */
   DENSEROW*             denserow            /**< pointer to snf relaxation to be destroyed */
   )
{
   SCIPfreeBufferArray(scip, &denserow->nonzcolisbin);
   SCIPfreeBufferArray(scip, &denserow->nonzcols);
   SCIPfreeBufferArray(scip, &denserow->rowcoefs);
}

static
SCIP_RETCODE rowToDenseRow(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_VAR**            vars,               /**< active problem variables */
   int                   nvars,              /**< number of active problem variables */
   SCIP_ROW*             row,                /**< given row */
   DENSEROW*             denserow            /**< pointer to store dense data of given row */
   )
{
   SCIP_COL** nonzcols;
   SCIP_Real* nonzcoefs;
   ...
   /* get data structures */
   BMSclearMemoryArray(denserow->rowcoefs, nvars);
   denserow->nnonzcols = 0;
   for( c = 0; c < nnonzcols; c++ )
   {
      ...
      assert(probidx > -1 &&  probidx < nvars);

      denserow->rowcoefs[probidx] = nonzcoefs[c];
      denserow->nonzcols[denserow->nnonzcols] = probidx;
      denserow->nonzcolisbin[denserow->nnonzcols] = SCIPvarIsBinary(var);
      ++denserow->nnonzcols;
   }

   denserow->rowlhs = SCIProwGetLhs(row) - SCIProwGetConstant(row);
   denserow->rowrhs = SCIProwGetRhs(row) - SCIProwGetConstant(row);
...
void getL1L2(
   SCIP*                 scip,               /**< SCIP data structure */
   SNF_RELAXATION*       snf,                /**< snf relaxation */
   ...
   )
{
   ...
   for( j = 0; j < snf->ntransvars; j++ )
   {
      ...
      assert(snf->transvarcoefs[j] == 1 || snf->transvarcoefs[j] == -1);
      assert(SCIPisGE(scip, snf->transvarvubcoefs[j], 0.0));
      assert(SCIPisFeasGE(scip, snf->transbinvarsolvals[j], 0.0) && SCIPisFeasLE(scip, snf->transbinvarsolvals[j], 1.0));

      /* j in N1\C1 */
      if( snf->transvarcoefs[j] == 1 && transvarflowcoverstatus[j] == -1 )
      {
          ...
          */
         d = snf->transvarvubcoefs[j] / delta;
         ...
         if( SCIPisFeasGE(scip, snf->transcontvarsolvals[j], ( snf->transvarvubcoefs[j] - ( lambda * mirval ) ) * snf->transbinvarsolvals[j]) )
            transvarflowcoverstatus[j] = 2;

         SCIPdebugMsg(scip, "       <%d>: in N1-C1: %g ?>=?  ( %g - %g F_{f_beta}(%g)(%g) ) %g = %g  ---> fcstatus = %d\n",
            j, snf->transcontvarsolvals[j], snf->transvarvubcoefs[j], lambda, d, mirval, snf->transbinvarsolvals[j],
            ( snf->transvarvubcoefs[j] - ( lambda * mirval ) ) * snf->transbinvarsolvals[j], transvarflowcoverstatus[j]);
      }
      ...
      if( snf->transvarcoefs[j] == -1 && transvarflowcoverstatus[j] == -1 )
      {
         ...
         if( SCIPisFeasGE(scip, snf->transcontvarsolvals[j], - ( lambda * mirval ) * snf->transbinvarsolvals[j]) )
            transvarflowcoverstatus[j] = 2;

         SCIPdebugMsg(scip, "       <%d>: in N2-C2: %g ?>=?  - %g F_{f_beta}(-%g)(%g) %g = %g  ---> fcstatus = %d\n",
            j, snf->transcontvarsolvals[j], lambda, d, mirval, snf->transbinvarsolvals[j],
            - ( lambda * mirval ) * snf->transbinvarsolvals[j], transvarflowcoverstatus[j]);
      }
   }
}
...
SCIP_RETCODE getBoundsForSubstitution(
   ...
   SCIP_VAR**            vars,               /**< active problem variables */
   int                   nvars,              /**< number of active problem variables */
   SNF_RELAXATION*       snf,                /**< the single node flow relaxation of the row */
   ...
   )
{
   ...
   assert(vars != NULL);
   assert(nvars >= 0);
   assert(snf->boundsfortrans != NULL);
   assert(snf->boundtypesfortrans != NULL);
   assert(snf->assoctransvars != NULL);
   assert(snf->transvarcoefs != NULL);
   assert(flowcoverstatus != NULL);
   assert(snf->ntransvars >= 0 && snf->ntransvars <= nvars);
   ...
   for( j = 0; j < nvars; j++ )
   {
      ...
      assert(snf->assoctransvars[j] == -1 || ( flowcoverstatus[snf->assoctransvars[j]] == -1
            || flowcoverstatus[snf->assoctransvars[j]] == 1 || flowcoverstatus[snf->assoctransvars[j]] == 2 ));

      /* variable has no associated variable in transformed problem */
      if( snf->assoctransvars[j] == -1 )
      {
         assert(snf->boundsfortrans[j] == -3);
         boundsforsubst[j] = -3;
         continue;
      }
      ...
      {
         /* j in C1 & C2 */
         if( flowcoverstatus[snf->assoctransvars[j]] == 1 )
         {
            boundsforsubst[j] = -1;
      ...
      else
      {
         ...
         if( flowcoverstatus[snf->assoctransvars[j]] >= 1 )
         {
            boundsforsubst[j] = snf->boundsfortrans[j];
            boundtypesforsubst[j] = snf->boundtypesfortrans[j];
         }
         /* j in N1\C1 & N2\(C2 & L2) */
         else
         {
            if( snf->boundtypesfortrans[j] == SCIP_BOUNDTYPE_UPPER )
            {
               SCIP_Real closestlb;
               int closestlbtype;
...
SCIP_RETCODE cutGenerationHeuristic(
   ...
   SCIP_SOL*             sol,                /**< the solution that should be separated, or NULL for LP solution */
   SCIP_Real*            varsolvals,         /**< solution values of active variables */
   SCIP_Real*            rowweights,         /**< array to store weight of rows in aggregated row */
   int                   rowind,             /**< index of row to set weight for */
   SCIP_Real             scalar,             /**< additional scaling factor of rows in aggregation */
   SNF_RELAXATION*       snf,                /**< single node flow relaxation */
   int*                  transvarflowcoverstatus, /**< pointer to store whether non-binary var is in L2 (2) or not (-1 or 1) */
   SCIP_Real             lambda,             /**< lambda */
   char                  normtype,           /**< type of norm to use for efficacy norm calculation */
   SCIP_Bool*            cutoff,             /**< whether a cutoff has been detected */
   ...
   )
{
   ...
   /* get data structures */
   SCIP_CALL( SCIPallocBufferArray(scip, &testeddeltas, snf->ntransvars + 4) );
   SCIP_CALL( SCIPallocBufferArray(scip, &candsetdelta, snf->ntransvars + 4) );
   SCIP_CALL( SCIPallocBufferArray(scip, &transvarflowcoverstatustmp, snf->ntransvars) );
   ...
   for( j = 0; j < snf->ntransvars; j++ )
   {
      ...
      if( snf->transvarcoefs[j] == 1 && transvarflowcoverstatus[j] == 1
         && SCIPisFeasGT(scip, snf->transvarvubcoefs[j], lambda) )
         c1vubcoefsmax = MAX(c1vubcoefsmax, snf->transvarvubcoefs[j]);

      /* j is in L~2 and u_j > lambda */
      val = MIN(snf->transvarvubcoefs[j], lambda) * snf->transbinvarsolvals[j];
      if( snf->transvarcoefs[j] == -1 && transvarflowcoverstatus[j] == -1 && SCIPisFeasLE(scip, val, snf->transcontvarsolvals[j])
         && SCIPisFeasGT(scip, snf->transvarvubcoefs[j], lambda) )
         l2tmpvubcoefsmax = MAX(l2tmpvubcoefsmax, snf->transvarvubcoefs[j]);

      /* u_j + 1 > lambda */
      if( SCIPisFeasGT(scip, snf->transvarvubcoefs[j] + 1.0, lambda) )
         nvubcoefsmax = MAX(nvubcoefsmax, snf->transvarvubcoefs[j]);

      /* u_j > lambda */
      if( SCIPisFeasGT(scip, snf->transvarvubcoefs[j], lambda) )
      {
         candsetdelta[startidx + ncandsetdelta] = snf->transvarvubcoefs[j];
         ncandsetdelta++;
         SCIPdebugMsg(scip, "     u_j              = %g\n", snf->transvarvubcoefs[j]);
      }
   }
   ...
   assert(ncandsetdelta >= 1 && ncandsetdelta <= snf->ntransvars + 4);
   ...
   for( j = startidx; j < startidx + ncandsetdelta && ntesteddeltas < sepadata->maxtestdelta; j++ )
   {
       ...
       */
      if( SCIPisZero(scip, onedivdelta) )
         continue;
       ...
       */
      BMScopyMemoryArray(transvarflowcoverstatustmp, transvarflowcoverstatus, snf->ntransvars);

      /* get L1 subset of N1\C1 and L2 subset of N2\C2 by comparison */
      getL1L2(scip, snf, transvarflowcoverstatustmp, delta, lambda);
       ...
       */
      SCIP_CALL( getBoundsForSubstitution(scip, vars, nvars, snf, transvarflowcoverstatustmp, boundsforsubst, boundtypesforsubst ) );
      ...
      SCIP_CALL( SCIPcalcMIR(scip, sol, BOUNDSWITCH, TRUE, ALLOWLOCAL, FIXINTEGRALRHS, boundsforsubst,
            boundtypesforsubst, (int) MAXAGGRLEN(nvars), 1.0, MINFRAC, MAXFRAC, rowweights, -1.0, &rowind, 1, -1, NULL,
            onedivdelta, NULL, NULL, cutcoefs, &cutrhs, &cutact, &success, &cutislocal, NULL) );
   ...
   if( SCIPisEfficacious(scip, bestefficacy) )
   {
      ...
      getL1L2(scip, snf, transvarflowcoverstatus, bestdelta, lambda);

      /* for best value of delta: get bounds for substitution in c-MIR routine for original mixed integer set
       * note that the scalar has already been considered in the constructed 0-1 single node flow relaxation
       */
      SCIP_CALL( getBoundsForSubstitution(scip, vars, nvars, snf, transvarflowcoverstatus, boundsforsubst, boundtypesforsubst ) );

      /* generate c-MIRFCI for flow cover (C1,C2), L1 subset N1\C1 and L2 subset N2\C2 and bestdelta */
      SCIP_CALL( SCIPcalcMIR(scip, sol, BOUNDSWITCH, TRUE, ALLOWLOCAL, FIXINTEGRALRHS, boundsforsubst,
            boundtypesforsubst, (int) MAXAGGRLEN(nvars), 1.0, MINFRAC, MAXFRAC, rowweights, -1.0, &rowind, 1, -1, NULL,
            onedivbestdelta, NULL, NULL, cutcoefs, &cutrhs, &cutact, &success, &cutislocal, &cutrank) );
      assert(ALLOWLOCAL || !cutislocal);
      assert(success);
   ...
   )
{
   ...
   *valid = TRUE;
   ...
   if( SCIPisGE(scip, sumC1LE + sumN2mC2LE, lambda) || liftingdata->t == liftingdata->r )
      liftingdata->exact = TRUE;
   else if( SCIPisGT(scip, liftingdata->m[liftingdata->t] - (liftingdata->mp - lambda) - sumC1LE - sumN2mC2LE, 0) )
      liftingdata->exact = TRUE;
...
SCIP_RETCODE generateLiftedFlowCoverCut(
   ...
   int                   nvars,              /**< number of active problem variables */
   SNF_RELAXATION*       snf,                /**< pointer to SNF relaxation */
   DENSEROW*             denserow,           /**< row used to construct SNF relaxation */
   int*                  flowcoverstatus,    /**< pointer to store whether variable is in flow cover (+1) or not (-1) */
   SCIP_Real             lambda,             /**< lambda */
   SCIP_Real*            cutcoefs,           /**< whether a cutoff has been detected */
   SCIP_Real*            cutrhs,             /**< pointer to count the number of generated cuts */
   SCIP_Bool*            success,            /**< was the cut successfully generated */
   SCIP_Bool*            exact               /**< was the lifting exact */
   )
{
   LIFTINGDATA liftingdata;
   int i;
      ...
      switch(s)
      {
         ...
         case 2: /* var is in N1 \ C1 */
         {
            ...
            if( alpha == 1 )
            {
               ...
               if( snf->origbinvars[i] != -1 )
                  cutcoefs[snf->origbinvars[i]] += binvarcoef;
               else
                  *cutrhs -= binvarcoef;
   ...
   *exact = liftingdata.exact;

   destroyLiftingData(scip, &liftingdata);

   /* handle the slack variable if present */
   if( snf->slack == SLACKTYPE_RHSMINUSROW )
   {
      *cutrhs += denserow->rowrhs;
      for( i = 0; i < denserow->nnonzcols; ++i )
      {
         int probidx = denserow->nonzcols[i];
         cutcoefs[probidx] += denserow->rowcoefs[probidx];
      }
   }
   else if( snf->slack == SLACKTYPE_ROWMINUSLHS )
   {
      *cutrhs -= denserow->rowlhs;
      for( i = 0; i < denserow->nnonzcols; ++i )
      {
         int probidx = denserow->nonzcols[i];
         cutcoefs[probidx] -= denserow->rowcoefs[probidx];
...
SCIP_RETCODE SCIPcalcFlowCoverCut(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_Real*            varsolvals,         /**< the solution that should be separated, or NULL for LP solution */
   SCIP_Real*            rowcoefs,
   SCIP_Real             rowrhs,
   SCIP_Real*            cutcoef,            /**< array to store MIR coefficients: must be of size SCIPgetNVars() */
   SCIP_Real*            cutrhs,             /**< pointer to store the right hand side of the flow cover cut */
   ...
   )
{
   ...
   *success = FALSE;
   ...
   denserow.rowcoefs = rowcoefs;
   denserow.rowrhs = rowrhs;
   denserow.rowlhs = -SCIPinfinity(scip);
   ...
   for( i = 0; i < nvars; ++i )
   {
      if( SCIPisZero(scip, rowcoefs[i]) )
         continue;

      denserow.nonzcols[denserow.nnonzcols] = i;
      denserow.nonzcolisbin[denserow.nnonzcols] = SCIPvarIsBinary(vars[i]);
      denserow.nnonzcols++;
   }

   SCIP_CALL( constructSNFRelaxation(scip, vars, nvars, varsolvals, &denserow, 1.0, &snf, success) );
   ...
   SCIP_CALL( generateLiftedFlowCoverCut(scip, nvars, &snf, &denserow, transvarflowcoverstatus, lambda, cutcoef, cutrhs, success, &exact) );
   ...
   )
{
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   SCIP_CALL( SCIPallocBufferArray(scip, &cutcoefs, nvars) );
   SCIP_CALL( allocSNFRelaxation(scip, &snfrelax, nvars) );
   SCIP_CALL( allocDenseRow(scip, &denserow, nvars) );
   ...
   if( depth == 0 )
   {
      ...
      maxfails = nrows * sepadata->maxfailsroot;
   ...
   else
   {
      ...
      maxfails = nrows * sepadata->maxfails;
           ...
           && !SCIPisStopped(scip); r++ )
   {
      ...
      SCIPdebugMsg(scip, "rowact=%g is closer to %s --> rowweight=%g\n", rowact, mult == 1.0 ? "rhs" : "lhs", mult);

      rowToDenseRow(scip, vars, nvars, rows[roworder[r]], &denserow);
      ...
      for( k = 0; k < 2; ++k )
      {
         ...
         SCIP_CALL( constructSNFRelaxation(scip, vars, nvars, varsolvals, &denserow, mult, &snfrelax, &transsuccess) );
         if( !transsuccess )
         {
         ...
         SCIP_CALL( generateLiftedFlowCoverCut(scip, nvars, &snfrelax, &denserow, transvarflowcoverstatus, lambda, cutcoefs, &cutrhs, &success, &exact) );

         if( success )
         {
            SCIP_CALL( addCut(scip, sepa, sepadata, vars, nvars, sol, varsolvals, cutcoefs, cutrhs, ALLOWLOCAL, SCIProwGetRank(rows[roworder[r]])+1, normtype, &cutoff, &ncuts) );
         }

         if( sepadata->gencmirfci )
         {
            SCIP_CALL( cutGenerationHeuristic(scip, sepa, sepadata, vars, nvars, sol, varsolvals, rowweights, roworder[r], mult, &snfrelax, transvarflowcoverstatus, lambda, normtype, &cutoff, &ncuts) );
         }

         mult *= -1.0;
         wastried = TRUE;

         if ( cutoff || !sepadata->multbyminusone )
            break;
      }
   ...
   /* free data structures */
   destroyDenseRow(scip, &denserow);
   destroySNFRelaxation(scip, &snfrelax);
   SCIPfreeBufferArray(scip, &cutcoefs);
...
{  /*lint --e{715}*/
   ...
   SCIP_CALL( SCIPincludeSepaLiftedFlowcover(scip) );
...
SCIP_RETCODE SCIPincludeSepaLiftedFlowcover(
   SCIP*                 scip                /**< SCIP data structure */
   )
{
   SCIP_SEPADATA* sepadata;
   ...
   SCIP_CALL( SCIPaddBoolParam(scip,
         "separating/"SEPA_NAME"/multbyminusone",
   ...
   SCIP_CALL( SCIPaddBoolParam(scip,
         "separating/"SEPA_NAME"/gencmirfci",
         "should c-MIRFCIs be generated additionally?",
         &sepadata->gencmirfci, TRUE, DEFAULT_GENCMIRFCI, NULL, NULL) );
   SCIP_CALL( SCIPaddIntParam(scip,
