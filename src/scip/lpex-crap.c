...
static SCIP_DECL_HASHKEYVAL(hashValExCol)
{
   /*lint --e{715}*/
   SCIP_COL *col;
   int i;
...
static SCIP_DECL_HASHGETKEY(hashKeyExCol)
{
   /*lint --e{715}*/
   SCIP_COLEX *col;
...
static SCIP_DECL_HASHKEYEQ(hashEqExCol)
{
   /*lint --e{715}*/
   SCIP_COL *col1;
   SCIP_COL *col2;
...
static SCIP_DECL_HASHKEYVAL(hashValExRow)
{
   /*lint --e{715}*/
   SCIP_ROW *row;
   int i;
...
static SCIP_DECL_HASHGETKEY(hashKeyExRow)
{
   /*lint --e{715}*/
   SCIP_ROWEX *row;
...
static SCIP_DECL_HASHKEYEQ(hashEqExRow)
{
   /*lint --e{715}*/
   SCIP_ROW *row1;
   SCIP_ROW *row2;
...
SCIP_DECL_SORTPTRCOMP(SCIProwexComp)
{
   ...
   {
      assert(SCIProwexGetIndex(((SCIP_ROWEX *)elem1)) == SCIProwexGetIndex(((SCIP_ROWEX *)elem2)));
      return 0;
   }
}
...
static SCIP_RETCODE ensureRowexsSize(
...
static SCIP_RETCODE colexEnsureSize(
...
static SCIP_RETCODE ensureColexsSize(
    SCIP_LPEX *lp, /**< current LP data */
    SCIP_SET *set, /**< global SCIP settings */
    int num        /**< minimum number of entries to store */
)
...
static SCIP_RETCODE ensureChgcolsSize(
    SCIP_LPEX *lp, /**< current LP data */
    SCIP_SET *set, /**< global SCIP settings */
    int num        /**< minimum number of entries to store */
)
...
static SCIP_RETCODE ensureLpiexcolsSize(
    SCIP_LPEX *lp, /**< current LP data */
    SCIP_SET *set, /**< global SCIP settings */
    int num        /**< minimum number of entries to store */
)
...
static SCIP_RETCODE ensureLpiexrowsSize(
    SCIP_LPEX *lp, /**< current LP data */
    SCIP_SET *set, /**< global SCIP settings */
    int num        /**< minimum number of entries to store */
)
...
static int colexSearchCoefPart(
...
static int rowexSearchCoefPart(
...
static SCIP_RETCODE rowexAddCoef(
    SCIP_ROWEX *row,             /**< LP row */
    BMS_BLKMEM *blkmem,          /**< block memory */
    SCIP_SET *set,               /**< global SCIP settings */
...
static SCIP_RETCODE insertColChgcols(
...
static SCIP_RETCODE colexAddCoef(
    SCIP_COLEX *col,             /**< LP column */
    BMS_BLKMEM *blkmem,          /**< block memory */
    SCIP_SET *set,               /**< global SCIP settings */
    SCIP_EVENTQUEUE *eventqueue, /**< event queue */
...
static SCIP_RETCODE colexDelCoefPos(
...
static SCIP_RETCODE colexChgCoefPos(
    SCIP_COLEX *col,   /**< LP column */
    SCIP_SET *set,     /**< global SCIP settings */
    SCIP_LPEX *lp,     /**< current LP data */
...
static SCIP_RETCODE rowexDelCoefPos(
...
static SCIP_RETCODE rowexChgCoefPos(
    SCIP_ROWEX *row,             /**< LP row */
    BMS_BLKMEM *blkmem,          /**< block memory */
    SCIP_SET *set,               /**< global SCIP settings */
    SCIP_EVENTQUEUE *eventqueue, /**< event queue */
...
static SCIP_RETCODE colexUnlink(
    SCIP_COLEX *col,             /**< column data */
    BMS_BLKMEM *blkmem,          /**< block memory */
    SCIP_SET *set,               /**< global SCIP settings */
    SCIP_EVENTQUEUE *eventqueue, /**< event queue */
...
static SCIP_RETCODE rowexUnlink(
    SCIP_ROWEX *row, /**< row data */
    SCIP_SET *set,   /**< global SCIP settings */
    SCIP_LPEX *lp    /**< current LP data */
)
...
static SCIP_RETCODE lpexFlushDelCols(
    SCIP_LPEX *lp /**< current LP data */
)
{
   assert(lp != NULL);
   ...
   while (lp->lpifirstchgcol < lp->nlpicols && lp->lpifirstchgcol < lp->ncols && lp->cols[lp->lpifirstchgcol]->lpipos == lp->lpifirstchgcol && !lp->cols[lp->lpifirstchgcol]->coefchanged)
   {
...
static SCIP_RETCODE lpexFlushAddCols(
    SCIP_LPEX *lp,              /**< current LP data */
    BMS_BLKMEM *blkmem,         /**< block memory */
    SCIP_SET *set,              /**< global SCIP settings */
    SCIP_EVENTQUEUE *eventqueue /**< event queue */
...
static SCIP_RETCODE lpexFlushDelRows(
    SCIP_LPEX *lp,      /**< current LP data */
    BMS_BLKMEM *blkmem, /**< block memory */
    SCIP_SET *set       /**< global SCIP settings */
)
{
   ...
   while (lp->lpifirstchgrow < lp->nlpirows && lp->lpifirstchgrow < lp->nrows && lp->rows[lp->lpifirstchgrow]->lpipos == lp->lpifirstchgrow && !lp->rows[lp->lpifirstchgrow]->coefchanged)
   {
...
static SCIP_RETCODE lpexFlushAddRows(
    SCIP_LPEX *lp,              /**< current LP data */
    BMS_BLKMEM *blkmem,         /**< block memory */
    SCIP_SET *set,              /**< global SCIP settings */
    SCIP_EVENTQUEUE *eventqueue /**< event queue */
...
static SCIP_RETCODE lpexFlushChgCols(
    SCIP_LPEX *lp, /**< current LP data */
    SCIP_SET *set  /**< global SCIP settings */
)
{
...
static SCIP_RETCODE lpexFlushChgRows(
    SCIP_LPEX *lp, /**< current LP data */
    SCIP_SET *set  /**< global SCIP settings */
)
{
...
extern SCIP_RETCODE SCIPcolexCreate(
...
/** frees an LP column */
extern SCIP_RETCODE SCIPcolexFree(
...
/** adds a previously non existing coefficient to an LP column */
extern SCIP_RETCODE SCIPcolexAddCoef(
    SCIP_COLEX *col,             /**< LP column */
    BMS_BLKMEM *blkmem,          /**< block memory */
    SCIP_SET *set,               /**< global SCIP settings */
    SCIP_EVENTQUEUE *eventqueue, /**< event queue */
...
extern SCIP_RETCODE SCIPcolexDelCoef(
    SCIP_COLEX *col,             /**< column to be changed */
    BMS_BLKMEM *blkmem,          /**< block memory */
    SCIP_SET *set,               /**< global SCIP settings */
    SCIP_EVENTQUEUE *eventqueue, /**< event queue */
...
extern SCIP_RETCODE SCIPcolexChgCoef(
    SCIP_COLEX *col,             /**< LP column */
    BMS_BLKMEM *blkmem,          /**< block memory */
    SCIP_SET *set,               /**< global SCIP settings */
    SCIP_EVENTQUEUE *eventqueue, /**< event queue */
...
extern SCIP_RETCODE SCIPcolexIncCoef(
    SCIP_COLEX *col,             /**< LP column */
    BMS_BLKMEM *blkmem,          /**< block memory */
    SCIP_SET *set,               /**< global SCIP settings */
    SCIP_EVENTQUEUE *eventqueue, /**< event queue */
...
extern SCIP_RETCODE SCIPcolexChgObj(
    SCIP_COLEX *col,      /**< LP column to change */
    SCIP_SET *set,        /**< global SCIP settings */
    SCIP_LPEX *lp,        /**< current LP data */
...
)
{
   ...
   if (!RisEqual(col->obj, newobj))
   {
       ...
       */
      else if ((RisNegative(col->obj) && RisPositive(newobj) && RisZero(col->ub)) || (RisPositive(col->obj) && RisNegative(newobj) && RisZero(col->lb)))
      {
...
extern SCIP_RETCODE SCIPcolexChgLb(
    SCIP_COLEX *col,     /**< LP column to change */
    SCIP_SET *set,       /**< global SCIP settings */
    SCIP_LPEX *lp,       /**< current LP data */
...
extern SCIP_RETCODE SCIPcolexChgUb(
    SCIP_COLEX *col,     /**< LP column to change */
    SCIP_SET *set,       /**< global SCIP settings */
    SCIP_LPEX *lp,       /**< current LP data */
...
static SCIP_RETCODE SCIPlpPsdataCreate(
...
static SCIP_RETCODE SCIPlpPsdataFree(
    SCIP_LPEX *lp,     /**< pointer to LP data object */
    SCIP_SET *set,     /**< global SCIP settings */
    BMS_BLKMEM *blkmem /**< block memory buffers */
)
...
extern SCIP_Bool SCIPlpexBSpossible(
    SCIP_LPEX *lp /**< pointer to LP data object */
)
{
   assert(lp != NULL);
...
extern SCIP_Bool SCIPlpexPSpossible(
    SCIP_LPEX *lp /**< pointer to LP data object */
)
{
   assert(lp != NULL);
...
extern SCIP_Bool SCIPlpexIsSynced(
    SCIP_LPEX *lp,        /**< pointer to LP data object */
    SCIP_SET *set,        /**< global SCIP settings */
    SCIP_MESSAGEHDLR *msg /**< message handler */
)
...
extern SCIP_RETCODE SCIPlpexCreate(
    SCIP_LPEX **lp,                /**< pointer to LP data object */
...
)
{
   ...
   for (i = 0; i < rowex->len; ++i)
   {
      ...
      assert((i < rowex->nlpcols) == (rowex->linkpos[i] >= 0 && colex->lppos >= 0));
...
static void getObjvalDeltaObjExact(
    SCIP_SET *set,           /**< global SCIP settings */
...
static void getObjvalDeltaLbExact(
    SCIP_SET *set,           /**< global SCIP settings */
...
static void getObjvalDeltaUbExact(
    SCIP_SET *set,           /**< global SCIP settings */
    SCIP_Rational *obj,      /**< objective value of variable */
...
)
{
   ...
   for (c = 0; c < nlpicols; ++c)
   {
      ...
      {
         stillprimalfeasible =
             (RisNegInfinity(lpicols[c]->lb) || !RisLT(lpicols[c]->primsol, lpicols[c]->lb)) && (RisInfinity(lpicols[c]->ub) || !RisGT(lpicols[c]->primsol, lpicols[c]->ub));
      ...
      /* complementary slackness means that if a variable is not at its lower or upper bound, its reduced costs
         ...
         */
      if (stilldualfeasible && (RisNegInfinity(lpicols[c]->lb) || RisGT(lpicols[c]->primsol, lpicols[c]->lb)))
         stilldualfeasible = !RisPositive(lpicols[c]->redcost);
      if (stilldualfeasible && (RisInfinity(lpicols[c]->ub) || RisLT(lpicols[c]->primsol, lpicols[c]->ub)))
   ...
   for (r = 0; r < nlpirows; ++r)
   {
      ...
      {
         stillprimalfeasible =
             (RisNegInfinity(lpirows[r]->lhs) || RisGE(lpirows[r]->activity, lpirows[r]->lhs)) && (RisInfinity(lpirows[r]->rhs) || RisLE(lpirows[r]->activity, lpirows[r]->rhs));
      }
   ...
   if (stillprimalfeasible && !(RisInfinity(primalbound) && RisInfinity(lp->lpobjval)) && !(RisNegInfinity(primalbound) && RisNegInfinity(lp->lpobjval)))
   {
    ...
    */
   /**@todo alternatively, if otherwise the LP solution is feasible, we could simply update the objective value */
   if (stilldualfeasible && !(RisInfinity(dualbound) && RisInfinity(lp->lpobjval)) && !(RisNegInfinity(dualbound) && RisNegInfinity(lp->lpobjval)))
   {
...
)
{
   ...
   for (r = 0; r < nlpirows; ++r)
   {
      ...
      if (checkfarkas)
      {
          ...
          */
         if ((RisPositive(dualfarkas[r]) && RisNegInfinity(lpirows[r]->lhs)) || (RisNegative(dualfarkas[r]) && RisInfinity(lpirows[r]->rhs)))
         {
