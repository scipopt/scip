...
/*
 * Functions related to a variable's ratio.
 */
   ...
   )
{
   SCIP_Real result = -1;
      ...
      ++branchrule->ncheckratiocalls;
      SCIPclockStart(branchrule->checkratiotime, set);
   #endif

   assert(set != NULL);
   assert(branchrule != NULL);
   assert(branchratio != NULL);
   assert(branchratio->valid == TRUE);
   assert(leftgain <= rightgain);

   /* We evaluate the characteristic polynomial of the variable on the given
    * ratio. If the result is positive, then it has a better ratio. */
   ...
   #ifdef ONEVAR_TIMINGS
      SCIPclockStop(branchrule->checkratiotime, set);
   #endif
   ...
   )
{
    ...
    * If this value if bigger than 100, we use a fixed-point method.
    * Otherwise, we use laguerre's method.
   ...
   usefp = (r > 100);
      ...
      if(usefp)
      {
         ++branchrule->nfpratiocalls;
         SCIPclockStart(branchrule->fpratiotime, set);
      }
      else
      {
         ++branchrule->nlaguerreratiocalls;
   ...
   while(doloop == TRUE && SCIPsetIsEQ(set, ratio, newratio) == FALSE && iters < maxiters)
   {
      ...
      if(usefp)
      {
         assert(SCIPsetIsLE(set, newratio, 2));
         if(newratio > 2) newratio = 2;
         newratio = pow(1.0-1.0/ratio, -1.0/r);
      }
      else
      {
         /* see Numerical Recipes: The Art of Scientific Computing (3rd ed.) for the algorithm
          * or, at the time of writing, wikipedia page on Laguerre's method */
         double G, H, a, p, p1, p2;
         p = pow(ratio, r) - pow(ratio, r-l) - 1;
         if(p != 0)
         {
            p1 = r * pow(ratio, r-1) - (r-l) * pow(ratio, r-l-1);
            p2 = r * (r-1) *  pow(ratio, r-2) - (r-l) * (r-l-1) * pow(ratio, r-l-2);
            G = p1 / p;
            H = G*G - (p2/p);
            a = r / (G + (G>=0?1:-1) * sqrt((r-1) * (r*H-G*G)));
            newratio = ratio - a;
         }       
      }
      assert(SCIPsetIsGE(set, newratio, 1));

      /* The following can happen due to numerical reasons */
      if(newratio <= 1)
      {
         branchratio->valid = FALSE;
         doloop = FALSE;
   ...
   {
      branchratio->valid = TRUE;
      /* We set the unpowered ratio to the average of the two bounds */
      branchratio->upratio = (ratio + newratio)/2;
      branchratio->invleft = 1.0 / leftgain;
     
   }

   /* We (hopefully) make finding bugs easier by setting these values */
   if( branchratio->valid == FALSE )
   {
   ...
   #ifdef ONEVAR_TIMINGS
      if(usefp)
         SCIPclockStop(branchrule->fpratiotime, set);
      else
         SCIPclockStop(branchrule->laguerreratiotime, set);

      SCIPclockStop(branchrule->computeratiotime, set);
   #endif
}
