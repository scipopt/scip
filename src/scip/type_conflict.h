...
typedef struct SCIP_ResolutionSet SCIP_RESOLUTIONSET; /**< set of variables and coefficients describing a resolution-constraint of type lhs <= a^Tx */
typedef struct SCIP_ConflictRow SCIP_CONFLICTROW; /**< conflict row of type lhs <= a^Tx, semi-sparse representation */
typedef struct SCIP_ReasonRow SCIP_REASONROW; /**< reason row of type lhs <= a^Tx, sparse representation */
...
enum SCIP_ConflictType
{
   ...
   SCIP_CONFTYPE_RESOLUTION   = 6                 /**< conflict constraint from resolution */
};
