...
{
   int*                  encodelen;          /**< encoding length of constraints in reflection symmetry data */
   SYM_CONSTYPE*         constypes;          /**< types of constraints */
   SCIP_EXPRHDLR**       consoptypes;        /**< first operator of constraints */
 ...
 *  encoding length, and then their rhs.
 *
 *  result:
 *    < 0: ind1 comes before (is better than) ind2
 *    = 0: both indices have the same value
 ...
 */
static
SCIP_RETCODE getActiveVariablesReflSym(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_VAR***           vars,               /**< pointer to vars array to get active variables for */
   SCIP_Real**           scalars,            /**< pointer to scalars a_1, ..., a_n in linear sum a_1*x_1 + ... + a_n*x_n + c */
   int*                  nvars,              /**< pointer to number of variables and values in vars and vals array */
   ...
   )
{
   ...
   /* compute shift of variables when centered at 0 */
   for (v = 0; v < *nvars; ++v)
   {
...
SCIP_Bool isSimpleConsLinear(
   int                   considx,            /**< index of constrain in relfection symmetry data */
   ...
   )
{
   ...
   consoptype = treeops[treemap[trees[treebegins[considx]]]];
   if ( (SCIP_EXPRHDLR*) SYM_CONSOPTYPE_AND == consoptype ||
      (SCIP_EXPRHDLR*) SYM_CONSOPTYPE_OR == consoptype ||
      (SCIP_EXPRHDLR*) SYM_CONSOPTYPE_XOR == consoptype )
      return TRUE;
 ...
 */
static
SCIP_RETCODE checkLinConssAreIdentical(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_Real*            vals,               /**< coefficients of linear constraint */
   SCIP_Real             rhs,                /**< rhs of linear constraint */
   int*                  varidx,             /**< indices of variables in linear constraint */
   int                   nvars,              /**< number of variables in linear constraint */
   int                   encodelen,          /**< encoding length of linear constraint in reflection symmetry data */
   SYM_NODETYPE*         trees,              /**< trees of reflection symmetry data */
   int*                  treemap,            /**< maps position in trees array to the corresponding position in
                                              *   treecoefs/treevaridx (depending on node type) */
   int*                  treebegins,         /**< array containing begin positions of new tree in trees */
   ...
   SCIP_Bool*            isidentical,        /**< pointer to store whether we have found identifcal constraint */
   int*                  postocons,          /**< for quicker checks: position in sorted list of conss to cons ID */
   int                   posbegin,           /**< begin position of family the constraint to be checked belongs to */
   int                   posend              /**< end position of family the constraint to be checked belongs to */
   )
{
   int pos2;
   int pos;
   ...
   assert( treeconstype != NULL );
   assert( isidentical != NULL );

   *isidentical = FALSE;
   for (pos2 = posbegin; pos2 < posend && ! *isidentical; ++pos2)
   {
      c = postocons[pos2];

      /* skip constraints of different type */
      if ( treeconstype[c] != SYM_CONSTYPE_LINEAR && treeconstype[c] != SYM_CONSTYPE_SIMPLE )
         continue;

      /* skip simple constraints not being represented by linear constraints */
      if ( treeconstype[c] == SYM_CONSTYPE_SIMPLE && ! isSimpleConsLinear(c, trees, treemap, treebegins, treeops) )

      /* skip constraints with different rhs or length */
      if ( (! SCIPisEQ(scip, rhs, treerhs[c])) || treebegins[c + 1] - treebegins[c] != encodelen )
         continue;

      /* check whether each variable occurs with the same coefficient */
      for (i = 0; i < nvars; ++i)
      {
         for (pos = treebegins[c]; pos < treebegins[c + 1]; ++pos)
         {
            if ( trees[pos] == SYM_NODETYPE_COEF )
            {
               assert( pos < treebegins[c + 1] - 1 );
               assert( trees[pos + 1] == SYM_NODETYPE_VAR );

               /* stop if we have found the variable */
               if ( varidx[i] == treevaridx[treemap[pos + 1]] )
               {
                  if ( SCIPisEQ(scip, treecoefs[treemap[pos]], vals[i]) )
                     *isidentical = TRUE;
                  else
                     *isidentical = FALSE;
                  break;
         ...
         if ( pos == treebegins[c + 1] || ! *isidentical )
         {
            *isidentical = FALSE;
            break;
         }
...
SCIP_RETCODE collectInformationPermLinearCons(
   ...
   int*                  perm,               /**< permutation applied to variables in constraint */
   SCIP_Real*            vals,               /**< allocated array to store coefficients of constraint */
   int*                  varidx,             /**< allocated array to store indices of vars in constraint */
   SCIP_Real*            rhs,                /**< pointer to rhs of constraint */
   int*                  nvars,              /**< pointer to store number of variables in constraint */
   ...
   )
{
   ...
   *nvars = 0;
   for (pos = treebegins[considx]; pos < treebegins[considx + 1]; ++pos)
   {
      ...
      if ( trees[pos] == SYM_NODETYPE_COEF )
         vals[*nvars] = treecoefs[treemap[pos]];
      else if ( trees[pos] == SYM_NODETYPE_VAR )
      {
         varidx[*nvars] = perm[treevaridx[treemap[pos]]];
         *nvars += 1;
      }
      else
      {
...
{
   SYM_SORTCONSREFLSYMTYPE consreflsymtype;
   SCIP_CONSHDLR* conshdlr;
   ...
   treecoefs = reflsymdata->treecoefs;
   ...
   SCIPfreeBlockMemoryArrayNull(scip, &(consreflsymtype.encodelen), ntreerhs);
   SCIPfreeBlockMemoryArrayNull(scip, &(consreflsymtype.consoptypes), ntreerhs);

   SCIP_CALL( SCIPallocBufferArray(scip, &vals, maxnvals) );
   SCIP_CALL( SCIPallocBufferArray(scip, &varidx, maxnvals) );
      ...
      for (ct = 0; ct < nconstypes; ++ct)
      {
         for (pos = constypebegins[ct]; pos < constypebegins[ct + 1]; ++pos)
         {
            ...
            if ( treeconstype[c] == SYM_CONSTYPE_LINEAR )
            {
               ...
               lencons = treebegins[c + 1] - treebegins[c];
               assert( lencons <= maxnvals );
            ...
            else if ( reflsymdata->treeconstype[c] == SYM_CONSTYPE_SIMPLE )
            {
               SCIP_EXPRHDLR* consoptype;
               int lencons;

               assert( trees[treebegins[c]] == SYM_NODETYPE_OPERATOR );
               consoptype = treeops[treemap[treebegins[c]]];

               lencons = treebegins[c + 1] - treebegins[c];
               assert( lencons <= maxnvals );

               if ( consoptype == (SCIP_EXPRHDLR*) SYM_CONSOPTYPE_AND ||
                  consoptype == (SCIP_EXPRHDLR*) SYM_CONSOPTYPE_OR ||
                  consoptype == (SCIP_EXPRHDLR*) SYM_CONSOPTYPE_XOR )
               {
                  SCIP_Real permrhs;
                  int nconsvars;
            ...
            {
               /* check nonlinear constraints and the objective differently */
               assert( reflsymdata->treeconstype[c] == SYM_CONSTYPE_EXPR ||
                  reflsymdata->treeconstype[c] == SYM_CONSTYPE_OBJ );
            }
         }
      }
   }
      ...
      for (p = 0; p < nperms && success; ++p)
      {
         ...
         for (i = 0; i < nnonlinconss; ++i)
         {
            ...
            for (j = 0; j < noccuringvars && ! permuted; ++j)
            {
               ...
               thisvaridx = SCIPvarGetProbindex(occuringvars[j]);
               assert( thisvaridx >= 0 && thisvaridx < nvars );

               if ( P[thisvaridx] != thisvaridx )
                  permuted = TRUE;
            }
   ...
   )
{
   ...
   {
      name = SCIPexprhdlrGetName(exprhdlrs[i]);

      /* check whether there are exprhdlr that we don't know how to handle */
      if ( strcmp(name, "abs") == 0 || strcmp(name, "entropy") == 0 ||
         strcmp(name, "erf") == 0 || strcmp(name, "exp") == 0 ||
         strcmp(name, "log") == 0 || strcmp(name, "pow") == 0 ||
         strcmp(name, "signpower") == 0 || strcmp(name, "prod") == 0 ||
         strcmp(name, "sum") == 0 || strcmp(name, "sin") == 0 ||
         strcmp(name, "cos") == 0 || strcmp(name, "val") == 0 ||
         strcmp(name, "var") == 0 || strcmp(name, "varidx") == 0 )
         continue;
      else
         return TRUE;
   }
...
SCIP_RETCODE ensureReflSymDataMemorySuffices(
   ...
   int                   reqtreevals         /**< required size for value indices */
   )
{
   int newsize;
   ...
   if ( reqtreevals > reflsymdata->maxntreevals )
   {
      newsize = SCIPcalcMemGrowSize(scip, reqtreevals);
      SCIP_CALL( SCIPreallocBlockMemoryArray(scip, &reflsymdata->treevals, reflsymdata->maxntreevals, newsize) );
      SCIP_CALL( SCIPreallocBlockMemoryArray(scip, &reflsymdata->validx, reflsymdata->maxntreevals, newsize) );
      reflsymdata->maxntreevals = newsize;
   }
...
SCIP_RETCODE addValReflSym(
   SYM_REFLSYMDATA*     reflsymdata,        /**< pointer to reflection symmetry data structure */
   SCIP_Real            val,                /**< value to be added */
   int                  parent,             /**< index of parent node of coefficient */
   int*                 posintrees          /**< pointer to store position in trees */
   )
{
   ...
   reflsymdata->treeparentidx[reflsymdata->ntrees] = parent;
   reflsymdata->treevals[reflsymdata->ntreevals] = val;
   reflsymdata->validx[reflsymdata->ntreevals++] = reflsymdata->ntrees;
   if ( posintrees != NULL )
      *posintrees = reflsymdata->ntrees;
   ++reflsymdata->ntrees;
   ...
   )
{
   ...
   SCIP_CALL( getActiveVariablesReflSym(scip, &vars, &vals, &nvars, &constant, istransformed) );
   ...
   )
{
   ...
   case SYM_CONSOPTYPE_OR :
      ...
      /* add variables with corresponding coefficients */
      for (i = 0; i < nvars; ++i)
      {
   ...
   case SYM_CONSOPTYPE_XOR :
      ...
      for (i = 0; i < nvars; ++i)
      {
         consvars[i] = SCIPgetVarsXor(scip, cons)[i];
         consvals[i] = 1.0;
      }

      /* integer variable might have been removed, identify it by coefficient 2.0 */
      if ( SCIPgetIntVarXor(scip, cons) != NULL )
      {
         consvars[nvars] = SCIPgetIntVarXor(scip, cons);
         consvals[nvars++] = 2.0;
      }
   ...
   case SYM_CONSOPTYPE_BDDISJ :
       ...
       * If the disjunction is based on an active variable, then the expression
       * is given by (>= val coef var) to encode \f$val \geq coef \cdot var\f$.
       * Note that we normalize bounddisjunction to the ">="-case.
       * Otherwise, it is given by ({<=,>=} val + coef1 auxvar1 ... coefl auxvarl),
       * where val incorporates the constant of the linear expression replacing var.
       *
       ...
       * [+ coef1 auxvar 1 coef2 auxvar2 coef3 auxvar3], which requires 7 positions.
       */
      nvars = SCIPgetNVarsBounddisjunction(scip, cons);
      exprsum = SCIPfindExprhdlr(scip, "sum");

      SCIP_CALL( ensureReflSymDataMemorySuffices(scip, reflsymdata, reflsymdata->ntrees + 7 * nvars + 1,
            reflsymdata->ntreeops + 2 * nvars + 1, reflsymdata->ntreecoefs + 3 * nvars,
      ...
      for (i = 0; i < nvars; ++i)
      {
         ...
         consvars[0] = SCIPgetVarsBounddisjunction(scip, cons)[i];
         consvals[0] = 1.0;
         constant = 0.0;
         ...
         if ( nlocvars == 1 )
         {
            /* ensure that [>= val coef var] fits into the data structure */
            SCIP_CALL( ensureReflSymDataMemorySuffices(scip, reflsymdata, reflsymdata->ntrees + 4,
         ...
         else
         {
            ...
            SCIP_CALL( ensureReflSymDataMemorySuffices(scip, reflsymdata, reflsymdata->ntrees + nlocvars + 3,
            reflsymdata->ntreeops + 2, reflsymdata->ntreecoefs + nlocvars,
   ...
   case SYM_CONSOPTYPE_INDICATOR :
       ...
       * Moreover, if \f$y\f$ is aggregated, it can be replaced by the corresponding sum
       * of binary variables.
       */
   {
      SCIP_CONS* lincons;
   ...
   case SYM_CONSOPTYPE_SOS1 :
      ...
      for (i = 0; i < nvars; ++i)
      {
         ...
         if ( nlocvars == 1 )
         {
            /* ensure that [coef var] or [+ val coef var ] fits into the data structure */
            if ( SCIPisZero(scip, constant) )
            {
...
SCIP_RETCODE ensureOpenidxSize(
   ...
   int                   nopenidx,           /**< number of elements that need to be stored */
   int*                  maxnopenidx         /**< address of maximum number that can be stored */
   )
{
   assert( scip != NULL );
   assert( openidx != NULL );
   ...
   if ( nopenidx >= *maxnopenidx )
   {
      int newsize;

      newsize = SCIPcalcMemGrowSize(scip, *maxnopenidx + 1);
      assert( newsize > *maxnopenidx );
   ...
   )
{
   ...
   *success = FALSE;
   ...
   if ( hasNonhandableExprhdlr(scip) )
   {
      /* we cannot compute symmetries due to unknown exprhdlr */
      *success = FALSE;
      *nperms = -1;
      return SCIP_OKAY;
   }
   ...
   for (c = 0; c < nconss; ++c)
   {
      ...
      else if ( strcmp(conshdlrname, "linking") == 0 )
      {
         ...
         /* get linking variable of constraint */
         consvars[nconsvars] = SCIPgetLinkvarLinking(scip, cons);
         consvals[nconsvars++] = -1.0;

         /* store the linking constraint itself */
         SCIP_CALL( storeLinearConstraint(scip, &reflsymdata, consvars, consvals, nconsvars,
               0.0, 0.0, SCIPconsIsTransformed(cons)) );

         /* also store set partitioning constraint corresponding to linking constraint */
         SCIP_CALL( storeLinearConstraint(scip, &reflsymdata, consvars, consvals, nconsvars - 1,
               1.0, 1.0, SCIPconsIsTransformed(cons)) );
      }
      ...
      {
         SCIP_CALL( storeNonlinearConstraint(scip, &reflsymdata, cons, consvars, consvals) );
      }
      else
      {
