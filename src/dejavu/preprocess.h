...
namespace dejavu {
    ...
    public:
        ...
        // for a vertex v of reduced graph, return whether recovery string contains negative values
        bool uses_aux_recovery(int v) {
            const int orig_v = translate_back(v);
            for(auto& rv : recovery_strings[orig_v])
                if(rv < 0) return true;
            return false;
        }
    ...
    private:
        ...
        void recovery_attached_edges_to_automorphism(int v, int* aut, dejavu::ds::worklist* aut_supp,
            ...
            for(auto & j : recovery_edge_adjacent[v_map]) {
                ...
                if(len == 1) {
                    ...
                    // general-purpose code for longer paths
                    for (int k = 0; k < len; ++k) {
                        dej_assert(orig_id + k < recovery_edge_attached.size());
                        dej_assert(id + k < recovery_edge_attached.size());
                        const int v_from = recovery_edge_attached[orig_id + k];
                        const int v_to   = recovery_edge_attached[id + k];
                        dej_assert(v_from >= 0);
                        dej_assert(v_to >= 0);
                        dej_assert(v_from < domain_size);
                        dej_assert(v_to < domain_size);
                        if (v_from != v_to && aut[v_from] == v_from) {
                            aut[v_from] = v_to;
                            aut_supp->push_back(v_from);
                        }
        ...
        void red_deg2_densify(dejavu::sgraph *g, int *colmap) {
            ...
            int total_paths = 0;
            bool aux_on_deg2 = false;

            for (int i = 0; i < g->v_size; ++i) {
                if(g->d[i] == 2) {
                    if(uses_aux_recovery(i)) {
                        aux_on_deg2 = true;
                        break;
                    }
            ...
            if(total_paths < g->v_size/10 || aux_on_deg2) return;
