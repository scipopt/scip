...
int main (int argc, char const *argv[])
{
   ...
   std::string fixFileName = DEFAULT_FIXFILENAME;
   // read model
   Model* model = new Model;
   MpsInput* mpsi = new MpsInput;

   bool success = mpsi->readMps(argv[1], model);
   std::cout << std::flush << "Read MPS: " << success << std::endl;
   if( !success ) return -1;

   std::cout << "MIP has " << model->numVars() << " vars and " << model->numConss() << " constraints" << std::endl << std::flush;
   ...
   for( int k = 3; k < argc; k++)
   {
      ...
      else if( strncmp(argv[k], "fixFile=", strlen("fixFile=")) == 0 )
      {
         fixFileName = std::string(argv[k] + strlen("fixFile="));
      }
      else
      {
         if( posArg == 0 )
   ...
   if( fixFileName.compare(DEFAULT_FIXFILENAME) != 0 )
   {
      std::cout << "Writing fix bound section to '" << fixFileName << "'" << std::endl;
      std::ofstream outfile (fixFileName.c_str());
      model->printBounds(outfile);
      outfile.close();
   }
   // clean up
   delete mpsi;
