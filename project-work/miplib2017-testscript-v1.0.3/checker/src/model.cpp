...
#define NUNEXPECTEVARS_MAX_MESSAGE 5 // maximum number of unexpected variables that are explicitly printed.
   ...
   while(true)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            if( nunexpectedvars <= NUNEXPECTEVARS_MAX_MESSAGE )
            {
               std::cerr << "unexpected variable<" << varname << "> in solution file" << std::endl << std::flush;
               if( nunexpectedvars == NUNEXPECTEVARS_MAX_MESSAGE )
                  std::cerr << "Further unexpected variables are omitted from the output." << std::endl << std::flush;

            }
         }
      }
   }
   isSolFeas = isSolFeas && hasObjectiveValue;

   if( nunexpectedvars > 0 )
     std::cerr << "Encountered " << nunexpectedvars << " unexpected variables" << std::endl << std::flush;
...
void Model::printBounds(std::ostream& out) const
{
   std::map<std::string, Var*>::const_iterator vitr = vars.begin();
   std::map<std::string, Var*>::const_iterator vend = vars.end();
   ...
   while( vitr != vend )
   {
      if ( vitr->second->type == Var::CONTINUOUS )
      {
         out << " LO bla       " << vitr->second->name << "                " << vitr->second->lb.toDouble() << std::endl;
         out << " UP bla       " << vitr->second->name << "                " << vitr->second->ub.toDouble() << std::endl;
      }
      else
      {
         //out << " LO bla       " << vitr->second->name << "                " << (int)(round(vitr->second->value.toDouble())) << std::endl;
         out << " FX bla       " << vitr->second->name << "                " << (int)(round(vitr->second->value.toDouble())) << std::endl;
      }
      ++vitr;
   }
}
