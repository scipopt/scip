...
Test(entropy, eval, .description = "Tests the expression evaluation.")
{
   ...
   for( i = 0; i < 3; ++i )
   {
      ...
      cr_expect(SCIPisFeasEQ(scip, SCIPgetConsExprExprValue(entropyexpr), results[i]));
   }
...
Test(entropy, inteval, .description = "Tests the expression interval evaluation.")
{
   ...
   for( i = 0; i < 4; ++i )
   {
      ...
      SCIP_CALL( SCIPevalConsExprExprInterval(scip, entropyexpr, FALSE, 0, SCIPepsilon(scip)) );
