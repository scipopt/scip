 ...
 *  In addition, one must specify which python it should run with the option <code>PYTHON=my-python</code>.
 *  The reason for this is that one usually installs PySCIPOpt in a virtual environment, thus only the python will work with the code.
 *
 ...
 *  One should either modify <code>scip/check/scip_runner.py</code> or include the <code>build_model()</code> function into your script to correctly read the options pass by the scripts.
 *
 *  Finally, an example of how to run the tests is
 *  \code
