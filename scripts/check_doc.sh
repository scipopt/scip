...
GREPRESULT=`ls $rEGEXFILE -a | grep "source" -v | xargs grep "@ref" -l | xargs grep "@ref" -c`
...
echo classes of unresolved references:`ls $rEGEXFILE -a | grep "source" -v | xargs grep -hP "@ref [A-Z][^ .<>]*" -o | sort | uniq`