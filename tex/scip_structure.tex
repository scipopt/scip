...
\begin{tikzpicture}[mindmap,concept color=gray!20]
  ...
  \tikzstyle{heurconcepteven} = [level 1 concept, sibling angle=11.3, level distance=3.9cm, concept color=f8, minimum size=.7cm];
  \tikzstyle{heurconceptodd} = [heurconcepteven, level distance=2.9cm, minimum size=.7cm];
  ...
  \tikzstyle{sepaconcept} = [level 2 concept,sibling angle=28,concept color=f5];
  ...
  [clockwise from=-90]
  child[level distance=8.45cm] { node[concept] (Heuristic) {Primal\\Heuristic}
    ...
    child[heurconcepteven] { node[concept] (fracdiving) {fracdiving}}
    ...
    child[heurconceptodd] { node[concept] (vbounds) {vbound}}
...
%   child { node[concept] (Variable) {Variable}}
  ...
  child[level distance=6.8cm] { node[concept] (Presolver) {Presolver}
    [clockwise from=50]
  ...
  child[level distance=8cm] { node[concept] (LP) {LP}
    [clockwise from=-120]
...
%   }
  ...
  child[level distance=8.3cm] { node[concept] (Branch) {Branch}
    [clockwise from=-170]
  ...
  child[level distance=7.8cm] { node[concept] (Separator) {Separator}
    [clockwise from=150]
