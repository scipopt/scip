...
| `SANITIZE=true`       | `[true, false, thread, address]` | sanitizers to enable: SANITIZE not set to false enables undefined-behavior sanitizer, SANITIZE=thread enables thread sanitizer, SANITIZE=address enables address sanitizer; only used if OPT=dbg |
