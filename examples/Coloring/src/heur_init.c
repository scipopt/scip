 ...
 *
 * This file implements a heuristic which computes a starting solution 
 * for the coloring problem. It therefor computes maximal stable sets 
 * and creates one variable for each set that is added to the LP.
 * 
 * The heuristic is called only one time: before solving the root node.
 *
 * It checks, whether a solution-file was read in and there already is a starting solution.
 * If this is not the case, a initial possible coloring is computed by a greedy method.
 * After that, a tabu-search is called, which tries to reduce the number of colors needed.
 * 
 * The tabu-search works as follows: given the graph and a number of colors
 * it tries to color the nodes of the graph with at most that many colors.
 * It starts with a random coloring. In each iteration, it counts the number of violated edges,
 * that is the number of edges, for which both incident nodes have the same color.
 * It now switches one node to another color in each iteration, taking the node and color, 
 ...
 * 
 * The tabu-search can be deactivated by setting the parameter <heuristics/initcol/usetabu> to FALSE,
 * the number of iteration, after which the tabu-search stops if no solution was yet found, can be
