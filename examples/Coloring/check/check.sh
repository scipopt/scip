...
#*                                                                           *
#* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
# $Id: check.sh,v 1.1 2010/09/17 17:03:13 bzfgamra Exp $
TSTNAME=$1
BINNAME=$2
SETNAME=$3
BINID=$4
...
do
    ...
    then
	LASTPROB=""
	if test -f instances/$i
	then
	    echo @01 instances/$i ===========
	    echo @01 instances/$i ===========      >> $ERRFILE
...
#	    waitcplex.sh # ??????????????????
	    ...
	    echo @02 FILE NOT FOUND: instances/$i ===========
	    echo @02 FILE NOT FOUND: instances/$i =========== >>$ERRFILE
	fi
    else
	echo skipping $i
	if test "$LASTPROB" == "$i"
