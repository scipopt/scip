   ...
   )
{
   ...
   else if( SCIPfileExists("sciptsp.set") )
   {
      ...
      CHECK_OKAY( SCIPreadParams(scip, "sciptsp.set") );
   }
   ...
   )
{
   ...
   CHECK_OKAY( SCIPincludeObjReader(scip, new ReaderTSP(scip), TRUE) );
   CHECK_OKAY( SCIPincludeObjConshdlr(scip, new ConshdlrSubtour(), TRUE) ); 
