   ...
   )
{
   ...
   *offset = 0.0;
   ...
   if( g->stp_type == STP_DEG_CONS || g->stp_type == STP_GRID || g->stp_type == STP_OBSTACLES_GRID )
      return SCIP_OKAY;
...
#endif
   if( level == 4 )
      SCIP_CALL( level4(scip, g, offset, minelims) );

   if (level == 5)
      SCIP_CALL( level5(scip, g, offset) );

   if (level == -4)
      *offset = levelm4(scip, g);

   if (level == -1)
      *offset = levelm1(scip, g);
