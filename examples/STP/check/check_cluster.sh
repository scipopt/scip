    ...
    do
        ...
        do
            ...
            # additional environment variables needed by runcluster.sh
                ...
                sbatch --job-name=SCIP$SHORTPROBNAME --mem=$CLUSTERMEMLIMIT -p $CLUSTERQUEUE -A $ACCOUNT $NICE --time=${CLUSTERTIMELIMIT} ${EXCLUSIVE} --output=/dev/null run.sh
            else
                # -V to copy all environment variables
                qsub -l walltime=$CLUSTERTIMELIMIT -l mem=$CLUSTERMEMLIMIT -l nodes=1:ppn=$PPN -N SCIP$SHORTPROBNAME -v SOLVERPATH=$SCIPPATH,EXECNAME=$SCIPPATH/../$BINNAME,BASENAME=$FILENAME,FILENAME=$i,CLIENTTMPDIR=$CLIENTTMPDIR -V -q $CLUSTERQUEUE -o /dev/null -e /dev/null run.sh
            fi
