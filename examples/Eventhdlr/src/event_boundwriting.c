...
#ifdef SCIP_DISABLED_CODE /* We could evaluate the pure impact of (node) presolve and nd run this event handler in subMIPs,
    * the following code block can be enabled. */
...
#ifdef SCIP_DISABLED_CODE /* We could evaluate the pure impact of (node) presolve and
   * cuts on the dual bound for the current node by disabling all heuristics and therefore
   * disregarding any sideeffects that are introduced due to new solutions and their subsequent reductions. */
