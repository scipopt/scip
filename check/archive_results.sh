...
sbatch --dependency=afterok:${jobidsstr} --kill-on-invalid-dep=yes --cpus-per-task=1 --mem=100 --time=2 --partition=opt --account=mip check/rbcliwrapper.sh
...
sbatch --dependency=afternotok:${jobidsstr} --kill-on-invalid-dep=yes --cpus-per-task=1 --mem=100 --time=2 --partition=opt --account=mip --mail-type=BEGIN --mail-user=munoz@zib.de --output=/dev/null --job-name=failed-run <<< "#! /bin/bash"
