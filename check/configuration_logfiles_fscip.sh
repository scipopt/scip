...
THREADS=${11}  # the number of threads
p=${12}      # the index of the current permutation
s=${13}      # shift of the global random seed

OUTFILE=results/check.$TSTNAME.$BINID.$QUEUE.$SETNAME.$THREADS.out
ERRFILE=results/check.$TSTNAME.$BINID.$QUEUE.$SETNAME.$THREADS.err
...
then
    ...
    if test $s -gt 0
    then
        EVALFILE=$SCIPPATH/results/check.$TSTNAME.$BINID.$QUEUE.$SETNAME.$THREADS"-s"$s"-p"$p.eval
    else
        EVALFILE=$SCIPPATH/results/check.$TSTNAME.$BINID.$QUEUE.$SETNAME.$THREADS"-p"$p.eval
    fi
else
    # if number of seeds is positive, add postfix
    if test $s -gt 0
    then
        EVALFILE=$SCIPPATH/results/check.$TSTNAME.$BINID.$QUEUE.$SETNAME.$THREADS"-s"$s.eval
    else
        EVALFILE=$SCIPPATH/results/check.$TSTNAME.$BINID.$QUEUE.$SETNAME.$THREADS.eval
    fi
fi
