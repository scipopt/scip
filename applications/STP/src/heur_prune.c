   ...
   )
{
   ...
   if( incumbentgiven )
   {
      ...
      objnew = graph_sol_getObj(prunegraph, soledge, 0.0, nedges);
   ...
   objnew = graph_sol_getObj(g, edgearrint, 0.0, nedges);
   ...
   )
{
   ...
   *success = TRUE;
   ...
   if( reducegraph )
   {
      if( pc )
         ...
         SCIP_CALL( redLoopMw(scip, prunegraph, vnoi, path, NULL, nodearrreal, cost, costrev, state,
               vbase, nodearrint, nodearrint2, heap, NULL, nodearrchar, &offset, FALSE, FALSE, FALSE, reductbound, FALSE) );
      else
   ...
   if( solgiven )
   {
      ...
      globalobj = graph_sol_getObj(g, soledge, 0.0, nedges);
   ...
   if( annodes > 0 )
   {
      ...
      for( int i = 0; i < PRUNE_MAXREDROUNDS; i++ )
      {
         ...
         if( pc )
            ...
            SCIP_CALL( redLoopMw(scip, prunegraph, vnoi, path, NULL, nodearrreal, cost, costrev, state,
                  vbase, nodearrint, nodearrint2, heap, solnode, nodearrchar, &offset, FALSE, FALSE, FALSE, reductbound, FALSE) );
         else
   ...
   *success = graph_sol_valid(scip, g, globalsoledge);
   ...
   SCIPdebugMessage("obj of best prune sol: %f \n", graph_sol_getObj(g, globalsoledge, 0.0, nedges));
