...
#define HEUR_NAME             "indoneopt"
#define HEUR_DESC             "modified 1-opt heuristic which tries to improve setting of single integer variables"
#define HEUR_DISPCHAR         '6'
...
{
   int                   lastsolindex;       /**< index of the last solution for which indoneopt was performed */
...
/** perform one try of changing a variable */
static
SCIP_RETCODE tryValue(
   ...
   SCIP_VAR*             var,                /**< variable to be changed */
   SCIP_Real             val,                /**< old value of variable */
   SCIP_Real             newval,             /**< new value of variable */
   ...
   )
{
   ...
   /* tentatively set variable to new value */
   SCIP_CALL( SCIPsetSolVal(scip, sol, var, newval) );

   /* try to fix indicators */
#ifdef SCIP_DISABLED_CODE
   SCIP_CALL( SCIPmakeIndicatorsFeasible(scip, heurdata->indicatorconshdlr, sol, &changed) );
#else
...
static
SCIP_DECL_HEURCOPY(heurCopyIndoneopt)
{  /*lint --e{715}*/
   assert( scip != NULL );
   assert( heur != NULL );
...
static
SCIP_DECL_HEURFREE(heurFreeIndoneopt)
{   /*lint --e{715}*/
...
#define heurInitIndoneopt NULL

/** deinitialization method of primal heuristic (called before transformed problem is freed) */
#define heurExitIndoneopt NULL

/** solving process initialization method of primal heuristic (called when branch and bound process is about to begin) */
static
SCIP_DECL_HEURINITSOL(heurInitsolIndoneopt)
{
   SCIP_HEURDATA* heurdata;
   SCIP_CONS** conss;
   SCIP_Bool* isindvar;
...
static
SCIP_DECL_HEUREXITSOL(heurExitsolIndoneopt)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEUREXEC(heurExecIndoneopt)
{  /*lint --e{715}*/
   ...
   *result = SCIP_DELAYED;
   ...
   if ( bestsol != NULL && SCIPsolGetOrigin(bestsol) != SCIP_SOLORIGIN_ORIGINAL )
   {
      ...
      SCIPdebugMessage("Starting indicator 1-opt heuristic ...\n");
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   SCIPdebugMessage("Starting bound adjustment in 1-opt heuristic (indicator version) ... \n");
   ...
   /* check all integer variables and determine rounding locks */
   ncands = 0;
   ...
   if ( ncands > 0 )
   {
      ...
      for (i = 0; i < ncands && i < MAXCANDS; i++)
      {
         ...
         val = SCIPgetSolVal(scip, worksol, var);
         lb = SCIPvarGetLbGlobal(var);
         ub = SCIPvarGetUbGlobal(var);
         assert( SCIPisIntegral(scip, lb) );
         assert( SCIPisIntegral(scip, ub) );

         /* check if we can shift down */
         if ( SCIPisGT(scip, val, lb) )
         {
            ...
            /* try whether setting var to newval improves the objective */
            SCIP_CALL( tryValue(scip, heurdata, bestobj, worksol, var, val, newval, &success) );

            if ( success )
            {
               SCIPdebugMessage("Changed variable <%s> [%d <= %f <= %d] to %d (solvalue: %f, weightvalue: %d, index: %d).\n",
                  SCIPvarGetName(var), (int) lb, val, (int) ub, (int) newval, SCIPgetSolOrigObj(scip, worksol), (int) candvals[i], i);
               bestobj = SCIPgetSolOrigObj(scip, worksol);
   ...
   )
{
   ...
   SCIP_CALL( SCIPincludeHeur(scip, HEUR_NAME, HEUR_DESC, HEUR_DISPCHAR, HEUR_PRIORITY, HEUR_FREQ, HEUR_FREQOFS,
         ...
         heurCopyIndoneopt, heurFreeIndoneopt, heurInitIndoneopt, heurExitIndoneopt,
         heurInitsolIndoneopt, heurExitsolIndoneopt, heurExecIndoneopt,
         heurdata) );

   /* add indoneopt primal heuristic parameters */
   SCIP_CALL( SCIPaddBoolParam(scip, "heuristics/indoneopt/largestweight",
