...
/**@file   reader_mop.c
 * @brief  mop file reader (modified mps file reader)
 * @author Timo Strunk
 */
...
#define READER_NAME             "mop reader"
#define READER_DESC             "reader for mop format"
#define READER_EXTENSION        "mop"
   ...
   )
{
   ...
   while( mpsinputReadLine(mpsi) )
   {
	 ...
	 c_addObjective(probdata->objectives, mpsinputField2(mpsi));
      }
      else
      {
         SCIP_CONS* cons;
...
)
{
   ...
   while( mpsinputReadLine(mpsi) )
   {
      ...
      {
	 /* row is objective */
	 c_addCost(probdata->objectives, var, mpsinputField2(mpsi), val);
      }
      ...
      if( mpsinputField5(mpsi) != NULL )
      {
         ...
         {
            /* row is objective */
	    c_addCost(probdata->objectives, var, mpsinputField4(mpsi), val);
         }
         else if( !SCIPisZero(scip, val) )
         {
            SCIP_CALL( SCIPaddCoefLinear(scip, cons, var, val));
   ...
   )
{
   ...
   probdata = malloc(sizeof(SCIP_PROBDATA));
   probdata->objectives = c_createProbdata(scip);

   SCIP_CALL( SCIPcreateProb(scip, mpsi->probname, NULL, NULL, NULL, NULL, NULL, NULL, probdata) );
...
/** includes the multi objective mps file reader in SCIP */
SCIP_RETCODE SCIPincludeReaderMop(SCIP* scip /**< SCIP data structure */
)
{
