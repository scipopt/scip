...
2. In the directory Ringpacking edit the variable SCIPDIR if necessary - it should
   point to the directory that contains SCIP.

3. Compile the ringpacking example project:
   In the main directory Ringpacking, enter "make OPT=<...> LPS=<...> COMP=<...>"
...
4. To run the program enter "bin/ringpacking.$(OSTYPE).$(ARCH).$(COMP).$(OPT).$(LPS)"
   (e.g. "bin/ringpacking.linux.x86.gnu.opt.spx") or "bin/ringpacking" which is a link
