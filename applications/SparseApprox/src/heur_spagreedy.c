...
)
{
   ...
   for ( c = 0; c < ncluster; ++c )
   {
      ...
      for ( i = 0; i <= c; ++i )
      {
         ...
         if( NULL == absvars[i + ncluster * c] || NULL == absvars[c + ncluster * i] || SCIPvarGetStatus(absvars[i + ncluster * c]) == SCIP_VARSTATUS_MULTAGGR || SCIPvarGetStatus(absvars[c + ncluster * i]) == SCIP_VARSTATUS_MULTAGGR )
            continue;
