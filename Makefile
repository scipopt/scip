...
#*                                                                           *
#* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
# $Id: Makefile,v 1.221 2007/08/28 10:18:12 bzfberth Exp $
...
LPILIBLINKNAME	=	lib$(LPILIBSHORTNAME).$(BASE).$(LIBEXT)
LPILIBLINK	=	$(LIBDIR)/$(LPILIBLINKNAME)
ALLSRC		+=	$(LPILIBSRC)
...
SCIPLIBLINKNAME	=	lib$(SCIPLIBSHORTNAME).$(BASE).$(LIBEXT)
SCIPLIBLINK	=	$(LIBDIR)/$(SCIPLIBLINKNAME)
ALLSRC		+=	$(SCIPLIBSRC)
...
OBJSCIPLIBLINKNAME=	lib$(OBJSCIPLIBSHORTNAME).$(BASE).$(LIBEXT)
OBJSCIPLIBLINK	=	$(LIBDIR)/$(OBJSCIPLIBLINKNAME)
...
MAINLINKNAME	=	$(MAINSHORTNAME).$(BASE).$(LPS)$(EXEEXTENSION)
MAINLINK	=	$(BINDIR)/$(MAINLINKNAME)
MAINSHORTLINKNAME=	$(MAINSHORTNAME)$(EXEEXTENSION)
MAINSHORTLINK	=	$(BINDIR)/$(MAINSHORTLINKNAME)
ALLSRC		+=	$(MAINSRC)
...
$(LPILIBLINK):	$(LPILIBFILE)
		@rm -f $@
		cd $(LIBDIR) && ln -s $(LPILIBFILENAME) $(LPILIBLINKNAME)

$(SCIPLIBLINK):	$(SCIPLIBFILE)
		@rm -f $@
		cd $(LIBDIR) && ln -s $(SCIPLIBFILENAME) $(SCIPLIBLINKNAME)

$(OBJSCIPLIBLINK):	$(OBJSCIPLIBFILE)
		@rm -f $@
		cd $(LIBDIR) && ln -s $(OBJSCIPLIBFILENAME) $(OBJSCIPLIBLINKNAME)

$(MAINLINK):	$(MAINFILE)
		@rm -f $@
		cd $(BINDIR) && ln -s $(MAINFILENAME) $(MAINLINKNAME)

$(MAINSHORTLINK):	$(MAINFILE)
		@rm -f $@
		cd $(BINDIR) && ln -s $(MAINFILENAME) $(MAINSHORTLINKNAME)
